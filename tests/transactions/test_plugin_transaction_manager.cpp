...
public:
    MockTransactionParticipant(const QString& id, bool should_fail = false)
        : m_id(id), m_should_fail(should_fail), m_prepared(false), m_committed(false), m_aborted(false) {}

    qtplugin::expected<void, PluginError> prepare(const QString& transaction_id) override {
        Q_UNUSED(transaction_id)
        if (m_should_fail) {
            return qtplugin::unexpected(PluginError{PluginErrorCode::ExecutionFailed, "Simulated prepare failure"});
        }
        m_prepared = true;
        return {};
    }
    ...
    qtplugin::expected<void, PluginError> commit(const QString& transaction_id) override {
        Q_UNUSED(transaction_id)
        if (m_should_fail) {
            return qtplugin::unexpected(PluginError{PluginErrorCode::ExecutionFailed, "Simulated commit failure"});
        }
        if (!m_prepared) {
            return qtplugin::unexpected(PluginError{PluginErrorCode::InvalidState, "Not prepared"});
        }
        m_committed = true;
        return {};
    }
...
private:
    ...
    TransactionOperation createTestOperation(const QString& plugin_id, const QString& method);
...
void TestPluginTransactionManager::testTransactionTimeout() {
    ...
    auto result = m_manager->begin_transaction(
        IsolationLevel::ReadCommitted, std::chrono::milliseconds(100));
    QVERIFY(result.has_value());
    QString transaction_id = result.value();
    
    // Setup signal spy for timeout
    QSignalSpy timeout_spy(m_manager, &PluginTransactionManager::transaction_timeout);
...
void TestPluginTransactionManager::testTransactionSavepoints() {
    ...
    auto rollback_result = m_manager->rollback_to_savepoint(transaction_id, "sp1");
    QVERIFY(rollback_result.has_value());
...
void TestPluginTransactionManager::testRegisterParticipant() {
    ...
    auto result = m_manager->register_participant("test_participant", participant);
    QVERIFY(result.has_value());
    ...
    auto duplicate_result = m_manager->register_participant("test_participant", participant);
...
void TestPluginTransactionManager::testPreparePhaseFailure() {
    ...
    auto participant1 = createMockParticipant("participant1");
    auto participant2 = createMockParticipant("participant2", true); // Will fail
...
void TestPluginTransactionManager::testReadCommitted() {
    ...
    auto context_result = m_manager->get_transaction(transaction_id);
    QVERIFY(context_result.has_value());
    QCOMPARE(context_result.value().isolation_level(), IsolationLevel::ReadCommitted);
...
void TestPluginTransactionManager::testSerializable() {
    ...
    auto context_result = m_manager->get_transaction(transaction_id);
    QVERIFY(context_result.has_value());
    QCOMPARE(context_result.value().isolation_level(), IsolationLevel::Serializable);
...
void TestPluginTransactionManager::testInvalidTransactionId() {
    ...
    QVERIFY(!rollback_result.has_value());
    QCOMPARE(rollback_result.error().code, PluginErrorCode::TransactionNotFound);
}
...
void TestPluginTransactionManager::testLargeTransactions() {
    ...
    // Add many operations
    for (int i = 0; i < 100; ++i) {
        auto operation = createTestOperation(QString("plugin_%1").arg(i), "test_method");
        auto add_result = m_manager->add_operation(transaction_id, operation);
        QVERIFY(add_result.has_value());
    }
...
std::shared_ptr<MockTransactionParticipant> TestPluginTransactionManager::createMockParticipant(
    const QString& id, bool should_fail) {
    
    auto participant = std::make_shared<MockTransactionParticipant>(id, should_fail);
...
TransactionOperation TestPluginTransactionManager::createTestOperation(
    ...
    operation.execute_func = []() -> qtplugin::expected<QJsonObject, PluginError> {
        QJsonObject result;

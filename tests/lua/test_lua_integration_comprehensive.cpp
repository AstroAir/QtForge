/**
 * @file test_lua_integration_comprehensive.cpp
 * @brief Comprehensive Lua integration tests for QtForge
 * @version 3.2.0
 */
...
class TestLuaIntegrationComprehensive : public QObject
{
    Q_OBJECT
...
void TestLuaIntegrationComprehensive::initTestCase()
{
    qDebug() << "Starting comprehensive Lua integration tests";
    QVERIFY(m_temp_dir.isValid());
    
    // Create the comprehensive test script
    m_lua_test_script_path = m_temp_dir.path() + "/test_comprehensive_lua_suite.lua";
    
    // Copy our comprehensive test script
    QFile source(":/tests/lua/test_comprehensive_lua_suite.lua");
    if (!source.exists()) {
...
return 0
        ...
        createTestScript("test_comprehensive_lua_suite.lua", basicScript);
    }
}

void TestLuaIntegrationComprehensive::cleanupTestCase()
{
    qDebug() << "Comprehensive Lua integration tests completed";
}

void TestLuaIntegrationComprehensive::init()
{
    // Setup for each test
}

void TestLuaIntegrationComprehensive::cleanup()
{
    // Cleanup after each test
}

void TestLuaIntegrationComprehensive::testLuaAvailability()
{
...
void TestLuaIntegrationComprehensive::testLuaScriptExecution()
{
    if (!isLuaAvailable()) {
...
void TestLuaIntegrationComprehensive::testLuaErrorHandling()
{
#ifdef QTFORGE_LUA_BINDINGS
...
void TestLuaIntegrationComprehensive::testQtForgeModuleAvailability()
{
#ifdef QTFORGE_LUA_BINDINGS
    if (!qtforge_lua::get_lua_state()) {
        QSKIP("Lua state not available");
...
void TestLuaIntegrationComprehensive::testQtForgeCoreBindings()
{
#ifdef QTFORGE_LUA_BINDINGS
    if (!qtforge_lua::get_lua_state()) {
        QSKIP("Lua state not available");
...
void TestLuaIntegrationComprehensive::testLuaPerformanceBaseline()
{
#ifdef QTFORGE_LUA_BINDINGS
    if (!qtforge_lua::get_lua_state()) {
        QSKIP("Lua state not available");
...
void TestLuaIntegrationComprehensive::createTestScript(const QString& filename, const QString& content)
{
...
bool TestLuaIntegrationComprehensive::executeLuaScript(const QString& script_path, QString& output, QString& error)
{
...
bool TestLuaIntegrationComprehensive::isLuaAvailable()
{
#ifdef QTFORGE_LUA_BINDINGS
...
QTEST_MAIN(TestLuaIntegrationComprehensive)
#include "test_lua_integration_comprehensive.moc"

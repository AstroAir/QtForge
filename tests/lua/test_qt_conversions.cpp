...
void TestQtConversions::testJsonArrayConversion() {
    ...
    sol::table lua_table = lua_obj.as<sol::table>();
    QCOMPARE(lua_table[1].get<std::string>(),
             std::string("item1"));  // Lua arrays are 1-indexed
...
void TestQtConversions::testQStringListToLua() {
    ...
    sol::object lua_obj =
        qtforge_lua::qstringlist_to_lua(string_list, *m_lua_state);
    QVERIFY(lua_obj.get_type() == sol::type::table);
    ...
    sol::object lua_empty =
        qtforge_lua::qstringlist_to_lua(empty_list, *m_lua_state);
...
void TestQtConversions::testVariantTypes() {
    ...
    sol::object lua_int =
        qtforge_lua::qvariant_to_lua(int_variant, *m_lua_state);
    QVERIFY(lua_int.get_type() == sol::type::number);
    QCOMPARE(lua_int.as<int>(), 42);

    QVariant string_variant = QString("test string");
    sol::object lua_string =
        qtforge_lua::qvariant_to_lua(string_variant, *m_lua_state);
    ...
    sol::object lua_bool =
        qtforge_lua::qvariant_to_lua(bool_variant, *m_lua_state);
    QVERIFY(lua_bool.get_type() == sol::type::boolean);
    QCOMPARE(lua_bool.as<bool>(), true);

    QVariant invalid_variant;
    sol::object lua_invalid =
        qtforge_lua::qvariant_to_lua(invalid_variant, *m_lua_state);
...
void TestQtConversions::testNestedObjects() {
    ...
    QCOMPARE(nested_table["inner_key"].get<std::string>(),

...
void TestResourceManagement::testResourceManagerCreation() {
    ...
    QVERIFY(
        m_resource_manager->is_resource_type_supported(ResourceType::Thread));
    QVERIFY(
        m_resource_manager->is_resource_type_supported(ResourceType::Timer));
    QVERIFY(
        m_resource_manager->is_resource_type_supported(ResourceType::Memory));
}
...
void TestResourceManagement::testResourcePoolCreation() {
    ...
    auto result = m_resource_manager->create_pool(ResourceType::Thread,
                                                  "test_thread_pool", quota);
    QVERIFY(result.has_value());

    // Test duplicate pool creation
    auto duplicate_result = m_resource_manager->create_pool(
        ResourceType::Thread, "test_thread_pool", quota);
    ...
    auto remove_nonexistent =
        m_resource_manager->remove_pool("nonexistent_pool");
...
void TestResourceManagement::testResourceQuotas() {
    ...
    auto set_result = m_resource_manager->set_plugin_quota(
        "test_plugin", ResourceType::Memory, quota);
    QVERIFY(set_result.has_value());

    auto get_result = m_resource_manager->get_plugin_quota(
        "test_plugin", ResourceType::Memory);
...
void TestResourceManagement::testLifecycleTracking() {
    ...
    auto unregister_result =
        m_lifecycle_manager->unregister_resource("test_resource");
...
void TestResourceManagement::testStateTransitions() {
    ...
    auto update_result = m_lifecycle_manager->update_state(
        "test_resource", LifecycleState::Initialized);
    ...
    m_lifecycle_manager->update_state("test_resource",
                                      LifecycleState::Destroyed);
    auto invalid_result = m_lifecycle_manager->update_state(
        "test_resource", LifecycleState::Active);
...
void TestResourceManagement::testDependencyManagement() {
    ...
    auto remove_result =
        m_lifecycle_manager->remove_dependency("resource2", "resource1");
    QVERIFY(remove_result.has_value());
...
void TestResourceManagement::testLifecycleEvents() {
    ...
    QCOMPARE(arguments.at(1).toInt(),
             static_cast<int>(LifecycleState::Created));
    QCOMPARE(arguments.at(2).toInt(),
             static_cast<int>(LifecycleState::Initialized));
}
...
void TestResourceManagement::testMetricsCollection() {
    ...
    auto access_result = m_resource_monitor->record_access(
        "test_resource", std::chrono::milliseconds(100));
    QVERIFY(access_result.has_value());

    // Test error recording
    auto error_result =
        m_resource_monitor->record_error("test_resource", "Test error message");
...
void TestResourceManagement::testQuotaViolations() {
    ...
    auto quota_result = m_resource_monitor->set_custom_quota(
        "test_plugin", ResourceType::Memory, "max_memory", 1024.0);
    QVERIFY(quota_result.has_value());

    // Test quota compliance checking
    auto violations = m_resource_monitor->check_quota_compliance(
        "test_plugin", ResourceType::Memory);
...
void TestResourceManagement::testDataExport() {
    ...
    // Test JSON export
    auto json_result =
        m_resource_monitor->export_metrics("json", hour_ago, now);
    ...
    // Test invalid format
    auto invalid_result =
        m_resource_monitor->export_metrics("invalid", hour_ago, now);
    QVERIFY(!invalid_result.has_value());
    QCOMPARE(invalid_result.error().code, PluginErrorCode::InvalidArgument);
}
...
void TestResourceManagement::testCompleteResourceLifecycle() {
    ...
    m_lifecycle_manager->update_state("complete_test",
                                      LifecycleState::Initialized);
    m_lifecycle_manager->update_state("complete_test", LifecycleState::Active);

    // 4. Record some activity
    m_resource_monitor->record_access("complete_test",
...
void TestResourceManagement::testResourceCleanupOnPluginUnload() {
    ...
    for (int i = 0; i < 5; ++i) {
        ResourceHandle handle("plugin_resource_" + std::to_string(i),
                              ResourceType::Thread, "test_plugin");
        m_lifecycle_manager->register_resource(handle, LifecycleState::Active);
        m_resource_monitor->start_monitoring(handle);
    }

    // Verify resources are tracked
    auto resources_in_active =
        m_lifecycle_manager->get_resources_in_state(LifecycleState::Active);
    QCOMPARE(resources_in_active.size(), size_t(5));

    // Cleanup all resources for the plugin
    auto cleaned_count =
        m_lifecycle_manager->cleanup_plugin_resources("test_plugin");
    QCOMPARE(cleaned_count, size_t(5));

    // Verify resources are cleaned up
    auto remaining_resources =
        m_lifecycle_manager->get_resources_in_state(LifecycleState::Active);

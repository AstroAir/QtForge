...
void TestPythonBindings::initTestCase() {
    ...
    QStringList python_names = {"python3", "python", "python3.8", "python3.9", "python3.10", "python3.11"};
...
try:
    ...
    QString result = runPythonScript(script);
    QVERIFY(result.contains("SUCCESS: Plugin interface binding test completed"));
}
...
try:
    ...
    QString result = runPythonScript(script);
    QVERIFY(result.contains("SUCCESS: Security manager binding test completed"));
}
...
void TestPythonBindings::createTestPythonScript(const QString& filename, const QString& content) {

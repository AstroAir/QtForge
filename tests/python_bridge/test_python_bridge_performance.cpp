...
void TestPythonBridgePerformance::cleanupTestCase()
{
    delete m_tempDir;
}
...
{
    QBENCHMARK {
        auto bridge = std::make_unique<qtplugin::PythonPluginBridge>(m_perfPluginPath);
...
void TestPythonBridgePerformance::testManyMethodCalls()
{
    ...
    for (int i = 0; i < numCalls; ++i) {
        auto result = m_bridge->invoke_method("increment_counter", QVariantList());
        if (result.has_value()) {
            successCount++;
        }
    }
...
void TestPythonBridgePerformance::testHighFrequencyEvents()
{
    ...
    qDebug() << "Average time per event:" << (double)elapsed / numEvents << "ms";
}
...
void TestPythonBridgePerformance::testConcurrentAccess()
{
    ...
    qDebug() << "Concurrent test:" << totalSuccess << "calls in" << elapsed << "ms";
...
void TestPythonBridgePerformance::testResourceCleanup()
{
    ...
    qDebug() << "Completed" << cycles << "init/cleanup cycles in" << elapsed << "ms";

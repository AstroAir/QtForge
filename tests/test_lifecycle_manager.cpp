...
public:
    ...
    qtplugin::expected<void, PluginError> initialize() override {
        if (m_initialization_should_fail) {
            return make_error<void>(PluginErrorCode::InitializationFailed, "Mock initialization failure");
        }
...
void TestLifecycleManager::testPluginStateMachine() {
    ...
    QVERIFY(!result.has_value());
    QCOMPARE(state_machine.current_state(), PluginState::Running); // Should remain unchanged
...
void TestLifecycleManager::testLifecycleManagerBasic() {
    ...
    QVERIFY(std::find(registered_plugins.begin(), registered_plugins.end(), "test.basic") != registered_plugins.end());
...
void TestLifecycleManager::testStateTransitions() {
    ...
    QVERIFY(PluginStateMachine::is_valid_transition(PluginState::Unloaded, PluginState::Loading));
    QVERIFY(PluginStateMachine::is_valid_transition(PluginState::Loading, PluginState::Loaded));
    QVERIFY(PluginStateMachine::is_valid_transition(PluginState::Loaded, PluginState::Initializing));
    QVERIFY(PluginStateMachine::is_valid_transition(PluginState::Initializing, PluginState::Running));
    QVERIFY(PluginStateMachine::is_valid_transition(PluginState::Running, PluginState::Paused));
    QVERIFY(PluginStateMachine::is_valid_transition(PluginState::Paused, PluginState::Running));
    QVERIFY(PluginStateMachine::is_valid_transition(PluginState::Running, PluginState::Stopping));
    QVERIFY(PluginStateMachine::is_valid_transition(PluginState::Stopping, PluginState::Stopped));
    QVERIFY(PluginStateMachine::is_valid_transition(PluginState::Stopped, PluginState::Unloaded));

    // Test invalid transitions
    QVERIFY(!PluginStateMachine::is_valid_transition(PluginState::Unloaded, PluginState::Running));
    QVERIFY(!PluginStateMachine::is_valid_transition(PluginState::Running, PluginState::Loading));
    QVERIFY(!PluginStateMachine::is_valid_transition(PluginState::Stopped, PluginState::Running));
}

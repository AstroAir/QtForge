...
def validate_test_structure():
    ...
    if test_files_in_root:
        warnings.append(f"Found {len(test_files_in_root)} test files in root directory")
    ...
    if not main_cmake.exists():
        ...
        for module in expected_modules:
            if f"add_subdirectory({module})" not in content:
                warnings.append(f"Module {module} not included in main CMakeLists.txt")
    
    # Summary
    print("\n📊 Validation Summary:")
    print(f"✅ Modules found: {len([m for m in expected_modules if (test_dir / m).exists()])}/{len(expected_modules)}")

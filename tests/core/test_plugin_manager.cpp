...
private:
    ...
    // Helper methods
    void createMockPlugin(const QString& name,
...
void TestPluginManager::testPluginManagerDestruction() {
    // Test that destruction properly cleans up resources
    {
        ...
        QVERIFY(!load_result.has_value());  // Should fail because it's not a
                                            // valid plugin library
...
void TestPluginManager::testPluginManagerInitialization() {
    ...
    QVERIFY(std::find(search_paths.begin(), search_paths.end(), m_plugin_dir) !=
            search_paths.end());
}
...
void TestPluginManager::testLoadValidPlugin() {
    ...
    // Try to load a JSON file as a plugin (should fail because it's not a valid
    // plugin library)
    auto result = m_plugin_manager->load_plugin(getPluginPath("valid_plugin"));
    QVERIFY(!result.has_value());  // Should fail because it's not a valid
                                   // plugin library
...
void TestPluginManager::createMockPlugin(const QString& name,
    ...
    QString plugin_path = QString::fromStdString(
        (m_plugin_dir / (name.toStdString() + ".json")).string());

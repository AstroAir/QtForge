...
public:
    ...
    std::string description() const override {
        return "Mock plugin for testing dynamic interfaces";
    }
    ...
    std::vector<InterfaceDescriptor> get_interface_descriptors()
        const override {
    ...
    bool supports_interface(
        ...
        auto it = std::find_if(
            m_interface_descriptors.begin(), m_interface_descriptors.end(),
    ...
    qtplugin::expected<void, PluginError> adapt_to_interface(
        ...
        if (!supports_interface(interface_id, target_version)) {
            return make_error<void>(
                PluginErrorCode::InterfaceNotSupported,
    ...
    std::vector<InterfaceCapability> negotiate_capabilities(
        ...
        for (const auto& requested_cap : requested) {
            ...
            for (const auto& desc : m_interface_descriptors) {
                auto it = std::find_if(
                    desc.capabilities.begin(), desc.capabilities.end(),
...
void TestDynamicPluginInterface::testInterfaceCapabilityJsonSerialization() {
    ...
    QCOMPARE(json["metadata"].toObject()["type"].toString(),
             QString("processing"));
}
...
void TestDynamicPluginInterface::testInterfaceCapabilityJsonDeserialization() {
    ...
    if (result.has_value()) {
        ...
        QVERIFY(capability.required);
        QCOMPARE(capability.metadata["category"].toString(),
                 QString("validation"));
    }
}
...
void TestDynamicPluginInterface::testInterfaceDescriptorJsonSerialization() {
    ...
    QCOMPARE(json["description"].toString(),
             QString("JSON serialization test"));
...
void TestDynamicPluginInterface::testInterfaceAdaptation() {
    ...
    auto result =
        m_plugin->adapt_to_interface("IDataProcessor", Version(1, 2, 0));
    QTFORGE_VERIFY_SUCCESS(result);
    QVERIFY(m_plugin->isAdaptedTo("IDataProcessor"));

    // Test adaptation to unsupported interface
    auto invalid_result =
        m_plugin->adapt_to_interface("IUnsupported", Version(1, 0, 0));
    QTFORGE_VERIFY_ERROR(invalid_result,
                         PluginErrorCode::InterfaceNotSupported);

    // Test adaptation to version that's too new
    auto version_result =
        m_plugin->adapt_to_interface("IDataProcessor", Version(3, 0, 0));
    QTFORGE_VERIFY_ERROR(version_result,
                         PluginErrorCode::InterfaceNotSupported);
}
...
void TestDynamicPluginInterface::testAdaptationErrors() {
    ...
    auto result =
        m_plugin->adapt_to_interface("INonExistent", Version(1, 0, 0));
    QTFORGE_VERIFY_ERROR(result, PluginErrorCode::InterfaceNotSupported);

    // Test adaptation with incompatible version
    auto version_result =
        m_plugin->adapt_to_interface("IDataProcessor", Version(10, 0, 0));
    QTFORGE_VERIFY_ERROR(version_result,
                         PluginErrorCode::InterfaceNotSupported);
}

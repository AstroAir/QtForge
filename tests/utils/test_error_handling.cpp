...
private:
    ...
    void verify_error(const PluginError& error, PluginErrorCode expected_code,
...
void TestErrorHandling::testPluginErrorCreation() {
    ...
    PluginError error1(PluginErrorCode::InvalidArgument,
    ...
    PluginError error2(PluginErrorCode::FileNotFound, "File not found",
                       "path/to/file.dll");
    ...
    // Test error creation with details
    PluginError error3(PluginErrorCode::LoadFailed, "Load failed",
                       "plugin.dll");
...
void TestErrorHandling::testPluginErrorCopy() {
    PluginError original(PluginErrorCode::InvalidFormat, "Invalid format",
                         "JSON parsing error");
...
void TestErrorHandling::testPluginErrorMove() {
    PluginError original(PluginErrorCode::PermissionDenied, "Permission denied",
    ...
    PluginError source(PluginErrorCode::NetworkError, "Network error",
                       "Connection timeout");
...
void TestErrorHandling::testPluginErrorCodes() {
    ...
    std::set<PluginErrorCode> codes = {PluginErrorCode::Success,
...
void TestErrorHandling::testErrorCodeToString() {
    ...
    QCOMPARE(error_code_to_string(PluginErrorCode::InvalidArgument),
             "InvalidArgument");
    QCOMPARE(error_code_to_string(PluginErrorCode::FileNotFound),
             "FileNotFound");
    QCOMPARE(error_code_to_string(PluginErrorCode::LoadFailed), "LoadFailed");
    QCOMPARE(error_code_to_string(PluginErrorCode::SecurityViolation),
             "SecurityViolation");
}
...
void TestErrorHandling::testExpectedError() {
    ...
    auto result =
        make_error<int>(PluginErrorCode::InvalidArgument, "Invalid input");
    QVERIFY(!result.has_value());
    QVERIFY(result.has_error());
...
void TestErrorHandling::testExpectedValueAccess() {
    ...
    auto error_result =
        make_error<int>(PluginErrorCode::UnknownError, "Test error");
...
void TestErrorHandling::testMakeError() {
    ...
    auto simple_error =
        make_error<int>(PluginErrorCode::InvalidArgument, "Simple error");
    QVERIFY(!simple_error.has_value());
    verify_error(simple_error.error(), PluginErrorCode::InvalidArgument,
                 "Simple error");

    auto detailed_error = make_error<std::string>(
        PluginErrorCode::FileNotFound, "File not found", "path/to/file");
    QVERIFY(!detailed_error.has_value());
    verify_error(detailed_error.error(), PluginErrorCode::FileNotFound,
                 "File not found");
...
void TestErrorHandling::testErrorChaining() {
    ...
    auto inner_error =
        make_error<int>(PluginErrorCode::FileNotFound, "Inner error");
    auto outer_error =
        make_error<int>(PluginErrorCode::LoadFailed, "Outer error");
...
void TestErrorHandling::testErrorToString() {
    PluginError error(PluginErrorCode::InvalidArgument, "Test error message",
                      "Additional details");
...
expected<int, PluginError> TestErrorHandling::divide(int a, int b) {
    if (b == 0) {
        return make_error<int>(PluginErrorCode::InvalidArgument,
                               "Division by zero");
    }
    return make_success<int>(a / b);
}

expected<std::string, PluginError> TestErrorHandling::process_string(
...
void TestErrorHandling::verify_error(const PluginError& error,
                                     PluginErrorCode expected_code,
                                     const std::string& expected_message) {

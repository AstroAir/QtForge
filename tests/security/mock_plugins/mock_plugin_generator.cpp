...
MockPluginGenerator::MockPluginGenerator(const QString& output_dir, QObject* parent)
...
MockPluginGenerator::~MockPluginGenerator() {
    cleanup();
}
...
QString MockPluginGenerator::createLongRunningPlugin(const QString& name, int duration_seconds) {
    QString script_content = R"(#!/usr/bin/env python3
import sys
import time
import json
...
QString MockPluginGenerator::createFileAccessPlugin(const QString& name, const QStringList& file_paths) {
    QString script_content = R"(#!/usr/bin/env python3
import sys
import os
import json
...
QString MockPluginGenerator::createNetworkAccessPlugin(const QString& name, const QStringList& hosts) {
    QString script_content = R"(#!/usr/bin/env python3
import sys
import json
...
QStringList MockPluginGenerator::createTestSuite(const QString& suite_name) {
    ...
    QStringList test_files = {
        "/etc/passwd",
        "/tmp/test_file.txt",
        QStandardPaths::writableLocation(QStandardPaths::TempLocation) + "/sandbox_test.txt"
    };
    plugins.append(createFileAccessPlugin(suite_name + "_files", test_files));
    
    // Network access tests
    QStringList test_hosts = {
        "example.com",
        "google.com",
        "localhost"
    };
    plugins.append(createNetworkAccessPlugin(suite_name + "_network", test_hosts));
...
QString MockPluginGenerator::createPythonPlugin(const QString& name, const QString& script_content) {

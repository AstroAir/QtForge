...
void TestPluginSandbox::cleanupTestCase() {
    delete m_temp_dir;
}
...
void TestPluginSandbox::testSecurityPolicyJsonSerialization() {
    ...
    QCOMPARE(deserialized.permissions.allow_file_system_read, original.permissions.allow_file_system_read);
    QCOMPARE(deserialized.permissions.allow_network_access, original.permissions.allow_network_access);
}
...
void TestPluginSandbox::testSandboxExecutePlugin() {
    ...
    auto exec_result = sandbox->execute_plugin("/non/existent/plugin", PluginType::Native);
...
void TestPluginSandbox::testSandboxManagerCreateSandbox() {
    ...
    QString sandbox_id = "test_sandbox_" + QString::number(QDateTime::currentMSecsSinceEpoch());
...
void TestPluginSandbox::testInvalidPluginPath() {
    ...
    auto result2 = sandbox->execute_plugin("/path/that/does/not/exist", PluginType::Native);
...
void TestPluginSandbox::testDuplicateSandboxId() {
    ...
    QString sandbox_id = "duplicate_test_" + QString::number(QDateTime::currentMSecsSinceEpoch());
...
void TestPluginSandbox::createTestPlugin() {
    ...
    if (temp_file.open()) {
        ...
        QFile::setPermissions(m_test_plugin_path, 

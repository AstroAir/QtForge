...
private:
    ...
    void printMetrics(const QString& test_name,
...
void TestSandboxPerformance::cleanupTestCase() { delete m_temp_dir; }
...
void TestSandboxPerformance::cleanup() { cleanupCreatedSandboxes(); }
...
void TestSandboxPerformance::testPluginExecutionOverhead() {
    SecurityPolicy policy = createLightweightPolicy();
    QString sandbox_id =
        QString("perf_test_%1").arg(QDateTime::currentMSecsSinceEpoch());
    ...
    for (int i = 0; i < iterations; ++i) {
        ...
        auto exec_result =
            sandbox->execute_plugin(plugin_path, PluginType::Python);
...
void TestSandboxPerformance::testMultipleSandboxPerformance() {
    ...
    for (int i = 0; i < sandbox_count; ++i) {
        QString sandbox_id = QString("multi_perf_%1_%2")
                                 .arg(i)
    ...
    // Performance expectations
    QVERIFY(creation_metrics.average() <
            100.0);  // Less than 100ms average creation
    QVERIFY(retrieval_metrics.average() <
            5.0);  // Less than 5ms average retrieval
}
...
void TestSandboxPerformance::testConcurrentExecutionPerformance() {
    ...
    for (int t = 0; t < thread_count; ++t) {
        threads.emplace_back(
            [this, t, operations_per_thread, policy, &thread_metrics]() {
    ...
    for (const auto& metrics : thread_metrics) {
        for (int i = 0; i < metrics.count; ++i) {
            ...
            overall_metrics.add_measurement(
                static_cast<qint64>(metrics.average()));
        }
    }
    ...
    QVERIFY(overall_metrics.average() <
            200.0);  // Less than 200ms average per operation
}

void TestSandboxPerformance::testSandboxMemoryFootprint() {
    // This is a simplified memory test - in a real scenario you'd use more
    // sophisticated tools
    ...
    size_t baseline_memory =
        0;  // Would need platform-specific memory measurement

    // Create sandboxes and measure memory growth
    for (int i = 0; i < sandbox_count; ++i) {
        QString sandbox_id = QString("memory_test_%1_%2")
                                 .arg(i)
                                 .arg(QDateTime::currentMSecsSinceEpoch());
...
void TestSandboxPerformance::testPluginExecutionThroughput() {
    SecurityPolicy policy = createLightweightPolicy();
    QString sandbox_id =
        QString("throughput_test_%1").arg(QDateTime::currentMSecsSinceEpoch());
    ...
    if (successful_executions > 0) {
        double throughput =
            (static_cast<double>(successful_executions) / total_time) *
            1000.0;  // executions per second
        qDebug() << "Plugin execution throughput:" << throughput
                 << "executions/second";
        qDebug() << "Total time for" << successful_executions
                 << "executions:" << total_time << "ms";
...
void TestSandboxPerformance::testSandboxCreationThroughput() {
    ...
    for (int i = 0; i < creation_count; ++i) {
        QString sandbox_id = QString("throughput_create_%1_%2")
                                 .arg(i)
                                 .arg(QDateTime::currentMSecsSinceEpoch());
    ...
    double throughput =
        (static_cast<double>(successful_creations) / total_time) *
        1000.0;  // creations per second
    qDebug() << "Sandbox creation throughput:" << throughput
             << "creations/second";
    qDebug() << "Total time for" << successful_creations
             << "creations:" << total_time << "ms";
...
void TestSandboxPerformance::testSandboxShutdownTime() {
    ...
    // Create sandboxes
    for (int i = 0; i < sandbox_count; ++i) {
        QString sandbox_id = QString("shutdown_test_%1_%2")
                                 .arg(i)
                                 .arg(QDateTime::currentMSecsSinceEpoch());
    ...
    qDebug() << "Shutdown time for" << sandbox_count
             << "sandboxes:" << shutdown_time << "ms";
    qDebug() << "Average shutdown time per sandbox:"
             << (static_cast<double>(shutdown_time) / sandbox_count) << "ms";

    // Performance expectations
    QVERIFY(shutdown_time < 1000);  // Less than 1 second total
    QVERIFY((static_cast<double>(shutdown_time) / sandbox_count) <
            50.0);  // Less than 50ms per sandbox
}
...
QString TestSandboxPerformance::createSimpleTestPlugin() {
    QTemporaryFile* temp_file =
        new QTemporaryFile(m_temp_dir->path() + "/perf_plugin_XXXXXX.py", this);
...
void TestSandboxPerformance::printMetrics(const QString& test_name,
                                          const PerformanceMetrics& metrics) {

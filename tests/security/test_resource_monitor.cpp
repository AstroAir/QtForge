...
void TestResourceMonitor::cleanupTestCase() {
    cleanupTestProcess();
}
...
void TestResourceMonitor::testCpuUsageCalculation() {
    ...
    double percentage = ResourceMonitorUtils::calculate_cpu_percentage(used_time, total_time);
    QCOMPARE(percentage, 50.0);
    
    // Test edge cases
    double zero_total = ResourceMonitorUtils::calculate_cpu_percentage(used_time, std::chrono::milliseconds(0));
    QCOMPARE(zero_total, 0.0);
    
    double zero_used = ResourceMonitorUtils::calculate_cpu_percentage(std::chrono::milliseconds(0), total_time);
...
void TestResourceMonitor::testMemoryUsageCalculation() {
    ...
    double percentage = ResourceMonitorUtils::calculate_memory_percentage(used_mb, total_mb);
    QCOMPARE(percentage, 50.0);
    
    // Test edge cases
    double zero_total = ResourceMonitorUtils::calculate_memory_percentage(used_mb, 0);
    QCOMPARE(zero_total, 0.0);
    
    double zero_used = ResourceMonitorUtils::calculate_memory_percentage(0, total_mb);
    QCOMPARE(zero_used, 0.0);
}
...
void TestResourceMonitor::testMonitoringOverhead() {
    ...
    // Resource monitoring should be reasonably fast (less than 10ms per call on average)
...
void TestResourceMonitor::testResourceMonitorUtils() {
    ...
    bool exceeds_80 = ResourceMonitorUtils::exceeds_threshold(usage, limits, 80.0);
    QVERIFY(exceeds_80);
    
    // Should not exceed 90% threshold
    bool exceeds_90 = ResourceMonitorUtils::exceeds_threshold(usage, limits, 90.0);
    QVERIFY(!exceeds_90);
}

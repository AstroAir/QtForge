...
void TestPluginMarketplace::testInstallationProgress() {
    ...
    QSignalSpy progress_spy(m_marketplace.get(),
                            &PluginMarketplace::installation_progress);
    QSignalSpy started_spy(m_marketplace.get(),
                           &PluginMarketplace::installation_started);
    QSignalSpy completed_spy(m_marketplace.get(),
                             &PluginMarketplace::installation_completed);
...
void TestPluginMarketplace::testInstallationCancellation() {
    ...
    if (result.has_value()) {
        ...
        // Cancel installation
        auto cancel_result =
            m_marketplace->cancel_installation(installation_id);
...
void TestPluginMarketplace::testInstallationErrors() {
    ...
    auto result =
        m_marketplace->install_plugin("nonexistent_plugin_xyz", "1.0.0");
    ...
    if (result.has_value()) {
        ...
        QSignalSpy error_spy(m_marketplace.get(),
                             &PluginMarketplace::installation_failed);
...
void TestPluginMarketplace::testNetworkErrors() {
    ...
    m_marketplace->set_marketplace_url(
...
void TestPluginMarketplace::testSearchCache() {
    ...
    qDebug() << "First search:" << first_time
             << "ms, Second search:" << second_time << "ms";
}
...
void TestPluginMarketplace::testSignatureValidation() {
    ...
    auto result =
        m_marketplace->validate_signature("test_plugin", "fake_signature");
...
MarketplacePlugin TestPluginMarketplace::createMockPlugin(const QString& id,
                                                          const QString& name) {
    MarketplacePlugin plugin;
    plugin.id = id;
    plugin.name = name;

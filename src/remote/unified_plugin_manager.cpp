...
UnifiedPluginManager::UnifiedPluginManager(QObject* parent)
    : QObject(parent)
    , d_ptr(std::make_unique<UnifiedPluginManagerPrivate>())
{
    ...
    connect(d->remotePluginManager, &RemotePluginManager::pluginUpdatesAvailable, this, &UnifiedPluginManager::pluginUpdatesAvailable);
...
void UnifiedPluginManager::setLoadStrategy(UnifiedLoadStrategy strategy)
{
    ...
    if (d->loadStrategy != strategy) {
        ...
        qCDebug(unifiedPluginLog) << "Load strategy changed to:" << static_cast<int>(strategy);
...
void UnifiedPluginManager::setAutoUpdateEnabled(bool enabled)
{
    ...
    if (d->autoUpdateEnabled != enabled) {
        ...
        qCDebug(unifiedPluginLog) << "Auto update" << (enabled ? "enabled" : "disabled");
...
{
    return QtConcurrent::run([this, pluginIdOrPath]() {
        return loadPluginImpl(pluginIdOrPath);
    });
}
...
bool UnifiedPluginManager::loadPluginImpl(const QString& pluginIdOrPath)
{
    ...
    bool isPath = pluginIdOrPath.contains('/') || pluginIdOrPath.contains('\\') || 
...
bool UnifiedPluginManager::loadLocalPlugin(const QString& path)
{
    ...
    if (!QFileInfo::exists(path)) {
        qCWarning(unifiedPluginLog) << "Local plugin file does not exist:" << path;
        return false;
    }
    ...
    try {
        ...
        if (result.has_value()) {
            ...
            qCWarning(unifiedPluginLog) << "Failed to load local plugin:" << path;
            return false;
        }
    } catch (const std::exception& e) {
        qCWarning(unifiedPluginLog) << "Failed to load local plugin:" << path << "-" << e.what();
        return false;
    }
}
...
bool UnifiedPluginManager::loadRemotePlugin(const QString& pluginId)
{
    ...
    if (localPath.isEmpty()) {
        qCWarning(unifiedPluginLog) << "Failed to download remote plugin:" << pluginId;
        return false;
    }
...
bool UnifiedPluginManager::unloadPlugin(const QString& pluginId)
{
    ...
    bool success = false;
    if (d->localPluginManager) {
        auto result = d->localPluginManager->unload_plugin(pluginId.toStdString());
...
{
    return QtConcurrent::run([this, pluginId]() {
        return updatePluginImpl(pluginId);
    });
}
...
bool UnifiedPluginManager::updatePluginImpl(const QString& pluginId)
{
    ...
    if (!d->pluginSources.contains(pluginId)) {
        qCWarning(unifiedPluginLog) << "Plugin not loaded, cannot update:" << pluginId;
        return false;
    }
    ...
    if (source == PluginSource::Local) {
        ...
        if (d->loadStrategy != UnifiedLoadStrategy::LocalOnly && isRemotePluginAvailable(pluginId)) {
            ...
            qCDebug(unifiedPluginLog) << "Local plugin update not implemented:" << pluginId;
            return false;
        }
    } else {
...
PluginSource UnifiedPluginManager::getPluginSource(const QString& pluginId) const
{
    Q_D(const UnifiedPluginManager);
...
QStringList UnifiedPluginManager::discoverLocalPlugins()
{
    ...
    qCDebug(unifiedPluginLog) << "Discovered" << plugins.size() << "local plugins";
...
QStringList UnifiedPluginManager::discoverRemotePlugins()
{
    ...
    qCDebug(unifiedPluginLog) << "Discovered" << plugins.size() << "remote plugins";
...
void UnifiedPluginManager::checkForUpdates()
{
    ...
    if (!d->autoUpdateEnabled) {
        qCDebug(unifiedPluginLog) << "Auto update disabled, skipping update check";
        return;
    }

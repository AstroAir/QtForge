...
QJsonObject RemotePluginInfo::to_json() const {
    ...
    json["download_time"] = static_cast<qint64>(
        std::chrono::duration_cast<std::chrono::seconds>(
            download_time.time_since_epoch()).count());
    
    json["last_update_check"] = static_cast<qint64>(
        std::chrono::duration_cast<std::chrono::seconds>(
            last_update_check.time_since_epoch()).count());
...
qtplugin::expected<RemotePluginInfo, PluginError> RemotePluginInfo::from_json(const QJsonObject& json) {
    ...
    if (json.contains("remote_source")) {
        auto source_result = RemotePluginSource::from_json(json["remote_source"].toObject());
        if (source_result) {
    ...
    if (json.contains("download_time")) {
        auto timestamp = std::chrono::seconds(json["download_time"].toInteger());
        info.download_time = std::chrono::system_clock::time_point(timestamp);
    }
    
    if (json.contains("last_update_check")) {
        auto timestamp = std::chrono::seconds(json["last_update_check"].toInteger());
        info.last_update_check = std::chrono::system_clock::time_point(timestamp);
    }
...
std::chrono::seconds RemotePluginInfo::cache_age() const {
    ...
    return std::chrono::duration_cast<std::chrono::seconds>(now - download_time);
}
...
qtplugin::expected<RemotePluginDiscoveryResult, PluginError> RemotePluginDiscoveryResult::from_json(const QJsonObject& json) {
    ...
    // Parse source
    auto source_result = RemotePluginSource::from_json(json["source"].toObject());
    if (!source_result) {
...
RemotePluginSearchCriteria RemotePluginSearchCriteria::from_json(const QJsonObject& json) {
...
qtplugin::expected<void, PluginError> RemotePluginRegistry::register_remote_plugin(
    ...
    auto base_info = std::make_unique<PluginInfo>(static_cast<const PluginInfo&>(*remote_plugin_info));
    auto base_result = PluginRegistry::register_plugin(plugin_id, std::move(base_info));
    if (!base_result) {
        return base_result;
    }
...
std::vector<RemotePluginInfo> RemotePluginRegistry::get_all_remote_plugin_info() const {
    std::shared_lock lock(m_remote_plugins_mutex);
...
RemotePluginRegistry::discover_remote_plugins(const RemotePluginSearchCriteria& criteria) const {
    ...
    if (!criteria.sort_by.empty()) {
        std::sort(all_results.begin(), all_results.end(),
                 [&criteria](const RemotePluginDiscoveryResult& a, const RemotePluginDiscoveryResult& b) {
            if (criteria.sort_by == "name") {
                return criteria.sort_ascending ? a.name < b.name : a.name > b.name;
            } else if (criteria.sort_by == "rating" && a.rating && b.rating) {
                return criteria.sort_ascending ? *a.rating < *b.rating : *a.rating > *b.rating;
            } else if (criteria.sort_by == "downloads" && a.download_count && b.download_count) {
                return criteria.sort_ascending ? *a.download_count < *b.download_count : *a.download_count > *b.download_count;
            }
    ...
    // Apply pagination
    if (criteria.offset > 0 && criteria.offset < static_cast<int>(all_results.size())) {
        all_results.erase(all_results.begin(), all_results.begin() + criteria.offset);
    }

    if (criteria.max_results > 0 && static_cast<int>(all_results.size()) > criteria.max_results) {
...
qtplugin::expected<std::vector<RemotePluginDiscoveryResult>, PluginError>
RemotePluginRegistry::search_remote_plugins(const std::string& query, int max_results) const {
...
qtplugin::expected<void, PluginError> RemotePluginRegistry::remove_remote_source(
    ...
    if (it == m_remote_sources.end()) {
        return qtplugin::make_error<void>(PluginErrorCode::NotFound,
                                         "Remote source not found: " + source_id);
    }
...
std::vector<RemotePluginSource> RemotePluginRegistry::get_remote_sources() const {
    std::shared_lock lock(m_remote_sources_mutex);
...
int RemotePluginRegistry::clear_remote_cache(int older_than_days) {
    ...
    while (it != m_remote_plugins.end()) {
        ...
        if (older_than_days == 0) {
            ...
            should_clear = it->second->is_cached && it->second->download_time < cutoff_time;
        }

        if (should_clear) {
            // Remove cached file if it exists
            if (it->second->cached_path && std::filesystem::exists(*it->second->cached_path)) {
...
QJsonObject RemotePluginRegistry::get_cache_statistics() const {
    ...
    for (const auto& [plugin_id, plugin_info] : m_remote_plugins) {
        if (plugin_info->is_cached) {
            ...
            if (plugin_info->cached_path && std::filesystem::exists(*plugin_info->cached_path)) {
                try {
                    total_cache_size += std::filesystem::file_size(*plugin_info->cached_path);
    ...
    stats["cache_hit_ratio"] = total_count > 0 ? static_cast<double>(cached_count) / total_count : 0.0;
    stats["total_cache_size_bytes"] = total_cache_size;
    stats["cache_directory"] = QString::fromStdString(m_cache_directory.string());
...
RemotePluginRegistry::check_plugin_update(const std::string& plugin_id) const {

    std::shared_lock plugins_lock(m_remote_plugins_mutex);

    auto it = m_remote_plugins.find(plugin_id);
    if (it == m_remote_plugins.end()) {
        return qtplugin::make_error<bool>(PluginErrorCode::NotFound,
                                         "Remote plugin not found: " + plugin_id);
    }
    ...
    auto latest_version_result = get_latest_version_from_source(
        plugin_id, *it->second->remote_source);
...
std::optional<PluginInfo> RemotePluginRegistry::get_plugin_info(const std::string& plugin_id) const {
...
RemotePluginInfo RemotePluginRegistry::create_remote_plugin_info_copy(const RemotePluginInfo& original) const {
    RemotePluginInfo copy = original;
    // Deep copy any shared resources if needed
    return copy;
}

bool RemotePluginRegistry::is_remote_plugin(const std::string& plugin_id) const {
    std::shared_lock lock(m_remote_plugins_mutex);
...
void RemotePluginRegistry::initialize_cache_directory() {
    ...
    m_cache_directory = std::filesystem::path(cache_dir.toStdString()) / "remote_plugins";
...
qtplugin::expected<std::vector<RemotePluginDiscoveryResult>, PluginError>
RemotePluginRegistry::discover_from_source(const RemotePluginSource& source,
...
RemotePluginRegistry::get_latest_version_from_source(const std::string& plugin_id,

...
RemotePluginManagerExtension::RemotePluginManagerExtension(
...
RemotePluginManagerExtension::load_remote_plugin(
...
QString RemotePluginManagerExtension::load_remote_plugin_async(
    ...
    if (!m_remote_plugins_enabled) {
        if (completion_callback) {
            PluginError error{PluginErrorCode::InvalidConfiguration,
                              "Remote plugin support is disabled"};
    ...
    if (HttpPluginLoader::is_http_url(source.url())) {
        ...
        if (!remote_operation_id.isEmpty()) {
            track_async_operation(operation_id,
                                  remote_operation_id.toStdString());
            return operation_id;
        }
    }
...
RemotePluginManagerExtension::cancel_remote_load(const QString& operation_id) {
    std::lock_guard<std::mutex> lock(m_remote_plugins_mutex);
...
qtplugin::expected<std::string, PluginError>
RemotePluginManagerExtension::load_plugin(
    const std::string& path_or_url, const RemotePluginLoadOptions& options) {
...
qtplugin::expected<void, PluginError>
RemotePluginManagerExtension::add_remote_source(
...
qtplugin::expected<void, PluginError>
RemotePluginManagerExtension::remove_remote_source(const QString& source_id) {
    if (!m_http_loader) {
        return qtplugin::make_error<void>(PluginErrorCode::InvalidConfiguration,
                                          "Remote plugin loader not available");
    }
...
std::vector<RemotePluginSource>
RemotePluginManagerExtension::get_remote_sources() const {
    if (!m_http_loader) {
        return {};
    }
...
RemotePluginManagerExtension::discover_remote_plugins(
    ...
    for (const auto& source : sources) {
        ...
        if (result) {
            ...
            all_plugins.insert(all_plugins.end(), plugins.begin(),
                               plugins.end());
        }
    }
...
qtplugin::expected<std::vector<QJsonObject>, PluginError>
RemotePluginManagerExtension::search_remote_plugins(const QString& query,
                                                    int max_results) {
    if (!m_http_loader) {
        return qtplugin::make_error<std::vector<QJsonObject>>(
            PluginErrorCode::InvalidConfiguration,
...
void RemotePluginManagerExtension::set_remote_configuration(
...
RemotePluginManagerExtension::remote_configuration() const {
...
QJsonObject RemotePluginManagerExtension::get_remote_statistics() const {
    ...
    {
        std::lock_guard<std::mutex> lock(m_remote_plugins_mutex);
        stats["remote_plugins_loaded"] =
            static_cast<int>(m_remote_plugin_sources.size());
        stats["active_operations"] =
            static_cast<int>(m_async_operations.size());
    }
...
std::vector<QString>
RemotePluginManagerExtension::get_active_remote_operations() const {
    std::lock_guard<std::mutex> lock(m_remote_plugins_mutex);
...
bool RemotePluginManagerExtension::is_url(
    const std::string& path_or_url) const {
    return is_plugin_url(path_or_url);
}

QUrl RemotePluginManagerExtension::parse_url(
    const std::string& url_string) const {
    return QUrl(QString::fromStdString(url_string));
}

qtplugin::expected<std::string, PluginError>
RemotePluginManagerExtension::load_from_cached_file(
...
void RemotePluginManagerExtension::initialize_remote_components() {
    ...
    // Create validator
    m_validator = std::make_shared<RemotePluginValidator>(
        nullptr, m_remote_configuration);
...
void RemotePluginManagerExtension::track_async_operation(
    const QString& operation_id, const std::string& plugin_id) {
    std::lock_guard<std::mutex> lock(m_remote_plugins_mutex);
    m_async_operations[operation_id] = plugin_id;
}

void RemotePluginManagerExtension::untrack_async_operation(
...
PluginLoadOptions RemotePluginManagerExtension::convert_to_base_options(
    ...
    base_options.enable_hot_reload =
        false;  // Remote plugins don't support hot reload by default
...
RemotePluginLoadOptions RemotePluginManagerExtension::convert_from_base_options(
...
std::unique_ptr<RemotePluginManagerExtension>
RemotePluginManagerFactory::create_with_remote_support() {
...
std::unique_ptr<RemotePluginManagerExtension>
RemotePluginManagerFactory::create_with_remote_config(
    ...
    // Create extension
    auto extension =
        std::make_unique<RemotePluginManagerExtension>(base_manager);
...
std::unique_ptr<RemotePluginManagerExtension>
RemotePluginManagerFactory::enhance_existing_manager(
    std::shared_ptr<PluginManager> base_manager) {
    return std::make_unique<RemotePluginManagerExtension>(base_manager);
}

std::unique_ptr<RemotePluginManagerExtension>
RemotePluginManagerFactory::create_enterprise() {
    // Create base plugin manager
    auto base_manager = std::make_shared<PluginManager>();

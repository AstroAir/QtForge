...
ValidationResult ValidationResult::from_json(const QJsonObject& json) {
    ...
    QDateTime dt = QDateTime::fromString(json["timestamp"].toString(), Qt::ISODate);
    result.timestamp = std::chrono::system_clock::from_time_t(dt.toSecsSinceEpoch());
...
QJsonObject CertificateValidation::to_json() const {
    ...
    json["expiry_date"] = QDateTime::fromSecsSinceEpoch(
        std::chrono::duration_cast<std::chrono::seconds>(expiry_date.time_since_epoch()).count()
    ).toString(Qt::ISODate);
...
QJsonObject SignatureValidation::to_json() const {
    ...
    json["signer"] = signer;
    json["fingerprint"] = fingerprint;
    json["signature_date"] = QDateTime::fromSecsSinceEpoch(
        std::chrono::duration_cast<std::chrono::seconds>(signature_date.time_since_epoch()).count()
    ).toString(Qt::ISODate);
...
QJsonObject SourceReputation::to_json() const {
    ...
    json["last_verified"] = QDateTime::fromSecsSinceEpoch(
        std::chrono::duration_cast<std::chrono::seconds>(last_verified.time_since_epoch()).count()
    ).toString(Qt::ISODate);
...
qtplugin::expected<ValidationResult, PluginError> RemotePluginValidator::validate_source(
    ...
    if (is_domain_blocked(domain)) {
        ValidationResult result = create_validation_result(
            ...
            "Domain is blocked",
            "Domain " + domain.toStdString() + " is in the blocked domains list"
        );
    ...
    if (m_configuration) {
        security_level = m_configuration->security_policy().default_security_level;
    }
...
qtplugin::expected<ValidationResult, PluginError> RemotePluginValidator::validate_url(
    const QUrl& url, RemoteSecurityLevel security_level) {
...
CertificateValidation RemotePluginValidator::validate_certificate(
    ...
    validation.fingerprint = certificate.digest(QCryptographicHash::Sha256).toHex();
    validation.issuer = certificate.issuerDisplayName();
    validation.subject = certificate.subjectDisplayName();
    validation.expiry_date = std::chrono::system_clock::from_time_t(certificate.expiryDate().toSecsSinceEpoch());
    
    // Check if self-signed
    validation.is_self_signed = (certificate.issuerDisplayName() == certificate.subjectDisplayName());
    
    // Check hostname matching if provided
    if (!hostname.isEmpty()) {
        // Simple hostname validation - in production, this should be more comprehensive
        QString subject_cn = certificate.subjectInfo(QSslCertificate::CommonName).join(", ");
    ...
    validation.is_trusted = !validation.is_expired && !validation.is_self_signed && validation.errors.isEmpty();
...
QString RemotePluginValidator::generate_cache_key(const RemotePluginSource& source) const {
    return source.id();
}
...
bool RemotePluginValidator::is_validation_cached(const QString& cache_key) const {
    std::lock_guard<std::mutex> lock(m_validation_cache_mutex);
    return m_validation_cache.find(cache_key) != m_validation_cache.end();
}

std::optional<ValidationResult> RemotePluginValidator::get_cached_validation(const QString& cache_key) const {
    std::lock_guard<std::mutex> lock(m_validation_cache_mutex);
    ...
    if (it != m_validation_cache.end()) {
        ...
        // Note: We can't erase from const method, expired entries will be cleaned up elsewhere
    }

    return std::nullopt;
}

void RemotePluginValidator::cache_validation_result(const QString& cache_key, const ValidationResult& result) {
    std::lock_guard<std::mutex> lock(m_validation_cache_mutex);
    m_validation_cache[cache_key] = result;
}

ValidationResult RemotePluginValidator::create_validation_result(ValidationLevel level,
...
qtplugin::expected<ValidationResult, PluginError> RemotePluginValidator::validate_url_scheme(const QUrl& url) const {
    QString scheme = url.scheme().toLower();

    // Check if scheme is supported
    auto supported_schemes = RemotePluginSource::supported_schemes();
    bool is_supported = std::find(supported_schemes.begin(), supported_schemes.end(), scheme) != supported_schemes.end();
...
qtplugin::expected<ValidationResult, PluginError> RemotePluginValidator::validate_url_security(
    ...
    if (level >= RemoteSecurityLevel::Standard && scheme != "https") {
        if (m_configuration && m_configuration->security_policy().require_https) {
            ValidationResult result = create_validation_result(
                ValidationLevel::Failed,
                "HTTPS required",
    ...
    ValidationResult result = create_validation_result(
        ValidationLevel::Passed,
        "URL security validation passed",
...
QString RemotePluginValidator::calculate_file_checksum(const std::filesystem::path& file_path) const {
...
bool RemotePluginValidator::verify_file_checksum(const std::filesystem::path& file_path,
...
void RemotePluginValidator::set_security_manager(std::shared_ptr<ISecurityManager> security_manager) {
    m_security_manager = security_manager;
}

void RemotePluginValidator::set_configuration(std::shared_ptr<RemotePluginConfiguration> configuration) {
    m_configuration = configuration;
}
...
QJsonObject RemotePluginValidator::get_validation_statistics() const {
    ...
    if (total > 0) {
        stats["success_rate"] = static_cast<double>(m_validations_passed.load()) / total;
        stats["failure_rate"] = static_cast<double>(m_validations_failed.load()) / total;
        stats["block_rate"] = static_cast<double>(m_validations_blocked.load()) / total;
    }

    {
        std::lock_guard<std::mutex> lock(m_validation_cache_mutex);
        stats["cached_validations"] = static_cast<int>(m_validation_cache.size());
    }

    {
        std::lock_guard<std::mutex> lock(m_reputation_cache_mutex);
        stats["cached_reputations"] = static_cast<int>(m_reputation_cache.size());
    }
...
bool RemotePluginValidator::load_reputation_cache() {
    ...
    for (auto it = reputations.begin(); it != reputations.end(); ++it) {
        try {
            ...
            reputation.level = static_cast<SourceReputation::Level>(rep_obj["level"].toInt());
            reputation.download_count = rep_obj["download_count"].toInt();
            reputation.success_rate = rep_obj["success_rate"].toInt();

            QDateTime dt = QDateTime::fromString(rep_obj["last_verified"].toString(), Qt::ISODate);
            reputation.last_verified = std::chrono::system_clock::from_time_t(dt.toSecsSinceEpoch());

            QJsonArray sources_array = rep_obj["reputation_sources"].toArray();
            for (const auto& source_value : sources_array) {
                reputation.reputation_sources.push_back(source_value.toString());
            }
...
std::filesystem::path RemotePluginValidator::get_reputation_cache_path() const {
    ...
    return std::filesystem::path(cache_dir.toStdString()) / "qtforge" / "remote_plugin_reputation.json";
}

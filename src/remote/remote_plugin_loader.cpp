...
QJsonObject RemotePluginLoadOptions::to_json() const {
    ...
    json["validation_timeout"] =
        static_cast<qint64>(validation_timeout.count());
    return json;
}

RemotePluginLoadOptions RemotePluginLoadOptions::from_json(
    const QJsonObject& json) {
    RemotePluginLoadOptions options;
    options.download_options =
        DownloadOptions::from_json(json["download_options"].toObject());
    ...
    options.validation_timeout =
        std::chrono::seconds(json["validation_timeout"].toInt());
    return options;
}
...
bool RemotePluginLoaderBase::can_load(
...
RemotePluginLoaderBase::load(const std::filesystem::path& file_path) {
...
std::vector<std::string> RemotePluginLoaderBase::supported_extensions() const {
    ...
    if (m_local_loader) {
        ...
        extensions.insert(extensions.end(), local_extensions.begin(),
                          local_extensions.end());
    }
...
std::optional<RemotePluginSource> RemotePluginLoaderBase::find_source_for_url(
...
RemotePluginLoaderBase::load_remote(const QUrl& url,
    ...
    if (source_opt) {
        return load_remote(*source_opt, options);
    }

    // Create a temporary source for this URL
    RemotePluginSource temp_source(url, RemoteSourceType::Http);
    return perform_remote_load(temp_source, options);
}

void RemotePluginLoaderBase::set_configuration(
    std::shared_ptr<RemotePluginConfiguration> configuration) {
    m_configuration = configuration;
...
RemotePluginLoaderBase::configuration() const {
    return m_configuration;
}

void RemotePluginLoaderBase::set_download_manager(
    std::shared_ptr<PluginDownloadManager> download_manager) {
    m_download_manager = download_manager;
}

std::shared_ptr<PluginDownloadManager>
RemotePluginLoaderBase::download_manager() const {
    return m_download_manager;
}

void RemotePluginLoaderBase::set_validator(
    std::shared_ptr<RemotePluginValidator> validator) {
    m_validator = validator;
...
std::shared_ptr<RemotePluginValidator> RemotePluginLoaderBase::validator()
    const {
    return m_validator;
}
...
QJsonObject RemotePluginLoaderBase::get_statistics() const {
    ...
    if (total > 0) {
        stats["success_rate"] =
            static_cast<double>(m_remote_loads_successful.load()) / total;
        stats["failure_rate"] =
            static_cast<double>(m_remote_loads_failed.load()) / total;
        stats["cache_hit_rate"] =
            static_cast<double>(m_cache_hits.load()) / total;
    }

    {
        std::lock_guard<std::mutex> ops_lock(m_operations_mutex);
        stats["active_operations"] =
            static_cast<int>(m_active_operations.size());
    }
...
void RemotePluginLoaderBase::track_operation(const QString& operation_id,
...
RemotePluginLoaderBase::load_from_cache(
    ...
    if (!std::filesystem::exists(cached_path)) {
        return qtplugin::unexpected(PluginError{
            PluginErrorCode::FileNotFound, "Cached plugin file not found"});
    }
...
void RemotePluginLoaderBase::initialize_components() {
    ...
    if (!m_validator) {
        m_validator =
            std::make_shared<RemotePluginValidator>(nullptr, m_configuration);
    }
...
void RemotePluginLoaderBase::validate_configuration() const {
    ...
    if (!validation_result) {
        throw std::runtime_error("Invalid remote plugin configuration: " +

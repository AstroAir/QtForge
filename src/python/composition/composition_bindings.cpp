...
void bind_composition(py::module& m) {
    ...
    py::class_<CompositionBinding>(m, "CompositionBinding")
        .def(py::init<>())
        .def(py::init<const QString&, const QString&, const QString&, const QString&>())
        .def_readwrite("source_plugin_id", &CompositionBinding::source_plugin_id)
        .def_readwrite("source_method", &CompositionBinding::source_method)
        .def_readwrite("target_plugin_id", &CompositionBinding::target_plugin_id)
        .def_readwrite("target_method", &CompositionBinding::target_method)
        .def_readwrite("parameter_mapping", &CompositionBinding::parameter_mapping)
        ...
        .def("__repr__", [](const CompositionBinding& binding) {
            return "<CompositionBinding " + binding.source_plugin_id.toStdString() + 
    ...
    py::class_<PluginComposition>(m, "PluginComposition")
        ...
        .def("set_description", &PluginComposition::set_description, py::return_value_policy::reference)
        .def("set_strategy", &PluginComposition::set_strategy, py::return_value_policy::reference)
        .def("add_plugin", &PluginComposition::add_plugin, 
             py::arg("plugin_id"), py::arg("role") = PluginRole::Secondary,
             py::return_value_policy::reference)
        .def("set_primary_plugin", &PluginComposition::set_primary_plugin, py::return_value_policy::reference)
        .def("add_binding", &PluginComposition::add_binding, py::return_value_policy::reference)
        .def("set_configuration", &PluginComposition::set_configuration, py::return_value_policy::reference)
    ...
    py::class_<PluginCompositionManager, std::shared_ptr<PluginCompositionManager>>(
        m, "PluginCompositionManager")
        .def(py::init<>())
        .def_static("create", &PluginCompositionManager::create)
        .def("register_composition", &PluginCompositionManager::register_composition)
        .def("unregister_composition", &PluginCompositionManager::unregister_composition)
        ...
        .def("activate_composition", &PluginCompositionManager::activate_composition)
        .def("deactivate_composition", &PluginCompositionManager::deactivate_composition)
        .def("is_composition_active", &PluginCompositionManager::is_composition_active)
        .def("get_active_compositions", &PluginCompositionManager::get_active_compositions)
        .def("execute_composition", &PluginCompositionManager::execute_composition)
        .def("clear_compositions", &PluginCompositionManager::clear_compositions)
    ...
    m.def(
        "create_composition",
        [](const std::string& id, const std::string& name) -> PluginComposition {
            return PluginComposition(QString::fromStdString(id), QString::fromStdString(name));
        },
        py::arg("composition_id"), py::arg("name") = "",
    ...
    m.def(
        ...
        [](const std::string& src_plugin, const std::string& src_method,
           const std::string& tgt_plugin, const std::string& tgt_method) -> CompositionBinding {
    ...
    m.def(
        ...
        [](const std::string& id, const std::vector<std::string>& plugin_ids) -> PluginComposition {
            ...
            for (size_t i = 0; i < plugin_ids.size(); ++i) {
                ...
                PluginRole role = (i == 0) ? PluginRole::Primary : PluginRole::Secondary;
                comp.add_plugin(plugin_id, role);
                
                // Create pipeline bindings
                if (i > 0) {
                    QString prev_plugin = QString::fromStdString(plugin_ids[i-1]);
                    CompositionBinding binding(prev_plugin, "output", plugin_id, "input");
    ...
    m.def(
        ...
        [](const std::string& id, const std::string& facade_plugin,
           const std::vector<std::string>& backend_plugins) -> PluginComposition {
            PluginComposition comp(QString::fromStdString(id));
            ...
            for (const auto& backend : backend_plugins) {
                comp.add_plugin(QString::fromStdString(backend), PluginRole::Secondary);
            }
            
            return comp;
        },
        py::arg("composition_id"), py::arg("facade_plugin"), py::arg("backend_plugins"),
        "Create a facade composition with a primary facade plugin and backend plugins");
}

...
bool is_valid_state_transition(LifecycleState from, LifecycleState to) {
    ...
    switch (from) {
        ...
        case LifecycleState::Initialized:
            return to == LifecycleState::Active || to == LifecycleState::Idle ||
                   to == LifecycleState::Cleanup ||
                   to == LifecycleState::Destroyed;
        case LifecycleState::Active:
            return to == LifecycleState::Idle ||
        ...
        case LifecycleState::Idle:
            return to == LifecycleState::Active ||
                   ...
                   to == LifecycleState::Destroyed;
        case LifecycleState::Deprecated:
            return to == LifecycleState::Cleanup ||
                   to == LifecycleState::Destroyed;
        case LifecycleState::Cleanup:
...
ResourceLifecycleManager::ResourceLifecycleManager(QObject* parent)
    ...
    connect(m_cleanup_timer.get(), &QTimer::timeout, this,
            &ResourceLifecycleManager::perform_automatic_cleanup);
...
ResourceLifecycleManager::register_resource(const ResourceHandle& handle,
    ...
    qCDebug(lifecycleLog) << "Registered resource:"
                          << QString::fromStdString(resource_id)
                          << "initial state:"
                          << QString::fromStdString(
                                 lifecycle_state_to_string(initial_state));
...
ResourceLifecycleManager::unregister_resource(const std::string& resource_id) {
    ...
    // Notify subscribers
    LifecycleEvent event(resource_id, tracker->handle.type(),
                         tracker->handle.plugin_id(), old_state,
    ...
    qCDebug(lifecycleLog) << "Unregistered resource:"
                          << QString::fromStdString(resource_id);

    return make_success();
}

qtplugin::expected<void, PluginError> ResourceLifecycleManager::update_state(
    const std::string& resource_id, LifecycleState new_state,
    ...
    if (!is_state_transition_valid(old_state, new_state)) {
        return make_error<void>(PluginErrorCode::InvalidArgument,
                                "Invalid state transition from " +
                                    lifecycle_state_to_string(old_state) +
                                    " to " +
    ...
    // Notify subscribers
    LifecycleEvent event(resource_id, tracker->handle.type(),
                         tracker->handle.plugin_id(), old_state, new_state,
                         metadata);
    notify_state_change(event);
    ...
    qCDebug(lifecycleLog)
        << "Updated resource state:" << QString::fromStdString(resource_id)
        << "from"
        << QString::fromStdString(lifecycle_state_to_string(old_state)) << "to"
        << QString::fromStdString(lifecycle_state_to_string(new_state));
...
qtplugin::expected<void, PluginError> ResourceLifecycleManager::add_dependency(
    const ResourceDependency& dependency) {
    ...
    qCDebug(lifecycleLog) << "Added dependency:"
                          << QString::fromStdString(dependency.dependent_id)
                          << "depends on"
                          << QString::fromStdString(dependency.dependency_id);

    return make_success();
}

qtplugin::expected<void, PluginError>
ResourceLifecycleManager::remove_dependency(const std::string& dependent_id,
    ...
    qCDebug(lifecycleLog) << "Removed dependency:"
                          << QString::fromStdString(dependent_id)
                          << "no longer depends on"
                          << QString::fromStdString(dependency_id);

    return make_success();
}

qtplugin::expected<std::vector<ResourceDependency>, PluginError>
ResourceLifecycleManager::get_dependencies(
    const std::string& resource_id) const {
    std::shared_lock<std::shared_mutex> lock(m_dependencies_mutex);

    auto it = m_dependencies.find(resource_id);
    if (it == m_dependencies.end()) {
        return std::vector<ResourceDependency>{};  // Return empty vector if no
                                                   // dependencies
    }
...
size_t ResourceLifecycleManager::perform_cleanup() {
    ...
    auto candidates =
        get_cleanup_candidates(1000);  // Get up to 1000 candidates
    if (candidates.empty()) {
        return 0;
    }
    ...
    if (cleaned > 0) {
        qCDebug(lifecycleLog)
            << "Automatic cleanup completed, cleaned" << cleaned << "resources";
    }

    return cleaned;
}

qtplugin::expected<void, PluginError> ResourceLifecycleManager::force_cleanup(
    const std::string& resource_id, bool force_cleanup) {
    ...
    qCDebug(lifecycleLog) << "Force cleanup completed for resource:"
                          << QString::fromStdString(resource_id);

    return make_success();
}

size_t ResourceLifecycleManager::cleanup_plugin_resources(
...
std::string ResourceLifecycleManager::subscribe_to_lifecycle_events(
    ...
    qCDebug(lifecycleLog) << "Created lifecycle event subscription:"
...
qtplugin::expected<void, PluginError>
ResourceLifecycleManager::unsubscribe_from_lifecycle_events(
    ...
    qCDebug(lifecycleLog) << "Removed lifecycle event subscription:"
                          << QString::fromStdString(subscription_id);
...
QJsonObject ResourceLifecycleManager::get_lifecycle_statistics() const {
    ...
    stats["total_resources_tracked"] =
        static_cast<qint64>(m_total_resources_tracked.load());
    stats["total_resources_cleaned"] =
        static_cast<qint64>(m_total_resources_cleaned.load());
    stats["total_state_transitions"] =
        static_cast<qint64>(m_total_state_transitions.load());
    ...
    QJsonObject state_counts;
    {
        ...
        for (const auto& [state, count] : counts) {
            state_counts[QString::fromStdString(
                lifecycle_state_to_string(state))] = static_cast<qint64>(count);
        }
    }
    ...
    // Dependency statistics
    {
        ...
        stats["total_dependencies"] =
            static_cast<qint64>(m_dependencies.size());
...
ResourceLifecycleManager::get_resource_history(const std::string& resource_id,
                                               size_t max_events) const {
    std::shared_lock<std::shared_mutex> lock(m_resources_mutex);

    auto it = m_tracked_resources.find(resource_id);
    if (it == m_tracked_resources.end()) {
        return make_error<std::vector<LifecycleEvent>>(
            PluginErrorCode::NotFound, "Resource not found: " + resource_id);
    }
...
std::vector<std::string> ResourceLifecycleManager::get_resources_in_state(
...
bool ResourceLifecycleManager::can_cleanup_resource(
    const std::string& resource_id) const {
    std::shared_lock<std::shared_mutex> resources_lock(m_resources_mutex);
    ...
    if (!m_cleanup_policy.should_cleanup_resource(tracker->handle,
...
std::vector<std::string> ResourceLifecycleManager::get_cleanup_candidates(
...
void ResourceLifecycleManager::set_automatic_cleanup_enabled(bool enabled) {
    ...
    qCDebug(lifecycleLog) << "Automatic cleanup"
...
void ResourceLifecycleManager::notify_state_change(
...
bool ResourceLifecycleManager::has_critical_dependents(
    const std::string& resource_id) const {
    std::shared_lock<std::shared_mutex> lock(m_dependencies_mutex);
...
std::vector<std::string> ResourceLifecycleManager::get_cleanup_order(
...
void ResourceLifecycleManager::cleanup_resource_internal(
...
bool ResourceLifecycleManager::is_state_transition_valid(
    LifecycleState from, LifecycleState to) const {
    return is_valid_state_transition(from, to);
}

// Factory function
std::unique_ptr<IResourceLifecycleManager> create_resource_lifecycle_manager(
    QObject* parent) {

...
public:
    ...
    qtplugin::expected<void, VersionError> install_version(
        ...
        const std::filesystem::path& file_path, bool replace_existing = false) override;

    qtplugin::expected<void, VersionError> uninstall_version(
        std::string_view plugin_id, const Version& version, bool force = false) override;
    ...
    qtplugin::expected<void, VersionError> register_migration(
        ...
        std::function<qtplugin::expected<void, PluginError>(const MigrationContext&)> migrator) override;

    bool is_migration_available(
        std::string_view plugin_id, const Version& from_version,
    ...
    qtplugin::expected<PluginVersionInfo, VersionError> get_version_info(
        std::string_view plugin_id, std::optional<Version> version = std::nullopt) const override;
    ...
    QJsonObject get_storage_usage(std::string_view plugin_id = {}) const override;

    // === Event Notifications ===
    std::string register_version_event_callback(
        std::function<void(const std::string&, const Version&, VersionInstallStatus)> callback) override;

    void unregister_version_event_callback(const std::string& subscription_id) override;
...
signals:
    ...
    void migration_started(const QString& plugin_id, const QString& from_version, const QString& to_version);
    void migration_completed(const QString& plugin_id, const QString& from_version, const QString& to_version);
    void rollback_started(const QString& plugin_id, const QString& target_version);
    void rollback_completed(const QString& plugin_id, const QString& target_version);
...
private:
    ...
    struct MigrationRule {
        ...
        std::function<qtplugin::expected<void, PluginError>(const MigrationContext&)> migrator;
    ...
    struct EventSubscription {
        std::string id;
        std::function<void(const std::string&, const Version&, VersionInstallStatus)> callback;
    };
    ...
    std::unordered_map<std::string, std::vector<PluginVersionInfo>> installed_versions_;
    std::unordered_map<std::string, std::string> active_versions_;  // plugin_id -> version_string

    mutable std::shared_mutex migrations_mutex_;
    std::unordered_map<std::string, std::vector<MigrationRule>> migration_rules_;

    mutable std::shared_mutex compatibility_mutex_;
    std::unordered_map<std::string, std::vector<CompatibilityRule>> compatibility_rules_;
    ...
    std::filesystem::path get_plugin_data_directory(
        std::string_view plugin_id, const Version& version) const;
    std::filesystem::path get_backup_directory(std::string_view plugin_id) const;
    ...
    void notify_version_event(const std::string& plugin_id, const Version& version,
    ...
    int cleanup_plugin_versions(std::vector<PluginVersionInfo>& versions, int keep_count);
    ...
    qtplugin::expected<void, VersionError> migrate_with_transformation(
        const MigrationContext& context);
    QJsonObject transform_configuration(
...
QJsonObject PluginVersionInfo::to_json() const {
    ...
    json["installation_path"] = QString::fromStdString(installation_path.string());
    json["status"] = static_cast<int>(status);
    json["install_time"] = static_cast<qint64>(
        std::chrono::duration_cast<std::chrono::milliseconds>(
            install_time.time_since_epoch()).count());
...
qtplugin::expected<PluginVersionInfo, PluginError> PluginVersionInfo::from_json(
    ...
    try {
        ...
        if (parts.size() >= 3) {
            ...
            return qtplugin::unexpected(PluginError{
                PluginErrorCode::InvalidConfiguration,
                "Invalid version format in PluginVersionInfo JSON: " + version_str
            });
        }
    ...
    if (json.contains("installation_path")) {
        info.installation_path = json["installation_path"].toString().toStdString();
    }
    ...
    if (json.contains("install_time")) {
        auto timestamp = std::chrono::milliseconds(json["install_time"].toVariant().toLongLong());
        info.install_time = std::chrono::system_clock::time_point(timestamp);
    }

    if (json.contains("last_used")) {
        auto timestamp = std::chrono::milliseconds(json["last_used"].toVariant().toLongLong());
    ...
    if (json.contains("migration_script")) {
        info.migration_script = json["migration_script"].toString().toStdString();
    }
    ...
    if (json.contains("usage_count")) {
        info.usage_count = static_cast<size_t>(json["usage_count"].toVariant().toLongLong());
    }
...
void PluginVersionManager::initialize() {
    ...
    storage_directory_ = std::filesystem::path(app_data_dir.toStdString()) / "plugin_versions";
    ...
    if (logger_) {
        logger_->log(LogLevel::Info, "PluginVersionManager",
                    "Version manager initialized with storage at: " + storage_directory_.string());
    }
}
...
qtplugin::expected<void, VersionError> PluginVersionManager::install_version(
    ...
    if (logger_) {
        logger_->log(LogLevel::Info, "PluginVersionManager",
                    std::string("Installing plugin version: ") + std::string(plugin_id) +
                    " v" + version.to_string());
    }
    ...
    if (existing != versions.end() && !replace_existing) {
        return qtplugin::unexpected(VersionError{
            ...
            "Version " + version.to_string() + " already exists for plugin " + std::string(plugin_id),
    ...
    notify_version_event(std::string(plugin_id), version, VersionInstallStatus::Installed);
    ...
    if (logger_) {
        logger_->log(LogLevel::Info, "PluginVersionManager",
                    "Successfully installed plugin version: " + std::string(plugin_id) +
                    " v" + version.to_string());
    }
...
qtplugin::expected<void, VersionError> PluginVersionManager::uninstall_version(
    ...
    if (logger_) {
        logger_->log(LogLevel::Info, "PluginVersionManager",
                    std::string("Uninstalling plugin version: ") + std::string(plugin_id) +
                    " v" + version.to_string());
    }
    ...
    if (it == versions.end()) {
        return qtplugin::unexpected(VersionError{
            ...
            "Version " + version.to_string() + " not found for plugin " + std::string(plugin_id),
            std::string(plugin_id),
            version
        });
    }
    ...
    if (it->is_active && !force) {
        return qtplugin::unexpected(VersionError{
            ...
            "Cannot uninstall active version " + version.to_string() + " for plugin " + std::string(plugin_id),
            std::string(plugin_id),
            version
        });
    }
    ...
    // Notify event subscribers
    notify_version_event(std::string(plugin_id), version, VersionInstallStatus::NotInstalled);
    ...
    if (logger_) {
        logger_->log(LogLevel::Info, "PluginVersionManager",
                    "Successfully uninstalled plugin version: " + std::string(plugin_id) +
                    " v" + version.to_string());
    }
...
std::optional<PluginVersionInfo> PluginVersionManager::get_active_version(
    ...
    for (const auto& version_info : versions_it->second) {
        if (version_info.version.to_string() == active_version_str && version_info.is_active) {
            return version_info;
        }
    }
...
qtplugin::expected<void, VersionError> PluginVersionManager::set_active_version(
    ...
    if (logger_) {
        logger_->log(LogLevel::Info, "PluginVersionManager",
                    std::string("Setting active version: ") + std::string(plugin_id) +
                    " v" + version.to_string());
    }
    ...
    auto current_active = std::find_if(versions.begin(), versions.end(),
        [](const PluginVersionInfo& info) {
            return info.is_active;
        });

    // Perform migration if requested and there's a current active version
    if (migrate_data && current_active != versions.end() && current_active->version != version) {
        MigrationContext context(plugin_id, current_active->version, version);
        context.data_directory = get_plugin_data_directory(plugin_id, current_active->version);

        auto migration_result = migrate_plugin_data(context);
        if (!migration_result) {
            return qtplugin::unexpected(VersionError{
                VersionErrorCode::MigrationFailed,
                "Failed to migrate data from " + current_active->version.to_string() +
                " to " + version.to_string() + ": " + migration_result.error().message,
                std::string(plugin_id),
                version
            });
        }
    ...
    // Notify event subscribers
    notify_version_event(std::string(plugin_id), version, VersionInstallStatus::Active);
    ...
    if (logger_) {
        logger_->log(LogLevel::Info, "PluginVersionManager",
                    "Successfully activated plugin version: " + std::string(plugin_id) +
                    " v" + version.to_string());
    }
...
std::filesystem::path PluginVersionManager::get_plugin_data_directory(
    std::string_view plugin_id, const Version& version) const {
    return storage_directory_ / std::string(plugin_id) / version.to_string() / "data";
}

std::filesystem::path PluginVersionManager::get_backup_directory(std::string_view plugin_id) const {
...
qtplugin::expected<void, VersionError> PluginVersionManager::copy_plugin_files(
    ...
    try {
        if (std::filesystem::is_regular_file(source)) {
            ...
            std::filesystem::copy_file(source, dest_file,
...
qtplugin::expected<void, VersionError> PluginVersionManager::validate_plugin_installation(
    std::string_view plugin_id, const Version& version) {
    ...
    // Additional validation could be added here (e.g., checking for required files)
        ...
        try {
            ...
            if (logger_) {
                logger_->log(LogLevel::Warning, "PluginVersionManager",
                            "Exception in version event callback: " + std::string(e.what()));
            }
        }
    }
}
...
void PluginVersionManager::load_version_database() {
    ...
    try {
        ...
        if (!file.open(QIODevice::ReadOnly)) {
            if (logger_) {
                logger_->log(LogLevel::Warning, "PluginVersionManager",
                            "Failed to open version database file: " + db_file.string());
            }
            return;
        }
        ...
        if (root.contains("installed_versions")) {
            ...
            for (auto it = installed.begin(); it != installed.end(); ++it) {
                ...
                for (const auto& version_value : versions_array) {
                    auto version_result = PluginVersionInfo::from_json(version_value.toObject());
                    if (version_result) {
        ...
        if (root.contains("active_versions")) {
            ...
            for (auto it = active.begin(); it != active.end(); ++it) {
                active_versions_[it.key().toStdString()] = it.value().toString().toStdString();
            }
        }
...
void PluginVersionManager::save_version_database() {
    ...
    try {
        ...
        for (const auto& [plugin_id, version_str] : active_versions_) {
            active[QString::fromStdString(plugin_id)] = QString::fromStdString(version_str);
        }
        ...
        if (!file.open(QIODevice::WriteOnly)) {
            if (logger_) {
                logger_->log(LogLevel::Error, "PluginVersionManager",
                            "Failed to open version database file for writing: " + db_file.string());
            }
            return;
        }
...
std::string PluginVersionManager::register_version_event_callback(
    ...
    event_subscriptions_[subscription_id] = EventSubscription{subscription_id, std::move(callback)};

    return subscription_id;
}

void PluginVersionManager::unregister_version_event_callback(const std::string& subscription_id) {
    std::unique_lock lock(events_mutex_);
...
qtplugin::expected<void, VersionError> PluginVersionManager::set_storage_directory(
...
QJsonObject PluginVersionManager::get_version_statistics() const {
    ...
    try {
        ...
        for (const auto& entry : std::filesystem::recursive_directory_iterator(storage_directory_)) {
...
qtplugin::expected<void, VersionError> PluginVersionManager::migrate_plugin_data(
    const MigrationContext& context) {

    if (logger_) {
        logger_->log(LogLevel::Info, "PluginVersionManager",
                    "Starting migration for plugin " + context.plugin_id +
                    " from " + context.from_version.to_string() +
                    " to " + context.to_version.to_string());
    }

    // Emit migration started signal
    emit migration_started(QString::fromStdString(context.plugin_id),
                          QString::fromStdString(context.from_version.to_string()),
                          QString::fromStdString(context.to_version.to_string()));

    // Notify event subscribers
    notify_version_event(context.plugin_id, context.from_version, VersionInstallStatus::Migrating);
    ...
    if (result) {
        ...
        if (logger_) {
            logger_->log(LogLevel::Info, "PluginVersionManager",
                        "Migration completed successfully for plugin " + context.plugin_id);
        }

        // Emit migration completed signal
        emit migration_completed(QString::fromStdString(context.plugin_id),
                                QString::fromStdString(context.from_version.to_string()),
                                QString::fromStdString(context.to_version.to_string()));
    } else {
        // Migration failed
        if (logger_) {
            logger_->log(LogLevel::Error, "PluginVersionManager",
                        "Migration failed for plugin " + context.plugin_id + ": " + result.error().message);
        }
    }
...
qtplugin::expected<void, VersionError> PluginVersionManager::register_migration(
    ...
    if (logger_) {
        logger_->log(LogLevel::Info, "PluginVersionManager",
                    "Registered migration rule for plugin " + std::string(plugin_id) +
                    " from " + from_version.to_string() + " to " + to_version.to_string());
    }
...
bool PluginVersionManager::is_migration_available(
    ...
    if (it == migration_rules_.end()) {
        ...
        return determine_compatibility_level(from_version, to_version) != CompatibilityLevel::Breaking;
    }
    ...
    // This is a simplified implementation; a full implementation would use graph algorithms
    for (const auto& rule1 : rules) {
        if (rule1.from_version == from_version) {
            for (const auto& rule2 : rules) {
                if (rule2.from_version == rule1.to_version && rule2.to_version == to_version) {
                    return true;
                }
            }
        }
...
qtplugin::expected<void, VersionError> PluginVersionManager::perform_automatic_migration(
    const MigrationContext& context) {
    ...
    // For automatic migration, we copy configuration files and attempt basic transformations
...
qtplugin::expected<void, VersionError> PluginVersionManager::perform_script_migration(
    const MigrationContext& context) {
    ...
    for (const auto& rule : it->second) {
        if (rule.from_version == context.from_version &&
            rule.to_version == context.to_version &&
            rule.script_path) {
...
qtplugin::expected<void, VersionError> PluginVersionManager::perform_callback_migration(
    const MigrationContext& context) {
...
qtplugin::expected<void, VersionError> PluginVersionManager::migrate_configuration_files(
    const MigrationContext& context) {

    auto source_config_dir = get_plugin_data_directory(context.plugin_id, context.from_version);
    auto target_config_dir = get_plugin_data_directory(context.plugin_id, context.to_version);

    try {
        if (std::filesystem::exists(source_config_dir)) {
            std::filesystem::create_directories(target_config_dir);
            std::filesystem::copy(source_config_dir, target_config_dir,
                std::filesystem::copy_options::recursive |
                std::filesystem::copy_options::overwrite_existing);
        }
        return {};
...
qtplugin::expected<void, VersionError> PluginVersionManager::migrate_with_transformation(
    const MigrationContext& context) {

    // Apply configuration transformations based on version differences
    auto config_file = get_plugin_data_directory(context.plugin_id, context.to_version) / "config.json";
...
QJsonObject PluginVersionManager::transform_configuration(
    ...
    // In a real implementation, this would apply version-specific transformations
    QJsonObject transformed = source_config;

    // Add version metadata
    transformed["migrated_from"] = QString::fromStdString(from_version.to_string());
    transformed["migrated_to"] = QString::fromStdString(to_version.to_string());
    transformed["migration_timestamp"] = QDateTime::currentDateTime().toString(Qt::ISODate);
...
qtplugin::expected<RollbackInfo, VersionError> PluginVersionManager::create_backup(
    std::string_view plugin_id, const Version& version) {
    ...
    if (logger_) {
        logger_->log(LogLevel::Info, "PluginVersionManager",
                    "Creating backup for plugin " + std::string(plugin_id) + " v" + version.to_string());
    }

    // Create backup info - use constructor
    RollbackInfo backup_info(plugin_id, version, version);  // current and target are same for backup
    ...
    try {
        ...
        for (const auto& entry : std::filesystem::recursive_directory_iterator(backup_dir)) {
            if (entry.is_regular_file()) {
...
qtplugin::expected<void, VersionError> PluginVersionManager::rollback_to_version(
    std::string_view plugin_id, const Version& target_version, bool preserve_user_data) {

    std::unique_lock lock(rollback_mutex_);

    if (logger_) {
        logger_->log(LogLevel::Info, "PluginVersionManager",
                    "Rolling back plugin " + std::string(plugin_id) + " to v" + target_version.to_string());
    }
    ...
    auto backup_it = std::find_if(it->second.begin(), it->second.end(),
        [&target_version](const RollbackInfo& info) {
            return info.current_version == target_version || info.target_version == target_version;
        });
    ...
    try {
        // Restore from backup
        auto version_dir = get_plugin_version_directory(plugin_id, target_version);
        ...
        if (!preserve_user_data) {
            auto data_dir = get_plugin_data_directory(plugin_id, target_version);
    ...
    // Update version status - deactivate all versions first, then activate target
    {
        ...
        // Activate target version
        auto version_it = std::find_if(versions.begin(), versions.end(),
        ...
        if (version_it != versions.end()) {
            ...
            active_versions_[std::string(plugin_id)] = target_version.to_string();
        }
    }
    ...
    if (logger_) {
        logger_->log(LogLevel::Info, "PluginVersionManager",
                    "Successfully rolled back plugin " + std::string(plugin_id) +
                    " to v" + target_version.to_string());
    }
...
int PluginVersionManager::cleanup_old_backups(std::string_view plugin_id, int keep_count) {
    std::unique_lock lock(rollback_mutex_);
...
CompatibilityLevel PluginVersionManager::check_compatibility(
    std::string_view plugin_id, const Version& version, const Version& host_version) const {
    ...
    if (it != compatibility_rules_.end()) {
        for (const auto& rule : it->second) {
            if (version >= rule.min_host_version && version <= rule.max_host_version) {
                return rule.level;
            }
        }
    }
...
std::vector<Version> PluginVersionManager::get_compatible_versions(
    ...
    for (const auto& version_info : versions_it->second) {
        auto compat_level = check_compatibility(plugin_id, version_info.version, host_version);
...
qtplugin::expected<void, VersionError> PluginVersionManager::register_compatibility_rules(
    std::string_view plugin_id, const QJsonObject& rules) {
    ...
    try {
        ...
        if (rules.contains("rules") && rules["rules"].isArray()) {
            ...
            for (const auto& rule_value : rules_array) {
                if (!rule_value.isObject()) continue;
                ...
                if (rule_obj.contains("min_host_version")) {
                    auto min_ver_str = rule_obj["min_host_version"].toString().toStdString();
                ...
                if (rule_obj.contains("max_host_version")) {
                    auto max_ver_str = rule_obj["max_host_version"].toString().toStdString();
                ...
                if (rule_obj.contains("level")) {
                    rule.level = static_cast<CompatibilityLevel>(rule_obj["level"].toInt());
                }
        ...
        if (logger_) {
            logger_->log(LogLevel::Info, "PluginVersionManager",
                        ...
                        " compatibility rules for plugin " + std::string(plugin_id));
        }
...
qtplugin::expected<PluginVersionInfo, VersionError> PluginVersionManager::get_version_info(
    std::string_view plugin_id, std::optional<Version> version) const {
    ...
    if (!version) {
        ...
        auto active_it = std::find_if(it->second.begin(), it->second.end(),
            [](const PluginVersionInfo& info) {
                return info.is_active;
            });
        ...
        if (!it->second.empty()) {
            auto latest = std::max_element(it->second.begin(), it->second.end(),
                [](const PluginVersionInfo& a, const PluginVersionInfo& b) {
                    return a.version < b.version;
                });
            return *latest;
        ...
        auto version_it = std::find_if(it->second.begin(), it->second.end(),
            [&version](const PluginVersionInfo& info) {
                return info.version == *version;
            });
    ...
    return qtplugin::unexpected(VersionError{
        VersionErrorCode::VersionNotFound,
        version ? "Version " + version->to_string() + " not found for plugin " + std::string(plugin_id)
...
int PluginVersionManager::cleanup_unused_versions(std::string_view plugin_id, int keep_count) {
    std::unique_lock lock(versions_mutex_);
    ...
    if (cleaned_count > 0) {
        ...
        if (logger_) {
            logger_->log(LogLevel::Info, "PluginVersionManager",
                        "Cleaned up " + std::to_string(cleaned_count) + " unused versions");
        }
    }
...
QJsonObject PluginVersionManager::get_storage_usage(std::string_view plugin_id) const {
    std::shared_lock lock(versions_mutex_);
    ...
    try {
        if (plugin_id.empty()) {
            ...
            for (const auto& [id, versions] : installed_versions_) {
                for (const auto& version_info : versions) {
                    if (std::filesystem::exists(version_info.installation_path)) {
            ...
            usage["total_plugins"] = static_cast<int>(installed_versions_.size());
            ...
            if (it != installed_versions_.end()) {
                ...
                usage["plugin_id"] = QString::fromStdString(std::string(plugin_id));
...
int PluginVersionManager::cleanup_plugin_versions(std::vector<PluginVersionInfo>& versions, int keep_count) {
    int cleaned_count = 0;
    ...
    for (int i = 0; i < to_remove; ++i) {
        ...
        if (!version_to_remove.is_active) {
            try {
                std::filesystem::remove_all(version_to_remove.installation_path);
                cleaned_count++;
            } catch (const std::filesystem::filesystem_error&) {

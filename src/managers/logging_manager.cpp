...
std::string DetailedLogFormatter::format(const LogEntry& entry) const {
    ...
    if (!entry.file.empty()) {
        oss << " (" << entry.file << ":" << entry.line << " in "
            << entry.function << ")";
    }

    if (!entry.context.isEmpty()) {
        oss << " Context: "
            << QJsonDocument(entry.context)
...
qtplugin::expected<void, PluginError> ConsoleOutputHandler::write(
    const std::string& formatted_message, const LogEntry& entry) {
...
FileOutputHandler::FileOutputHandler(const std::filesystem::path& file_path,
    ...
    : m_file_path(file_path), m_max_size(max_size), m_max_backups(max_backups) {
    open_file();
}
...
qtplugin::expected<void, PluginError> FileOutputHandler::write(
    const std::string& formatted_message, const LogEntry& entry) {
...
void FileOutputHandler::rotate_file() {
    ...
    for (size_t i = m_max_backups; i > 1; --i) {
        std::filesystem::path old_backup =
            m_file_path.string() + "." + std::to_string(i - 1);
        std::filesystem::path new_backup =
            m_file_path.string() + "." + std::to_string(i);
...
bool FileOutputHandler::open_file() {
    ...
    m_current_size = std::filesystem::exists(m_file_path)
                         ? std::filesystem::file_size(m_file_path)
                         : 0;
...
LoggingManager::LoggingManager(QObject* parent)
    ...
    connect(m_process_timer.get(), &QTimer::timeout, this,
            &LoggingManager::process_log_queue);
    m_process_timer->start();

    // Add default console output
    m_output_handlers[LogOutput::Console] =
...
qtplugin::expected<void, PluginError> LoggingManager::log(
    LogLevel level, std::string_view category, std::string_view message,
    std::string_view plugin_id, const QJsonObject& context) {
    return log_with_location(level, category, message, "", 0, "", plugin_id,
                             context);
}

qtplugin::expected<void, PluginError> LoggingManager::log_with_location(
    LogLevel level, std::string_view category, std::string_view message,
...
void LoggingManager::set_category_level(std::string_view category,
                                        LogLevel level) {
    std::unique_lock<std::shared_mutex> lock(m_config_mutex);
    m_config.category_levels[std::string(category)] = level;
}

void LoggingManager::set_plugin_level(std::string_view plugin_id,
                                      LogLevel level) {
    std::unique_lock<std::shared_mutex> lock(m_config_mutex);
    m_config.plugin_levels[std::string(plugin_id)] = level;
}

LogLevel LoggingManager::get_effective_level(std::string_view category,
...
qtplugin::expected<void, PluginError> LoggingManager::add_output_handler(
    LogOutput output_type, std::unique_ptr<ILogOutputHandler> handler) {
    if (!handler) {
        return make_error<void>(PluginErrorCode::InvalidArgument,
                                "Output handler cannot be null");
    }
...
qtplugin::expected<void, PluginError> LoggingManager::remove_output_handler(
    LogOutput output_type) {
    {
        std::unique_lock<std::shared_mutex> lock(m_handlers_mutex);
        ...
        if (it == m_output_handlers.end()) {
            return make_error<void>(PluginErrorCode::NotFound,
                                    "Output handler not found: " +
                                        log_output_to_string(output_type));
        }
...
qtplugin::expected<void, PluginError> LoggingManager::remove_filter(
...
qtplugin::expected<void, PluginError> LoggingManager::flush_all() {
    ...
    for (auto& [output_type, handler] : m_output_handlers) {
        if (handler) {
            ...
            if (!result) {
                qCWarning(loggingLog)
                    << "Failed to flush output handler:"
...
QJsonObject LoggingManager::get_statistics() const {
    ...
    for (const auto& [level, count] : m_level_counts) {
        level_counts[QString::fromStdString(log_level_to_string(level))] =
            static_cast<qint64>(count.load());
    }
    ...
    // Queue status
    {
        ...
        stats["recent_entries_count"] =
            static_cast<qint64>(m_recent_entries.size());
    }
    ...
    // Configuration
    {
        std::shared_lock<std::shared_mutex> lock(m_config_mutex);
        stats["global_level"] =
            QString::fromStdString(log_level_to_string(m_config.global_level));
        stats["category_levels_count"] =
            static_cast<qint64>(m_config.category_levels.size());
        stats["plugin_levels_count"] =
            static_cast<qint64>(m_config.plugin_levels.size());
    }

    // Output handlers
    {
        std::shared_lock<std::shared_mutex> lock(m_handlers_mutex);
        stats["output_handlers_count"] =
            static_cast<qint64>(m_output_handlers.size());
        stats["filters_count"] = static_cast<qint64>(m_filters.size());
        stats["formatter"] =
            QString::fromStdString(m_formatter ? m_formatter->name() : "none");
    }

    return stats;
}

std::vector<LogEntry> LoggingManager::get_recent_entries(
    size_t count, std::optional<LogLevel> level_filter) const {
    std::lock_guard<std::mutex> lock(m_queue_mutex);
...
bool LoggingManager::should_log_entry(const LogEntry& entry) const {
    ...
    LogLevel effective_level =
        get_effective_level(entry.category, entry.plugin_id);
    ...
    for (auto& [output_type, handler] : m_output_handlers) {
        if (handler && handler->is_available()) {
            ...
            if (!result) {
                ...
                qCWarning(loggingLog)
                    << "Failed to write to output handler:"
                    << QString::fromStdString(handler->name())
                    << QString::fromStdString(result.error().message);
            }
        }

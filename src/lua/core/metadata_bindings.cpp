...
void register_plugin_metadata_bindings(sol::state& lua) {
    auto metadata_type = lua.new_usertype<qtplugin::PluginMetadata>(
        "PluginMetadata", sol::constructors<qtplugin::PluginMetadata>());
    ...
    // Version constraints
    metadata_type["min_host_version"] =
        &qtplugin::PluginMetadata::min_host_version;
    metadata_type["max_host_version"] =
        &qtplugin::PluginMetadata::max_host_version;
    ...
    // Utility methods
    metadata_type["to_json"] =
        [&lua](const qtplugin::PluginMetadata& metadata) -> sol::object {
        return qtforge_lua::qjson_to_lua(metadata.to_json(), lua);
    };

    metadata_type["from_json"] = [](qtplugin::PluginMetadata& metadata,
                                    const sol::object& json) {
...
void register_lifecycle_bindings(sol::state& lua) {
    ...
    lifecycle["state_to_string"] =
        [](qtplugin::PluginState state) -> std::string {
        switch (state) {
    ...
    lifecycle["string_to_state"] =
        [](const std::string& state_str) -> qtplugin::PluginState {
        if (state_str == "Unloaded")
            return qtplugin::PluginState::Unloaded;
        if (state_str == "Loading")
            return qtplugin::PluginState::Loading;
        if (state_str == "Loaded")
            return qtplugin::PluginState::Loaded;
        if (state_str == "Initializing")
            return qtplugin::PluginState::Initializing;
        if (state_str == "Running")
            return qtplugin::PluginState::Running;
        if (state_str == "Paused")
            return qtplugin::PluginState::Paused;
        if (state_str == "Stopping")
            return qtplugin::PluginState::Stopping;
        if (state_str == "Stopped")
            return qtplugin::PluginState::Stopped;
        if (state_str == "Error")
            return qtplugin::PluginState::Error;
        if (state_str == "Reloading")
            return qtplugin::PluginState::Reloading;
        return qtplugin::PluginState::Unloaded;
    };

    // Plugin capability utilities
    lifecycle["capability_to_string"] =
        [](qtplugin::PluginCapability capability) -> std::string {
        switch (capability) {
    ...
    lifecycle["has_capability"] =
        [](qtplugin::PluginCapabilities capabilities,
           qtplugin::PluginCapability capability) -> bool {
        return (capabilities &
                static_cast<qtplugin::PluginCapabilities>(capability)) != 0;
    };

    lifecycle["add_capability"] = [](qtplugin::PluginCapabilities& capabilities,
                                     qtplugin::PluginCapability capability) {
    ...
    lifecycle["remove_capability"] =
        [](qtplugin::PluginCapabilities& capabilities,
           qtplugin::PluginCapability capability) {
            capabilities &=
                ~static_cast<qtplugin::PluginCapabilities>(capability);
        };

    // Plugin priority utilities
    lifecycle["priority_to_string"] =
        [](qtplugin::PluginPriority priority) -> std::string {
        switch (priority) {
    ...
    lifecycle["priority_to_int"] =
        [](qtplugin::PluginPriority priority) -> int {
        return static_cast<int>(priority);
    };

    // Metadata creation helpers
    lifecycle["create_metadata"] =
        [](const std::string& name, const std::string& description, int major,
           int minor, int patch) -> qtplugin::PluginMetadata {
    ...
    lifecycle["create_basic_metadata"] =
        [](const std::string& name) -> qtplugin::PluginMetadata {
        qtplugin::PluginMetadata metadata;
        metadata.name = name;
        metadata.description = "Lua Plugin";
        metadata.version = qtplugin::Version{1, 0, 0};
        metadata.author = "Unknown";
        metadata.capabilities = static_cast<qtplugin::PluginCapabilities>(
...
void register_metadata_lifecycle_bindings(sol::state& lua) {
    qCDebug(metadataBindingsLog)
        << "Registering metadata and lifecycle bindings...";

    register_plugin_metadata_bindings(lua);
    register_lifecycle_bindings(lua);

    qCDebug(metadataBindingsLog)
        << "Metadata and lifecycle bindings registered successfully";
}
...
void register_metadata_lifecycle_bindings(sol::state& lua) {
    ...
    qCWarning(metadataBindingsLog)
        << "Metadata bindings not available - Lua support not compiled";
}

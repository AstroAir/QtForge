    ...
    switch (value.type()) {
        ...
        case QJsonValue::Array: {
            ...
            for (int i = 0; i < array.size(); ++i) {
                table[i + 1] = qjson_to_lua(array[i], lua); // Lua arrays are 1-indexed
            }
            return table;
        }
        ...
        default:
            qCWarning(qtConversionsLog) << "Unknown QJsonValue type:" << value.type();
            return sol::nil;
    }
}
...
QJsonValue lua_to_qjson(const sol::object& obj) {
    switch (obj.get_type()) {
        ...
        default:
            qCWarning(qtConversionsLog) << "Unknown Lua type:" << static_cast<int>(obj.get_type());
            return QJsonValue::Null;
    }
}
...
sol::object qvariant_to_lua(const QVariant& variant, sol::state& lua) {
    switch (variant.type()) {
        ...
        case QVariant::List: {
            ...
            for (int i = 0; i < list.size(); ++i) {
                table[i + 1] = qvariant_to_lua(list[i], lua); // Lua arrays are 1-indexed
            }
            return table;
        }
        ...
        case QVariant::Map: {
            ...
            for (auto it = map.begin(); it != map.end(); ++it) {
                table[it.key().toStdString()] = qvariant_to_lua(it.value(), lua);
            }
            return table;
        }

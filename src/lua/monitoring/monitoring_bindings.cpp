...
void register_hot_reload_manager_bindings(sol::state& lua) {
    auto hot_reload_type = lua.new_usertype<qtplugin::IPluginHotReloadManager>(
        "IPluginHotReloadManager");

    hot_reload_type["enable_hot_reload"] =
        &qtplugin::IPluginHotReloadManager::enable_hot_reload;
    hot_reload_type["disable_hot_reload"] =
        &qtplugin::IPluginHotReloadManager::disable_hot_reload;
    hot_reload_type["is_hot_reload_enabled"] =
        &qtplugin::IPluginHotReloadManager::is_hot_reload_enabled;
    hot_reload_type["get_hot_reload_plugins"] =
        &qtplugin::IPluginHotReloadManager::get_hot_reload_plugins;
    hot_reload_type["clear"] = &qtplugin::IPluginHotReloadManager::clear;
    hot_reload_type["set_global_hot_reload_enabled"] =
        &qtplugin::IPluginHotReloadManager::set_global_hot_reload_enabled;
    hot_reload_type["is_global_hot_reload_enabled"] =
        &qtplugin::IPluginHotReloadManager::is_global_hot_reload_enabled;

    qCDebug(monitoringBindingsLog)
        << "IPluginHotReloadManager bindings registered";
}
...
void register_metrics_collector_bindings(sol::state& lua) {
    auto metrics_type = lua.new_usertype<qtplugin::IPluginMetricsCollector>(
        "IPluginMetricsCollector");

    metrics_type["start_monitoring"] =
        &qtplugin::IPluginMetricsCollector::start_monitoring;
    metrics_type["stop_monitoring"] =
        &qtplugin::IPluginMetricsCollector::stop_monitoring;
    metrics_type["is_monitoring_active"] =
        &qtplugin::IPluginMetricsCollector::is_monitoring_active;
    metrics_type["clear_metrics"] =
        &qtplugin::IPluginMetricsCollector::clear_metrics;
    metrics_type["set_monitoring_interval"] =
        &qtplugin::IPluginMetricsCollector::set_monitoring_interval;
    metrics_type["get_monitoring_interval"] =
        &qtplugin::IPluginMetricsCollector::get_monitoring_interval;

    qCDebug(monitoringBindingsLog)
        << "IPluginMetricsCollector bindings registered";
}
...
void register_plugin_metrics_bindings(sol::state& lua) {
    auto metrics_type =
        lua.new_usertype<qtplugin::PluginMetrics>("PluginMetrics");

    metrics_type["plugin_id"] = &qtplugin::PluginMetrics::plugin_id;
    metrics_type["load_time_ms"] = &qtplugin::PluginMetrics::load_time_ms;
    metrics_type["initialization_time_ms"] =
        &qtplugin::PluginMetrics::initialization_time_ms;
    metrics_type["total_execution_time_ms"] =
        &qtplugin::PluginMetrics::total_execution_time_ms;
    ...
    metrics_type["memory_usage_bytes"] =
        &qtplugin::PluginMetrics::memory_usage_bytes;
    metrics_type["cpu_usage_percent"] =
        &qtplugin::PluginMetrics::cpu_usage_percent;

    // Timestamp
    metrics_type["last_activity"] =
        sol::property([](const qtplugin::PluginMetrics& metrics) -> double {
...
void register_plugin_health_bindings(sol::state& lua) {
    // Health status enum
    lua.new_enum<qtplugin::PluginHealthStatus>(
        "PluginHealthStatus",
    ...
    // Timestamp
    health_type["last_check"] =
        sol::property([](const qtplugin::PluginHealth& health) -> double {
...
void register_monitoring_bindings(sol::state& lua) {
    ...
    // Utility functions
    monitoring["health_status_to_string"] =
        [](qtplugin::PluginHealthStatus status) -> std::string {
        switch (status) {
            case qtplugin::PluginHealthStatus::Unknown:
                return "Unknown";
            case qtplugin::PluginHealthStatus::Healthy:
                return "Healthy";
            case qtplugin::PluginHealthStatus::Warning:
                return "Warning";
            case qtplugin::PluginHealthStatus::Critical:
                return "Critical";
            case qtplugin::PluginHealthStatus::Unhealthy:
                return "Unhealthy";
            default:
                return "Unknown";
        }
    ...
    monitoring["create_health"] = [](const std::string& plugin_id,
    ...
    monitoring["get_current_memory_usage"] = []() -> size_t {
        // Basic implementation - in real scenario would use platform-specific
        // APIs
    ...
    qCDebug(monitoringBindingsLog)
        << "Monitoring bindings registered successfully";
}
...
void register_monitoring_bindings(sol::state& lua) {
    (void)lua;
    qCWarning(monitoringBindingsLog)
        << "Monitoring bindings not available - Lua support not compiled";
}

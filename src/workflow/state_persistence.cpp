...
Q_LOGGING_CATEGORY(workflowStatePersistenceLog, "qtplugin.workflow.state_persistence")
...
qtplugin::expected<WorkflowStepState, PluginError> WorkflowStepState::from_json(const QJsonObject& json) {
    WorkflowStepState state;
    
    if (!json.contains("step_id") || !json["step_id"].isString()) {
        return make_error<WorkflowStepState>(PluginErrorCode::InvalidFormat, "Missing or invalid step_id");
    }
    ...
    if (json.contains("start_time") && json["start_time"].isString()) {
        state.start_time = QDateTime::fromString(json["start_time"].toString(), Qt::ISODate);
    }
    
    if (json.contains("end_time") && json["end_time"].isString()) {
        state.end_time = QDateTime::fromString(json["end_time"].toString(), Qt::ISODate);
    }
...
qtplugin::expected<WorkflowExecutionContext, PluginError> WorkflowExecutionContext::from_json(const QJsonObject& json) {
    WorkflowExecutionContext context;
    
    // Required fields
    if (!json.contains("execution_id") || !json["execution_id"].isString()) {
        return make_error<WorkflowExecutionContext>(PluginErrorCode::InvalidFormat, "Missing or invalid execution_id");
    }
    context.execution_id = json["execution_id"].toString();
    
    if (!json.contains("workflow_id") || !json["workflow_id"].isString()) {
        return make_error<WorkflowExecutionContext>(PluginErrorCode::InvalidFormat, "Missing or invalid workflow_id");
    }
    ...
    if (json.contains("state") && json["state"].isDouble()) {
        context.state = static_cast<WorkflowExecutionState>(json["state"].toInt());
    }
    ...
    if (json.contains("start_time") && json["start_time"].isString()) {
        context.start_time = QDateTime::fromString(json["start_time"].toString(), Qt::ISODate);
    }
    
    if (json.contains("end_time") && json["end_time"].isString()) {
        context.end_time = QDateTime::fromString(json["end_time"].toString(), Qt::ISODate);
    }
    
    if (json.contains("current_step_id") && json["current_step_id"].isString()) {
        context.current_step_id = json["current_step_id"].toString();
    }
    
    if (json.contains("execution_metadata") && json["execution_metadata"].isObject()) {
        context.execution_metadata = json["execution_metadata"].toObject();
    }
    
    if (json.contains("is_transactional") && json["is_transactional"].isBool()) {
    ...
    if (json.contains("step_states") && json["step_states"].isObject()) {
        ...
        for (auto it = step_states_json.begin(); it != step_states_json.end(); ++it) {
            ...
            if (!step_state_result) {
                return qtplugin::unexpected<PluginError>(step_state_result.error());
            }
...
void WorkflowExecutionContext::update_step_state(const QString& step_id, const WorkflowStepState& state) {
    ...
    qCDebug(workflowStatePersistenceLog) << "Updated step state for execution:" << execution_id
                                         << "step:" << step_id << "state:" << static_cast<int>(state.state);
}

std::optional<WorkflowStepState> WorkflowExecutionContext::get_step_state(const QString& step_id) const {
...
qtplugin::expected<WorkflowCheckpoint, PluginError> WorkflowCheckpoint::from_json(const QJsonObject& json) {
    WorkflowCheckpoint checkpoint;
    
    if (!json.contains("checkpoint_id") || !json["checkpoint_id"].isString()) {
        return make_error<WorkflowCheckpoint>(PluginErrorCode::InvalidFormat, "Missing or invalid checkpoint_id");
    }
    checkpoint.checkpoint_id = json["checkpoint_id"].toString();
    
    if (!json.contains("execution_id") || !json["execution_id"].isString()) {
        return make_error<WorkflowCheckpoint>(PluginErrorCode::InvalidFormat, "Missing or invalid execution_id");
    }
    checkpoint.execution_id = json["execution_id"].toString();
    
    if (json.contains("timestamp") && json["timestamp"].isString()) {
        checkpoint.timestamp = QDateTime::fromString(json["timestamp"].toString(), Qt::ISODate);
    }
    
    if (json.contains("context") && json["context"].isObject()) {
        auto context_result = WorkflowExecutionContext::from_json(json["context"].toObject());
        if (!context_result) {
    ...
    if (json.contains("checkpoint_metadata") && json["checkpoint_metadata"].isObject()) {
...
QJsonObject StatePersistenceConfig::to_json() const {
    ...
    json["checkpoint_interval_ms"] = static_cast<int>(checkpoint_interval.count());
    json["max_checkpoints_per_workflow"] = static_cast<int>(max_checkpoints_per_workflow);
...
qtplugin::expected<StatePersistenceConfig, PluginError> StatePersistenceConfig::from_json(const QJsonObject& json) {
    ...
    if (json.contains("checkpoint_interval_ms") && json["checkpoint_interval_ms"].isDouble()) {
        config.checkpoint_interval = std::chrono::milliseconds(json["checkpoint_interval_ms"].toInt());
    }

    if (json.contains("max_checkpoints_per_workflow") && json["max_checkpoints_per_workflow"].isDouble()) {
        config.max_checkpoints_per_workflow = static_cast<size_t>(json["max_checkpoints_per_workflow"].toInt());
    }

    if (json.contains("storage_directory") && json["storage_directory"].isString()) {
        config.storage_directory = json["storage_directory"].toString();
    }

    if (json.contains("compress_checkpoints") && json["compress_checkpoints"].isBool()) {
        config.compress_checkpoints = json["compress_checkpoints"].toBool();
    }

    if (json.contains("encrypt_checkpoints") && json["encrypt_checkpoints"].isBool()) {
...
FileWorkflowStateStorage::FileWorkflowStateStorage(const QString& base_directory)
    : m_base_directory(base_directory) {

    qCDebug(workflowStatePersistenceLog) << "Created file workflow state storage with base directory:" << base_directory;

    // Ensure base directory exists
    auto result = ensure_directory_exists(m_base_directory);
    if (!result) {
        qCWarning(workflowStatePersistenceLog) << "Failed to create base directory:" << base_directory;
    }
}
...
qtplugin::expected<void, PluginError> FileWorkflowStateStorage::save_checkpoint(
    ...
    qCDebug(workflowStatePersistenceLog) << "Saved checkpoint:" << checkpoint.checkpoint_id
                                         << "for execution:" << checkpoint.execution_id;

    return make_success();
}

qtplugin::expected<WorkflowCheckpoint, PluginError> FileWorkflowStateStorage::load_checkpoint(
    ...
    qCDebug(workflowStatePersistenceLog) << "Loaded checkpoint:" << checkpoint_id;

    return checkpoint_result.value();
}

qtplugin::expected<std::vector<WorkflowCheckpoint>, PluginError> FileWorkflowStateStorage::list_checkpoints(
    ...
    if (!dir.exists()) {
        return std::vector<WorkflowCheckpoint>{}; // Return empty list if directory doesn't exist
    }

    QStringList checkpoint_files = dir.entryList(QStringList() << "checkpoint_*.json", QDir::Files, QDir::Name);
    ...
    for (const QString& filename : checkpoint_files) {
        ...
        auto json_result = load_json_file(checkpoint_path);
        if (!json_result) {
            qCWarning(workflowStatePersistenceLog) << "Failed to load checkpoint file:" << checkpoint_path;
            continue;
        }

        auto checkpoint_result = WorkflowCheckpoint::from_json(json_result.value());
        if (!checkpoint_result) {
            qCWarning(workflowStatePersistenceLog) << "Failed to parse checkpoint file:" << checkpoint_path;
            continue;
        }
    ...
    qCDebug(workflowStatePersistenceLog) << "Listed" << checkpoints.size() << "checkpoints for execution:" << execution_id;

    return checkpoints;
}

qtplugin::expected<void, PluginError> FileWorkflowStateStorage::delete_checkpoint(
    const QString& checkpoint_id) {
    ...
    if (!file.exists()) {
        return make_error<void>(PluginErrorCode::FileNotFound, "Checkpoint file not found: " + checkpoint_id.toStdString());
    }

    if (!file.remove()) {
        return make_error<void>(PluginErrorCode::FileSystemError,
                                "Failed to delete checkpoint file: " + file.errorString().toStdString());
    }

    qCDebug(workflowStatePersistenceLog) << "Deleted checkpoint:" << checkpoint_id;

    return make_success();
}

qtplugin::expected<void, PluginError> FileWorkflowStateStorage::save_execution_context(
    ...
    qCDebug(workflowStatePersistenceLog) << "Saved execution context:" << context.execution_id;

    return make_success();
}

qtplugin::expected<WorkflowExecutionContext, PluginError> FileWorkflowStateStorage::load_execution_context(
    const QString& execution_id) {
    ...
    auto context_result = WorkflowExecutionContext::from_json(json_result.value());
    if (!context_result) {
        return qtplugin::unexpected<PluginError>(context_result.error());
    }

    qCDebug(workflowStatePersistenceLog) << "Loaded execution context:" << execution_id;

    return context_result.value();
}

qtplugin::expected<void, PluginError> FileWorkflowStateStorage::delete_execution_context(
    const QString& execution_id) {
    ...
    if (!dir.exists()) {
        return make_error<void>(PluginErrorCode::FileNotFound, "Execution directory not found: " + execution_id.toStdString());
    }

    // Remove all files in the execution directory
    if (!dir.removeRecursively()) {
        return make_error<void>(PluginErrorCode::FileSystemError,
                                "Failed to delete execution directory: " + execution_id.toStdString());
    }

    qCDebug(workflowStatePersistenceLog) << "Deleted execution context and all checkpoints for:" << execution_id;

    return make_success();
}

qtplugin::expected<void, PluginError> FileWorkflowStateStorage::cleanup_old_checkpoints(
    ...
    QDateTime cutoff_time = QDateTime::currentDateTime().addSecs(-static_cast<qint64>(max_age.count() * 3600));

    QStringList execution_dirs = base_dir.entryList(QDir::Dirs | QDir::NoDotAndDotDot);
    ...
    for (const QString& execution_dir : execution_dirs) {
        ...
        if (context_info.exists() && context_info.lastModified() < cutoff_time) {
            if (exec_dir.removeRecursively()) {
                cleaned_count++;
                qCDebug(workflowStatePersistenceLog) << "Cleaned up old execution:" << execution_dir;
            } else {
                qCWarning(workflowStatePersistenceLog) << "Failed to clean up execution:" << execution_dir;
            }
        }
    }

    qCDebug(workflowStatePersistenceLog) << "Cleaned up" << cleaned_count << "old executions";
...
QString FileWorkflowStateStorage::get_checkpoint_path(const QString& checkpoint_id) const {
    ...
    if (parts.size() >= 2) {
        ...
        return QDir(get_execution_directory(execution_id)).absoluteFilePath(QString("checkpoint_%1.json").arg(checkpoint_id));
    }

    // Fallback: use checkpoint_id directly
    return QDir(m_base_directory).absoluteFilePath(QString("checkpoint_%1.json").arg(checkpoint_id));
}

QString FileWorkflowStateStorage::get_execution_context_path(const QString& execution_id) const {
    return QDir(get_execution_directory(execution_id)).absoluteFilePath("context.json");
}

QString FileWorkflowStateStorage::get_execution_directory(const QString& execution_id) const {
    return QDir(m_base_directory).absoluteFilePath(execution_id);
}

qtplugin::expected<void, PluginError> FileWorkflowStateStorage::ensure_directory_exists(const QString& path) const {
    QDir dir;
    if (!dir.mkpath(path)) {
        return make_error<void>(PluginErrorCode::FileSystemError, "Failed to create directory: " + path.toStdString());
    }
    return make_success();
}

qtplugin::expected<QJsonObject, PluginError> FileWorkflowStateStorage::load_json_file(const QString& file_path) const {
    ...
    if (!doc.isObject()) {
        return make_error<QJsonObject>(PluginErrorCode::InvalidFormat, "JSON document is not an object");
    }

    return doc.object();
}

qtplugin::expected<void, PluginError> FileWorkflowStateStorage::save_json_file(const QString& file_path, const QJsonObject& json) const {
    QFile file(file_path);
...
WorkflowCheckpointManager::WorkflowCheckpointManager(
    ...
    qCDebug(workflowStatePersistenceLog) << "Created workflow checkpoint manager with config:"
                                         << "enabled:" << m_config.enabled
                                         << "interval:" << m_config.checkpoint_interval.count() << "ms";
}

void WorkflowCheckpointManager::set_config(const StatePersistenceConfig& config) {
    m_config = config;
    ...
    qCDebug(workflowStatePersistenceLog) << "Updated checkpoint manager config";
}

qtplugin::expected<QString, PluginError> WorkflowCheckpointManager::create_checkpoint(
    const WorkflowExecutionContext& context,
    const QJsonObject& metadata) {

    if (!m_config.enabled) {
        return make_error<QString>(PluginErrorCode::InvalidConfiguration, "Checkpoint persistence is disabled");
    }
    ...
    if (!save_result) {
        emit checkpoint_failed(context.execution_id, QString::fromStdString(save_result.error().message));
    ...
    if (checkpoints_result && checkpoints_result.value().size() > m_config.max_checkpoints_per_workflow) {
        ...
        size_t to_remove = checkpoints.size() - m_config.max_checkpoints_per_workflow;
        for (size_t i = 0; i < to_remove; ++i) {
            auto delete_result = m_storage->delete_checkpoint(checkpoints[i].checkpoint_id);
            if (!delete_result) {
                qCWarning(workflowStatePersistenceLog) << "Failed to delete old checkpoint:"
    ...
    qCDebug(workflowStatePersistenceLog) << "Created checkpoint:" << checkpoint_id
                                         << "for execution:" << context.execution_id;

    return checkpoint_id;
}

qtplugin::expected<WorkflowCheckpoint, PluginError> WorkflowCheckpointManager::load_checkpoint(
    const QString& checkpoint_id) {

    return m_storage->load_checkpoint(checkpoint_id);
}

qtplugin::expected<std::vector<WorkflowCheckpoint>, PluginError> WorkflowCheckpointManager::list_checkpoints(
    const QString& execution_id) {

    return m_storage->list_checkpoints(execution_id);
}

qtplugin::expected<void, PluginError> WorkflowCheckpointManager::delete_checkpoint(
    const QString& checkpoint_id) {

    return m_storage->delete_checkpoint(checkpoint_id);
}

qtplugin::expected<void, PluginError> WorkflowCheckpointManager::save_execution_context(
    const WorkflowExecutionContext& context) {

    auto save_result = m_storage->save_execution_context(context);
    if (!save_result) {
        emit execution_context_failed(context.execution_id, QString::fromStdString(save_result.error().message));
        return save_result;
    }
...
qtplugin::expected<WorkflowExecutionContext, PluginError> WorkflowCheckpointManager::load_execution_context(
    const QString& execution_id) {

    return m_storage->load_execution_context(execution_id);
}

qtplugin::expected<void, PluginError> WorkflowCheckpointManager::delete_execution_context(
    const QString& execution_id) {
...
void WorkflowCheckpointManager::start_automatic_checkpointing(const QString& execution_id) {
    if (!m_config.enabled) {
        qCDebug(workflowStatePersistenceLog) << "Automatic checkpointing disabled, not starting for:" << execution_id;
        return;
    }
    ...
    connect(timer.get(), &QTimer::timeout, this, [this, execution_id]() {
        ...
        if (it != m_active_contexts.end()) {
            ...
            if (!checkpoint_result) {
                qCWarning(workflowStatePersistenceLog) << "Automatic checkpoint failed for execution:"
                                                       << execution_id << "error:" << QString::fromStdString(checkpoint_result.error().message);
            }
        }
    });
    ...
    qCDebug(workflowStatePersistenceLog) << "Started automatic checkpointing for execution:" << execution_id
                                         << "interval:" << m_config.checkpoint_interval.count() << "ms";
}

void WorkflowCheckpointManager::stop_automatic_checkpointing(const QString& execution_id) {
    cleanup_checkpoint_timer(execution_id);
    m_active_contexts.erase(execution_id);

    qCDebug(workflowStatePersistenceLog) << "Stopped automatic checkpointing for execution:" << execution_id;
}

void WorkflowCheckpointManager::update_execution_context(const WorkflowExecutionContext& context) {
    ...
    if (!save_result) {
        qCWarning(workflowStatePersistenceLog) << "Failed to save execution context:" << context.execution_id;
    }
}

qtplugin::expected<void, PluginError> WorkflowCheckpointManager::cleanup_old_checkpoints(
    std::chrono::hours max_age) {
...
void WorkflowCheckpointManager::on_checkpoint_timer() {
    // This slot is not used as we use lambda connections for per-execution timers
}

QString WorkflowCheckpointManager::generate_checkpoint_id(const QString& execution_id) const {
    QString timestamp = QString::number(QDateTime::currentMSecsSinceEpoch());
    return QString("%1_%2").arg(execution_id, timestamp);
}

void WorkflowCheckpointManager::cleanup_checkpoint_timer(const QString& execution_id) {
...
qtplugin::expected<WorkflowRecoveryOptions, PluginError> WorkflowRecoveryOptions::from_json(const QJsonObject& json) {
    WorkflowRecoveryOptions options;

    if (json.contains("strategy") && json["strategy"].isDouble()) {
        options.strategy = static_cast<RecoveryStrategy>(json["strategy"].toInt());
    }

    if (json.contains("specific_checkpoint_id") && json["specific_checkpoint_id"].isString()) {
        options.specific_checkpoint_id = json["specific_checkpoint_id"].toString();
    }

    if (json.contains("validate_checkpoint") && json["validate_checkpoint"].isBool()) {
        options.validate_checkpoint = json["validate_checkpoint"].toBool();
    }

    if (json.contains("resume_execution") && json["resume_execution"].isBool()) {
        options.resume_execution = json["resume_execution"].toBool();
    }

    if (json.contains("recovery_metadata") && json["recovery_metadata"].isObject()) {
...
qtplugin::expected<WorkflowRecoveryResult, PluginError> WorkflowRecoveryResult::from_json(const QJsonObject& json) {
    ...
    if (json.contains("restored_context") && json["restored_context"].isObject()) {
        auto context_result = WorkflowExecutionContext::from_json(json["restored_context"].toObject());
        if (!context_result) {
            return qtplugin::unexpected<PluginError>(context_result.error());
        }
...
WorkflowRecoveryManager::WorkflowRecoveryManager(
    ...
    qCDebug(workflowStatePersistenceLog) << "Created workflow recovery manager";
}

qtplugin::expected<WorkflowRecoveryResult, PluginError> WorkflowRecoveryManager::recover_workflow(
    const QString& execution_id,
    const WorkflowRecoveryOptions& options) {

    qCDebug(workflowStatePersistenceLog) << "Starting workflow recovery for execution:" << execution_id
    ...
    try {
        ...
        auto checkpoint_result = select_checkpoint_by_strategy(execution_id, options);
        if (!checkpoint_result) {
            result.error_message = QString::fromStdString(checkpoint_result.error().message);
        ...
        if (!context_result) {
            result.error_message = QString::fromStdString(context_result.error().message);
            emit recovery_failed(execution_id, result.error_message);
            return result;
        }
        ...
        result.recovery_metadata["recovery_timestamp"] = QDateTime::currentDateTime().toString(Qt::ISODate);
        result.recovery_metadata["original_checkpoint_timestamp"] = checkpoint.timestamp.toString(Qt::ISODate);

        qCDebug(workflowStatePersistenceLog) << "Workflow recovery completed successfully for execution:" << execution_id
        ...
        result.error_message = QString("Recovery failed with exception: %1").arg(e.what());
        emit recovery_failed(execution_id, result.error_message);
        return result;
    }
}

qtplugin::expected<WorkflowRecoveryResult, PluginError> WorkflowRecoveryManager::recover_from_checkpoint(
    const QString& checkpoint_id,
    const WorkflowRecoveryOptions& options) {

    qCDebug(workflowStatePersistenceLog) << "Starting workflow recovery from checkpoint:" << checkpoint_id;

    // Load the specific checkpoint
    auto checkpoint_result = m_checkpoint_manager->load_checkpoint(checkpoint_id);
    if (!checkpoint_result) {
        return qtplugin::unexpected<PluginError>(checkpoint_result.error());
    }
...
qtplugin::expected<bool, PluginError> WorkflowRecoveryManager::validate_checkpoint_for_recovery(
    const WorkflowCheckpoint& checkpoint) {

    // Basic validation checks
    if (checkpoint.checkpoint_id.isEmpty()) {
        return make_error<bool>(PluginErrorCode::InvalidParameters, "Checkpoint ID is empty");
    }

    if (checkpoint.execution_id.isEmpty()) {
        return make_error<bool>(PluginErrorCode::InvalidParameters, "Execution ID is empty");
    }

    if (checkpoint.context.execution_id != checkpoint.execution_id) {
        return make_error<bool>(PluginErrorCode::InvalidConfiguration, "Execution ID mismatch in checkpoint context");
    }
    ...
    if (checkpoint.timestamp < cutoff) {
        qCWarning(workflowStatePersistenceLog) << "Checkpoint is older than 7 days:" << checkpoint.checkpoint_id;
        // Don't fail, just warn
    }

    // Validate execution context
    if (checkpoint.context.workflow_id.isEmpty()) {
        return make_error<bool>(PluginErrorCode::InvalidConfiguration, "Workflow ID is empty in checkpoint context");
    }

    // Check if workflow is in a recoverable state
    if (checkpoint.context.state == WorkflowExecutionState::Completed) {
        return make_error<bool>(PluginErrorCode::InvalidConfiguration, "Cannot recover from completed workflow");
    }

    qCDebug(workflowStatePersistenceLog) << "Checkpoint validation passed for:" << checkpoint.checkpoint_id;
    return true;
}

qtplugin::expected<std::vector<WorkflowCheckpoint>, PluginError> WorkflowRecoveryManager::find_recoverable_checkpoints(
    const QString& execution_id) {

    auto checkpoints_result = m_checkpoint_manager->list_checkpoints(execution_id);
    ...
    qCDebug(workflowStatePersistenceLog) << "Found" << recoverable_checkpoints.size()
                                         << "recoverable checkpoints for execution:" << execution_id;

    return recoverable_checkpoints;
}

qtplugin::expected<WorkflowCheckpoint, PluginError> WorkflowRecoveryManager::find_best_checkpoint(
    const QString& execution_id) {
    ...
    if (checkpoints.empty()) {
        return make_error<WorkflowCheckpoint>(PluginErrorCode::NotFound, "No recoverable checkpoints found");
    }
    ...
    for (const auto& checkpoint : checkpoints) {
        ...
        if (checkpoint.context.state == WorkflowExecutionState::Running ||
            ...
            for (const auto& [step_id, step_state] : checkpoint.context.step_states) {
                if (step_state.state == StepExecutionState::Completed) {
            ...
            if (has_completed_steps) {
                qCDebug(workflowStatePersistenceLog) << "Selected best checkpoint:" << checkpoint.checkpoint_id
    ...
    qCDebug(workflowStatePersistenceLog) << "Using latest checkpoint as best:" << checkpoints[0].checkpoint_id
                                         << "for execution:" << execution_id;
    return checkpoints[0];
}

qtplugin::expected<WorkflowExecutionContext, PluginError> WorkflowRecoveryManager::prepare_recovery_context(
    const WorkflowCheckpoint& checkpoint,
    const WorkflowRecoveryOptions& options) {
    ...
    recovery_context.execution_metadata["recovery_checkpoint_id"] = checkpoint.checkpoint_id;
    recovery_context.execution_metadata["recovery_timestamp"] = QDateTime::currentDateTime().toString(Qt::ISODate);
    recovery_context.execution_metadata["recovery_strategy"] = static_cast<int>(options.strategy);

    // Merge recovery options metadata
    for (auto it = options.recovery_metadata.begin(); it != options.recovery_metadata.end(); ++it) {
        recovery_context.execution_metadata[QString("recovery_%1").arg(it.key())] = it.value();
    }
    ...
    qCDebug(workflowStatePersistenceLog) << "Prepared recovery context for execution:" << recovery_context.execution_id
                                         << "from checkpoint:" << checkpoint.checkpoint_id;

    return recovery_context;
}

qtplugin::expected<WorkflowCheckpoint, PluginError> WorkflowRecoveryManager::select_checkpoint_by_strategy(
    const QString& execution_id,
    const WorkflowRecoveryOptions& options) {
    ...
    switch (options.strategy) {
        case RecoveryStrategy::RestoreFromLatest: {
            ...
            return checkpoints[0]; // Already sorted by timestamp (newest first)
        }

        case RecoveryStrategy::RestoreFromSpecific: {
            if (options.specific_checkpoint_id.isEmpty()) {
                return make_error<WorkflowCheckpoint>(PluginErrorCode::InvalidParameters,
                                                      "Specific checkpoint ID required for RestoreFromSpecific strategy");
            }

            return m_checkpoint_manager->load_checkpoint(options.specific_checkpoint_id);
        }
        ...
        case RecoveryStrategy::RestartFromBeginning: {
            ...
            // This would typically involve loading the original workflow definition
            return make_error<WorkflowCheckpoint>(PluginErrorCode::NotImplemented,
                                                  "RestartFromBeginning strategy not yet implemented");
        }

        default:
            return make_error<WorkflowCheckpoint>(PluginErrorCode::InvalidParameters, "Unknown recovery strategy");
    }
}

bool WorkflowRecoveryManager::is_checkpoint_valid_for_recovery(const WorkflowCheckpoint& checkpoint) {
    auto validation_result = validate_checkpoint_for_recovery(checkpoint);
...
WorkflowStatePersistenceConfigManager::WorkflowStatePersistenceConfigManager(QObject* parent)
    : QObject(parent), m_config(create_default_config()) {

    qCDebug(workflowStatePersistenceLog) << "Created workflow state persistence config manager";

    // Try to load existing configuration
    auto load_result = load_config();
    if (!load_result) {
        qCWarning(workflowStatePersistenceLog) << "Failed to load workflow state persistence config, using defaults";
    }
}

qtplugin::expected<void, PluginError> WorkflowStatePersistenceConfigManager::load_config() {
    ...
    QString config_file = QDir(config_dir).absoluteFilePath("workflow_state_persistence.json");

    QFile file(config_file);
    if (!file.exists()) {
        // No existing config, use defaults
        qCDebug(workflowStatePersistenceLog) << "No existing config file, using defaults";
        emit config_loaded();
        return make_success();
    }
    ...
    if (parse_error.error != QJsonParseError::NoError) {
        return make_error<void>(PluginErrorCode::InvalidFormat,
                                "Failed to parse config JSON: " + parse_error.errorString().toStdString());
    }

    if (!doc.isObject()) {
        return make_error<void>(PluginErrorCode::InvalidFormat, "Config file is not a JSON object");
    }
    ...
    qCDebug(workflowStatePersistenceLog) << "Loaded workflow state persistence config from:" << config_file;
    emit config_loaded();
...
qtplugin::expected<void, PluginError> WorkflowStatePersistenceConfigManager::save_config() {
    QString config_dir = QStandardPaths::writableLocation(QStandardPaths::AppConfigLocation);
    QDir dir;
    if (!dir.mkpath(config_dir)) {
        return make_error<void>(PluginErrorCode::FileSystemError, "Failed to create config directory");
    }
    ...
    if (!file.open(QIODevice::WriteOnly)) {
        return make_error<void>(PluginErrorCode::FileSystemError,
                                "Failed to open config file for writing: " + file.errorString().toStdString());
    }
    ...
    if (file.write(data) != data.size()) {
        return make_error<void>(PluginErrorCode::FileSystemError, "Failed to write complete config data");
    }

    file.close();

    qCDebug(workflowStatePersistenceLog) << "Saved workflow state persistence config to:" << config_file;
    emit config_saved();

    return make_success();
}

qtplugin::expected<void, PluginError> WorkflowStatePersistenceConfigManager::reset_to_defaults() {
    StatePersistenceConfig old_config = m_config;
    ...
    qCDebug(workflowStatePersistenceLog) << "Reset workflow state persistence config to defaults";
    emit config_changed(m_config);

    return make_success();
}

void WorkflowStatePersistenceConfigManager::set_config(const StatePersistenceConfig& config) {
    if (m_config.enabled != config.enabled ||
        m_config.checkpoint_interval != config.checkpoint_interval ||
        m_config.max_checkpoints_per_workflow != config.max_checkpoints_per_workflow ||
        ...
        qCDebug(workflowStatePersistenceLog) << "Updated workflow state persistence config";
    }
}
...
void WorkflowStatePersistenceConfigManager::set_enabled(bool enabled) {
    if (m_config.enabled != enabled) {
        ...
        qCDebug(workflowStatePersistenceLog) << "Set workflow state persistence enabled:" << enabled;
    }
}

void WorkflowStatePersistenceConfigManager::set_checkpoint_interval(std::chrono::milliseconds interval) {
    if (m_config.checkpoint_interval != interval) {
        m_config.checkpoint_interval = interval;
        emit config_changed(m_config);

        qCDebug(workflowStatePersistenceLog) << "Set checkpoint interval:" << interval.count() << "ms";
    }
}

void WorkflowStatePersistenceConfigManager::set_max_checkpoints_per_workflow(size_t max_checkpoints) {
    if (m_config.max_checkpoints_per_workflow != max_checkpoints) {
        m_config.max_checkpoints_per_workflow = max_checkpoints;
        emit config_changed(m_config);

        qCDebug(workflowStatePersistenceLog) << "Set max checkpoints per workflow:" << max_checkpoints;
    }
}

void WorkflowStatePersistenceConfigManager::set_storage_directory(const QString& directory) {
    if (m_config.storage_directory != directory) {
        m_config.storage_directory = directory;
        emit config_changed(m_config);

        qCDebug(workflowStatePersistenceLog) << "Set storage directory:" << directory;
    }
}

void WorkflowStatePersistenceConfigManager::set_compress_checkpoints(bool compress) {
    if (m_config.compress_checkpoints != compress) {
        m_config.compress_checkpoints = compress;
        emit config_changed(m_config);

        qCDebug(workflowStatePersistenceLog) << "Set compress checkpoints:" << compress;
    }
}

void WorkflowStatePersistenceConfigManager::set_encrypt_checkpoints(bool encrypt) {
    if (m_config.encrypt_checkpoints != encrypt) {
        m_config.encrypt_checkpoints = encrypt;
        emit config_changed(m_config);

        qCDebug(workflowStatePersistenceLog) << "Set encrypt checkpoints:" << encrypt;
    }
}

qtplugin::expected<void, PluginError> WorkflowStatePersistenceConfigManager::validate_config() const {
...
StatePersistenceConfig WorkflowStatePersistenceConfigManager::create_default_config() {
    StatePersistenceConfig config;
    config.enabled = true;
    ...
    config.storage_directory = QDir(app_data).absoluteFilePath("workflow_state");
...
qtplugin::expected<void, PluginError> WorkflowStatePersistenceConfigManager::ensure_storage_directory() const {
    QDir dir;
    if (!dir.mkpath(m_config.storage_directory)) {
        return make_error<void>(PluginErrorCode::FileSystemError,
                                "Failed to create storage directory: " + m_config.storage_directory.toStdString());
    }
    ...
    if (!dir_info.isWritable()) {
        return make_error<void>(PluginErrorCode::FileSystemError,
                                "Storage directory is not writable: " + m_config.storage_directory.toStdString());
    }

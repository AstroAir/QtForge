...
QJsonObject WorkflowProgressData::to_json() const {
    ...
    json["estimated_completion_time"] = estimated_completion_time.toString(Qt::ISODate);
    json["elapsed_time_ms"] = static_cast<int>(elapsed_time.count());
    json["estimated_remaining_time_ms"] = static_cast<int>(estimated_remaining_time.count());
...
qtplugin::expected<WorkflowProgressData, PluginError> WorkflowProgressData::from_json(const QJsonObject& json) {
    WorkflowProgressData data;
    
    if (!json.contains("execution_id") || !json["execution_id"].isString()) {
        return make_error<WorkflowProgressData>(PluginErrorCode::InvalidFormat, "Missing or invalid execution_id");
    }
    data.execution_id = json["execution_id"].toString();
    
    if (!json.contains("workflow_id") || !json["workflow_id"].isString()) {
        return make_error<WorkflowProgressData>(PluginErrorCode::InvalidFormat, "Missing or invalid workflow_id");
    }
    ...
    if (json.contains("event_type") && json["event_type"].isDouble()) {
        data.event_type = static_cast<WorkflowProgressEventType>(json["event_type"].toInt());
    }
    
    if (json.contains("overall_progress") && json["overall_progress"].isDouble()) {
        data.overall_progress = json["overall_progress"].toDouble();
    }
    
    if (json.contains("completed_steps") && json["completed_steps"].isDouble()) {
    ...
    if (json.contains("current_step_name") && json["current_step_name"].isString()) {
        data.current_step_name = json["current_step_name"].toString();
    }
    
    if (json.contains("start_time") && json["start_time"].isString()) {
        data.start_time = QDateTime::fromString(json["start_time"].toString(), Qt::ISODate);
    }
    
    if (json.contains("current_time") && json["current_time"].isString()) {
        data.current_time = QDateTime::fromString(json["current_time"].toString(), Qt::ISODate);
    }
    
    if (json.contains("estimated_completion_time") && json["estimated_completion_time"].isString()) {
        data.estimated_completion_time = QDateTime::fromString(json["estimated_completion_time"].toString(), Qt::ISODate);
    }
    
    if (json.contains("elapsed_time_ms") && json["elapsed_time_ms"].isDouble()) {
        data.elapsed_time = std::chrono::milliseconds(json["elapsed_time_ms"].toInt());
    }
    
    if (json.contains("estimated_remaining_time_ms") && json["estimated_remaining_time_ms"].isDouble()) {
        data.estimated_remaining_time = std::chrono::milliseconds(json["estimated_remaining_time_ms"].toInt());
    }
...
QJsonObject ProgressTrackingConfig::to_json() const {
    ...
    json["progress_update_interval_ms"] = static_cast<int>(progress_update_interval.count());
    ...
    json["default_message_priority"] = static_cast<int>(default_message_priority);
    return json;
}

qtplugin::expected<ProgressTrackingConfig, PluginError> ProgressTrackingConfig::from_json(const QJsonObject& json) {
    ...
    if (json.contains("publish_workflow_events") && json["publish_workflow_events"].isBool()) {
        config.publish_workflow_events = json["publish_workflow_events"].toBool();
    }
    
    if (json.contains("publish_step_events") && json["publish_step_events"].isBool()) {
        config.publish_step_events = json["publish_step_events"].toBool();
    }
    
    if (json.contains("publish_progress_updates") && json["publish_progress_updates"].isBool()) {
        config.publish_progress_updates = json["publish_progress_updates"].toBool();
    }
    
    if (json.contains("progress_update_interval_ms") && json["progress_update_interval_ms"].isDouble()) {
        config.progress_update_interval = std::chrono::milliseconds(json["progress_update_interval_ms"].toInt());
    }
    
    if (json.contains("include_metadata") && json["include_metadata"].isBool()) {
        config.include_metadata = json["include_metadata"].toBool();
    }
    
    if (json.contains("include_step_data") && json["include_step_data"].isBool()) {
        config.include_step_data = json["include_step_data"].toBool();
    }
    
    if (json.contains("message_bus_topic_prefix") && json["message_bus_topic_prefix"].isString()) {
        config.message_bus_topic_prefix = json["message_bus_topic_prefix"].toString();
    }
    
    if (json.contains("default_message_priority") && json["default_message_priority"].isDouble()) {
        config.default_message_priority = static_cast<MessagePriority>(json["default_message_priority"].toInt());
    }
...
QJsonObject ProgressAggregationData::to_json() const {
    ...
    json["total_execution_time_ms"] = static_cast<int>(total_execution_time.count());
    json["average_execution_time_ms"] = static_cast<int>(average_execution_time.count());
...
qtplugin::expected<ProgressAggregationData, PluginError> ProgressAggregationData::from_json(const QJsonObject& json) {
    ProgressAggregationData data;
    
    if (json.contains("active_workflows") && json["active_workflows"].isDouble()) {
        data.active_workflows = json["active_workflows"].toInt();
    }
    
    if (json.contains("completed_workflows") && json["completed_workflows"].isDouble()) {
        data.completed_workflows = json["completed_workflows"].toInt();
    }
    
    if (json.contains("failed_workflows") && json["failed_workflows"].isDouble()) {
        data.failed_workflows = json["failed_workflows"].toInt();
    }
    
    if (json.contains("cancelled_workflows") && json["cancelled_workflows"].isDouble()) {
        data.cancelled_workflows = json["cancelled_workflows"].toInt();
    }
    
    if (json.contains("average_progress") && json["average_progress"].isDouble()) {
        data.average_progress = json["average_progress"].toDouble();
    }
    
    if (json.contains("total_execution_time_ms") && json["total_execution_time_ms"].isDouble()) {
        data.total_execution_time = std::chrono::milliseconds(json["total_execution_time_ms"].toInt());
    }
    
    if (json.contains("average_execution_time_ms") && json["average_execution_time_ms"].isDouble()) {
        data.average_execution_time = std::chrono::milliseconds(json["average_execution_time_ms"].toInt());
    }
    
    if (json.contains("last_update_time") && json["last_update_time"].isString()) {
        data.last_update_time = QDateTime::fromString(json["last_update_time"].toString(), Qt::ISODate);
    }
    
    if (json.contains("workflow_progress") && json["workflow_progress"].isObject()) {
        QJsonObject workflow_progress_json = json["workflow_progress"].toObject();
        for (auto it = workflow_progress_json.begin(); it != workflow_progress_json.end(); ++it) {
            const QString& execution_id = it.key();
            const QJsonObject& progress_json = it.value().toObject();
            
            auto progress_result = WorkflowProgressData::from_json(progress_json);
            if (progress_result) {
...
WorkflowProgressTracker::WorkflowProgressTracker(
    ...
    connect(m_progress_timer.get(), &QTimer::timeout, this, &WorkflowProgressTracker::on_progress_update_timer);

    qCDebug(workflowProgressLog) << "Created progress tracker for execution:" << m_execution_id
...
void WorkflowProgressTracker::start_tracking() {
    ...
    if (m_config.enabled && m_config.publish_progress_updates) {
        m_progress_timer->setInterval(static_cast<int>(m_config.progress_update_interval.count()));
        m_progress_timer->start();
    }

    qCDebug(workflowProgressLog) << "Started progress tracking for execution:" << m_execution_id;
}
...
void WorkflowProgressTracker::stop_tracking() {
    ...
    qCDebug(workflowProgressLog) << "Stopped progress tracking for execution:" << m_execution_id;
}

void WorkflowProgressTracker::update_progress(const WorkflowProgressData& progress_data) {
    ...
    qCDebug(workflowProgressLog) << "Updated progress for execution:" << m_execution_id
                                 << "progress:" << m_current_progress.overall_progress << "%";
}

void WorkflowProgressTracker::update_step_progress(const QString& step_id, const state::WorkflowStepState& step_state) {
    ...
    if (step_state.state == state::StepExecutionState::Running) {
        ...
        if (step_state.metadata.contains("step_name")) {
            m_current_progress.current_step_name = step_state.metadata["step_name"].toString();
        }
    }
    ...
    qCDebug(workflowProgressLog) << "Updated step progress for execution:" << m_execution_id
...
void WorkflowProgressTracker::report_workflow_started() {
    ...
    qCDebug(workflowProgressLog) << "Reported workflow started for execution:" << m_execution_id;
}

void WorkflowProgressTracker::report_workflow_completed(const QJsonObject& result) {
    m_current_progress.event_type = WorkflowProgressEventType::WorkflowCompleted;
    ...
    qCDebug(workflowProgressLog) << "Reported workflow completed for execution:" << m_execution_id;
}

void WorkflowProgressTracker::report_workflow_failed(const QString& error_message) {
    ...
    qCDebug(workflowProgressLog) << "Reported workflow failed for execution:" << m_execution_id
                                 << "error:" << error_message;
}

void WorkflowProgressTracker::report_workflow_cancelled() {
    m_current_progress.event_type = WorkflowProgressEventType::WorkflowCancelled;
    ...
    qCDebug(workflowProgressLog) << "Reported workflow cancelled for execution:" << m_execution_id;
}

void WorkflowProgressTracker::report_workflow_suspended() {
    m_current_progress.event_type = WorkflowProgressEventType::WorkflowSuspended;
    ...
    qCDebug(workflowProgressLog) << "Reported workflow suspended for execution:" << m_execution_id;
}
...
void WorkflowProgressTracker::report_workflow_resumed() {
    ...
    qCDebug(workflowProgressLog) << "Reported workflow resumed for execution:" << m_execution_id;
}

void WorkflowProgressTracker::report_step_started(const QString& step_id, const QString& step_name) {
    ...
    qCDebug(workflowProgressLog) << "Reported step started for execution:" << m_execution_id
                                 << "step:" << step_id;
}

void WorkflowProgressTracker::report_step_completed(const QString& step_id, const QJsonObject& result) {
    ...
    qCDebug(workflowProgressLog) << "Reported step completed for execution:" << m_execution_id
                                 << "step:" << step_id;
}

void WorkflowProgressTracker::report_step_failed(const QString& step_id, const QString& error_message) {
    auto it = m_step_states.find(step_id);
    if (it != m_step_states.end()) {
    ...
    qCDebug(workflowProgressLog) << "Reported step failed for execution:" << m_execution_id
                                 << "step:" << step_id << "error:" << error_message;
}

void WorkflowProgressTracker::report_step_skipped(const QString& step_id, const QString& reason) {
    auto it = m_step_states.find(step_id);
    if (it != m_step_states.end()) {
    ...
    qCDebug(workflowProgressLog) << "Reported step skipped for execution:" << m_execution_id
                                 << "step:" << step_id << "reason:" << reason;
}

void WorkflowProgressTracker::report_step_retrying(const QString& step_id, int retry_count) {
    auto it = m_step_states.find(step_id);
    if (it != m_step_states.end()) {
        it->second.state = state::StepExecutionState::Running; // Reset to running for retry
    ...
    qCDebug(workflowProgressLog) << "Reported step retrying for execution:" << m_execution_id
                                 << "step:" << step_id << "retry:" << retry_count;
}

void WorkflowProgressTracker::report_checkpoint_created(const QString& checkpoint_id) {
    m_current_progress.event_type = WorkflowProgressEventType::CheckpointCreated;
    m_current_progress.metadata["checkpoint_id"] = checkpoint_id;
    m_current_progress.metadata["checkpoint_time"] = QDateTime::currentDateTime().toString(Qt::ISODate);
    ...
    qCDebug(workflowProgressLog) << "Reported checkpoint created for execution:" << m_execution_id
                                 << "checkpoint:" << checkpoint_id;
}

void WorkflowProgressTracker::report_checkpoint_restored(const QString& checkpoint_id) {
    m_current_progress.event_type = WorkflowProgressEventType::CheckpointRestored;
    m_current_progress.metadata["restored_checkpoint_id"] = checkpoint_id;
    m_current_progress.metadata["restore_time"] = QDateTime::currentDateTime().toString(Qt::ISODate);
    ...
    qCDebug(workflowProgressLog) << "Reported checkpoint restored for execution:" << m_execution_id
                                 << "checkpoint:" << checkpoint_id;
}
...
void WorkflowProgressTracker::calculate_and_update_progress() {
    ...
    if (std::abs(new_progress - m_current_progress.overall_progress) > 0.01) { // Only update if significant change
...
double WorkflowProgressTracker::calculate_overall_progress() const {
    ...
    return (static_cast<double>(completed_steps) / m_step_states.size()) * 100.0;
}

std::chrono::milliseconds WorkflowProgressTracker::estimate_remaining_time() const {
    if (m_current_progress.overall_progress <= 0.0 || m_current_progress.overall_progress >= 100.0) {
    ...
    double time_per_percent = static_cast<double>(elapsed.count()) / m_current_progress.overall_progress;

    return std::chrono::milliseconds(static_cast<long long>(remaining_progress * time_per_percent));
}

void WorkflowProgressTracker::set_config(const ProgressTrackingConfig& config) {
    bool interval_changed = (m_config.progress_update_interval != config.progress_update_interval);
    ...
    qCDebug(workflowProgressLog) << "Updated progress tracking config for execution:" << m_execution_id;
}

void WorkflowProgressTracker::on_progress_update_timer() {
    if (m_tracking_active && m_config.enabled && m_config.publish_progress_updates) {
        update_timing_information();
        publish_progress_message(m_current_progress);
        emit progress_updated(m_current_progress);
    }
...
void WorkflowProgressTracker::publish_progress_message(const WorkflowProgressData& progress_data) {
    // Get message bus instance (this would need to be injected or accessed via singleton)
    // For now, we'll just log the intent
    qCDebug(workflowProgressLog) << "Publishing progress message for execution:" << progress_data.execution_id
    ...
    // WorkflowProgressMessage message("workflow_progress_tracker", progress_data);
    // message_bus->publish(message);
}

void WorkflowProgressTracker::publish_step_progress_message(const QString& step_id, const state::WorkflowStepState& step_state) {
    qCDebug(workflowProgressLog) << "Publishing step progress message for execution:" << m_execution_id
                                 << "step:" << step_id << "state:" << static_cast<int>(step_state.state);

    // In a real implementation, this would publish to the message bus:
    // auto message_bus = MessageBus::instance();
    // WorkflowStepProgressMessage message("workflow_progress_tracker", m_execution_id, step_id, step_state);
    // message_bus->publish(message);
}
...
void WorkflowProgressTracker::update_timing_information() {
    ...
    if (m_current_progress.overall_progress > 0.0 && m_current_progress.overall_progress < 100.0) {
        ...
        m_current_progress.estimated_completion_time = m_current_progress.current_time.addMSecs(
            static_cast<qint64>(estimated_remaining.count()));
        m_current_progress.estimated_remaining_time = estimated_remaining;
    }
}

QString WorkflowProgressTracker::generate_message_topic(const QString& event_type) const {
    return QString("%1.%2.%3").arg(m_config.message_bus_topic_prefix, m_workflow_id, event_type);
}
...
WorkflowProgressAggregator::WorkflowProgressAggregator(QObject* parent)
    ...
    connect(m_aggregation_timer.get(), &QTimer::timeout, this, &WorkflowProgressAggregator::on_aggregation_update_timer);

    qCDebug(workflowProgressLog) << "Created workflow progress aggregator";
}

void WorkflowProgressAggregator::add_workflow_tracker(const QString& execution_id, WorkflowProgressTracker* tracker) {
    if (!tracker) {
        qCWarning(workflowProgressLog) << "Cannot add null workflow tracker for execution:" << execution_id;
        return;
    }
    ...
    connect(tracker, &WorkflowProgressTracker::progress_updated,
            this, &WorkflowProgressAggregator::on_workflow_progress_updated);

    // Initialize with current progress
    m_last_progress_data[execution_id] = tracker->current_progress();
    m_aggregation_data.workflow_progress[execution_id] = tracker->current_progress();

    update_aggregation();

    emit workflow_added(execution_id);

    qCDebug(workflowProgressLog) << "Added workflow tracker for execution:" << execution_id;
}

void WorkflowProgressAggregator::remove_workflow_tracker(const QString& execution_id) {
    ...
    if (tracker_it != m_workflow_trackers.end()) {
        ...
        qCDebug(workflowProgressLog) << "Removed workflow tracker for execution:" << execution_id;
    }
}
...
ProgressAggregationData WorkflowProgressAggregator::get_aggregated_progress() const {
...
void WorkflowProgressAggregator::set_config(const ProgressTrackingConfig& config) {
    m_config = config;
...
std::chrono::milliseconds WorkflowProgressAggregator::total_execution_time() const {
    return m_aggregation_data.total_execution_time;
}

std::chrono::milliseconds WorkflowProgressAggregator::average_execution_time() const {
...
std::vector<QString> WorkflowProgressAggregator::get_active_executions() const {
    ...
    for (const auto& [execution_id, progress_data] : m_aggregation_data.workflow_progress) {
        if (progress_data.event_type == WorkflowProgressEventType::WorkflowStarted ||
            progress_data.event_type == WorkflowProgressEventType::WorkflowResumed ||
            progress_data.event_type == WorkflowProgressEventType::ProgressUpdate) {
...
std::vector<QString> WorkflowProgressAggregator::get_completed_executions() const {
    std::vector<QString> completed_executions;

    for (const auto& [execution_id, progress_data] : m_aggregation_data.workflow_progress) {
        if (progress_data.event_type == WorkflowProgressEventType::WorkflowCompleted) {
...
std::vector<QString> WorkflowProgressAggregator::get_failed_executions() const {
    ...
    for (const auto& [execution_id, progress_data] : m_aggregation_data.workflow_progress) {
        if (progress_data.event_type == WorkflowProgressEventType::WorkflowFailed) {
...
std::optional<WorkflowProgressData> WorkflowProgressAggregator::get_workflow_progress(const QString& execution_id) const {
...
std::vector<WorkflowProgressData> WorkflowProgressAggregator::get_workflows_by_status(WorkflowProgressEventType status) const {
...
void WorkflowProgressAggregator::on_workflow_progress_updated(const WorkflowProgressData& progress_data) {
    const QString& execution_id = progress_data.execution_id;

    // Check for status change
    WorkflowProgressEventType old_status = WorkflowProgressEventType::WorkflowStarted;
    ...
    if (old_status != progress_data.event_type) {
        emit workflow_status_changed(execution_id, old_status, progress_data.event_type);
    }
...
void WorkflowProgressAggregator::calculate_aggregation_statistics() {
    if (m_aggregation_data.workflow_progress.empty()) {
        ...
        m_aggregation_data.average_execution_time = std::chrono::milliseconds(0);
        return;
    }
    ...
    m_aggregation_data.average_progress = total_progress / m_aggregation_data.workflow_progress.size();
...
void WorkflowProgressAggregator::publish_aggregation_message() {
    qCDebug(workflowProgressLog) << "Publishing aggregation message - Active:" << m_aggregation_data.active_workflows
    ...
    // In a real implementation, this would publish to the message bus:
    // auto message_bus = MessageBus::instance();
    // CustomDataMessage message("workflow_progress_aggregator", "aggregation_update", m_aggregation_data.to_json());
    // message_bus->publish(message);
}
...
void WorkflowProgressMonitorManager::add_monitor(const QString& monitor_id, IWorkflowProgressMonitor* monitor) {
    if (!monitor) {
        qCWarning(workflowProgressLog) << "Cannot add null progress monitor:" << monitor_id;
        return;
    }
...
void WorkflowProgressMonitorManager::remove_monitor(const QString& monitor_id) {
    ...
    if (it != m_monitors.end()) {
        ...
        qCDebug(workflowProgressLog) << "Removed progress monitor:" << monitor_id;
    }
}
...
void WorkflowProgressMonitorManager::set_execution_filter(const std::vector<QString>& execution_ids) {
    m_execution_filter = execution_ids;

    qCDebug(workflowProgressLog) << "Set execution filter with" << execution_ids.size() << "entries";
}

void WorkflowProgressMonitorManager::set_workflow_filter(const std::vector<QString>& workflow_ids) {
    m_workflow_filter = workflow_ids;

    qCDebug(workflowProgressLog) << "Set workflow filter with" << workflow_ids.size() << "entries";
}

void WorkflowProgressMonitorManager::set_event_type_filter(const std::vector<WorkflowProgressEventType>& event_types) {
    m_event_type_filter = event_types;

    qCDebug(workflowProgressLog) << "Set event type filter with" << event_types.size() << "entries";
}
...
void WorkflowProgressMonitorManager::on_progress_message_received(const WorkflowProgressMessage& message) {
...
void WorkflowProgressMonitorManager::on_step_progress_message_received(const WorkflowStepProgressMessage& message) {
    ...
    notify_monitors_step_event(message.execution_id(), message.step_id(), message.step_state());
}

bool WorkflowProgressMonitorManager::passes_filters(const WorkflowProgressData& progress_data) const {
...
void WorkflowProgressMonitorManager::notify_monitors_workflow_event(const WorkflowProgressData& progress_data) {
    for (const auto& [monitor_id, monitor] : m_monitors) {
        try {
            ...
            qCWarning(workflowProgressLog) << "Monitor" << monitor_id << "threw exception:" << e.what();
        } catch (...) {
            qCWarning(workflowProgressLog) << "Monitor" << monitor_id << "threw unknown exception";
        }
    }
}

void WorkflowProgressMonitorManager::notify_monitors_step_event(const QString& execution_id, const QString& step_id, const state::WorkflowStepState& step_state) {
    for (const auto& [monitor_id, monitor] : m_monitors) {
        try {
            switch (step_state.state) {
                ...
                case state::StepExecutionState::Completed:
                    monitor->on_step_completed(execution_id, step_id, step_state);
                    break;

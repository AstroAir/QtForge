...
namespace {
Q_LOGGING_CATEGORY(workflow_validator_log, "qtplugin.workflow.validator")
}  // namespace

// Bring orchestration types into scope for convenience
using qtplugin::workflow::orchestration::Workflow;
using qtplugin::workflow::orchestration::WorkflowStep;
...
qtplugin::expected<ValidationIssue, PluginError> ValidationIssue::from_json(
    ...
    if (json.contains("rule_type") && json["rule_type"].isDouble()) {
        issue.rule_type =
            static_cast<ValidationRuleType>(json["rule_type"].toInt());
    }

    if (json.contains("severity") && json["severity"].isDouble()) {
        issue.severity =
            static_cast<ValidationSeverity>(json["severity"].toInt());
    }
    ...
    if (json.contains("detected_time") && json["detected_time"].isString()) {
        issue.detected_time = QDateTime::fromString(
            json["detected_time"].toString(), Qt::ISODate);
    }
...
QJsonObject ValidationResult::to_json() const {
    ...
    json["validation_duration_ms"] =
        static_cast<int>(validation_duration.count());
...
qtplugin::expected<ValidationResult, PluginError> ValidationResult::from_json(
    const QJsonObject& json) {
    ...
    if (json.contains("critical_issues") &&
        json["critical_issues"].isDouble()) {
    ...
    if (json.contains("validation_start_time") &&
        json["validation_start_time"].isString()) {
        result.validation_start_time = QDateTime::fromString(
            json["validation_start_time"].toString(), Qt::ISODate);
    }

    if (json.contains("validation_end_time") &&
        json["validation_end_time"].isString()) {
        result.validation_end_time = QDateTime::fromString(
            json["validation_end_time"].toString(), Qt::ISODate);
    }

    if (json.contains("validation_duration_ms") &&
        json["validation_duration_ms"].isDouble()) {
        result.validation_duration =
            std::chrono::milliseconds(json["validation_duration_ms"].toInt());
    }
    ...
    if (json.contains("execution_recommended") &&
        json["execution_recommended"].isBool()) {
        result.execution_recommended = json["execution_recommended"].toBool();
    }

    if (json.contains("execution_recommendation") &&
        json["execution_recommendation"].isString()) {
        result.execution_recommendation =
            json["execution_recommendation"].toString();
    }
    ...
    if (json.contains("issues") && json["issues"].isArray()) {
        ...
        for (const QJsonValue& value : issues_array) {
            if (value.isObject()) {
                auto issue_result =
                    ValidationIssue::from_json(value.toObject());
                if (issue_result) {
...
DependencyValidator::DependencyValidator(QObject* parent) : QObject(parent) {
    qCDebug(workflow_validator_log) << "Created dependency validator";
}

std::vector<ValidationIssue> DependencyValidator::validate_dependencies(
    ...
    if (has_circular_dependencies(workflow)) {
        ...
        issue.description =
            "The workflow contains circular dependencies that would prevent "
            "execution";
        issue.recommendation =
            "Review and remove circular dependencies between workflow steps";
        ...
        if (!cycle.empty()) {
            ...
            // Convert std::vector<QString> to QStringList for join
            QStringList cycle_list;
            for (const auto& step : cycle) {
                cycle_list.append(step);
            }
            issue.description +=
                QString(". Cycle: %1").arg(cycle_list.join(" -> "));
        }
...
std::vector<ValidationIssue> DependencyValidator::validate_step_dependencies(
    const WorkflowStep& step, const Workflow& workflow) const {
    std::vector<ValidationIssue> issues;

    // Check if all dependencies exist
    const auto& dependencies = step.dependencies;
    const auto& all_steps = workflow.steps();

    std::unordered_set<QString> available_steps;
    for (const auto& [step_id, available_step] : all_steps) {
        available_steps.insert(step_id);
    }
    ...
    for (const QString& dependency : dependencies) {
        if (available_steps.find(dependency) == available_steps.end()) {
            ...
            issue.description = QString(
                                    "Step '%1' depends on step '%2' which does "
                                    "not exist in the workflow")
                                    .arg(step.id, dependency);
            issue.recommendation =
                QString(
                    "Add step '%1' to the workflow or remove the dependency")
                    .arg(dependency);
            issue.detected_time = QDateTime::currentDateTime();

            QJsonObject issue_data;
            issue_data["missing_dependency"] = dependency;
            issue_data["dependent_step"] = step.id;
...
std::vector<ValidationIssue> DependencyValidator::validate_plugin_dependencies(
...
bool DependencyValidator::has_circular_dependencies(
    const Workflow& workflow) const {
    ...
    for (const auto& [node, dependencies] : dependency_graph) {
        if (visited.find(node) == visited.end()) {
            if (has_cycle_dfs(dependency_graph, node, visited, rec_stack,
                              cycle_path)) {
                return true;
            }
        }
...
std::vector<QString> DependencyValidator::find_dependency_cycle(
    const Workflow& workflow) const {
    auto dependency_graph = build_dependency_graph(workflow);
...
std::vector<QString> DependencyValidator::get_execution_order(
    const Workflow& workflow) const {
    auto dependency_graph = build_dependency_graph(workflow);
...
DependencyValidator::build_dependency_graph(const Workflow& workflow) const {
...
bool DependencyValidator::has_cycle_dfs(
...
std::vector<QString> DependencyValidator::topological_sort(
...
PluginValidator::PluginValidator(QObject* parent) : QObject(parent) {
    qCDebug(workflow_validator_log) << "Created plugin validator";
}
...
ConfigurationValidator::ConfigurationValidator(QObject* parent)
    : QObject(parent) {
    qCDebug(workflow_validator_log) << "Created configuration validator";
}

// === ResourceValidator Implementation ===

ResourceValidator::ResourceValidator(QObject* parent) : QObject(parent) {
    qCDebug(workflow_validator_log) << "Created resource validator";
}

// === WorkflowValidator Implementation ===

WorkflowValidator::WorkflowValidator(QObject* parent) : QObject(parent) {
    qCDebug(workflow_validator_log) << "Created workflow validator";
}

WorkflowValidator::~WorkflowValidator() {
    qCDebug(workflow_validator_log) << "Destroyed workflow validator";
}

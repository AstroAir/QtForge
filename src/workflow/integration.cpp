...
namespace {
Q_LOGGING_CATEGORY(workflow_integration_log, "qtplugin.workflow.integration")
}  // namespace
...
UnifiedWorkflow::UnifiedWorkflow(const QString& id, const QString& name)
    ...
    qCDebug(workflow_integration_log) << "Created unified workflow:" << m_id;
}

UnifiedWorkflow& UnifiedWorkflow::set_composition(
    const composition::PluginComposition& comp) {
    m_composition = comp;
    qCDebug(workflow_integration_log)
        << "Set composition for workflow:" << m_id << "with"
        << comp.plugins().size() << "plugins";
    return *this;
}

UnifiedWorkflow& UnifiedWorkflow::add_workflow_step(
    const orchestration::WorkflowStep& step) {
    m_workflow_steps[step.id] = step;
    qCDebug(workflow_integration_log)
        << "Added workflow step:" << step.id << "to workflow:" << m_id;
    return *this;
}
...
UnifiedWorkflow& UnifiedWorkflow::set_transactional(bool transactional) {
    ...
    qCDebug(workflow_integration_log)
        << "Set transactional mode:" << transactional
        << "for workflow:" << m_id;
    return *this;
}

UnifiedWorkflow& UnifiedWorkflow::set_isolation_level(
    transactions::IsolationLevel level) {
    m_isolation_level = level;
    qCDebug(workflow_integration_log)
        << "Set isolation level:" << static_cast<int>(level)
        << "for workflow:" << m_id;
    return *this;
}
...
qtplugin::expected<QJsonObject, PluginError> UnifiedWorkflow::execute(
    ...
    qCDebug(workflow_integration_log) << "Executing unified workflow:" << m_id;
    ...
    try {
        ...
        if (m_composition.has_value()) {
            if (m_transactional) {
                ...
                auto execution_result =
                    WorkflowManager::instance().execute_composite_workflow(
            ...
            if (m_transactional) {
                ...
                auto execution_result =
                    WorkflowManager::instance().execute_transactional_workflow(
                ...
                auto execution_result =
                    orchestrator.execute_workflow(m_id, initial_data, false);
        ...
        qCDebug(workflow_integration_log)
            << "Unified workflow executed successfully:" << m_id;

        return result;

    } catch (const std::exception& e) {
        const QString error_msg =
            QString("Unified workflow execution failed: %1").arg(e.what());
        qCWarning(workflow_integration_log) << error_msg;
...
qtplugin::expected<void, PluginError> UnifiedWorkflow::validate() const {
    ...
    qCDebug(workflow_integration_log)
        << "Unified workflow validation passed:" << m_id;
    return make_success();
}
...
TransactionalComposition::TransactionalComposition(
    ...
    transactions::IsolationLevel isolation, QObject* parent)
    : CompositePlugin(composition, parent), m_isolation_level(isolation) {
    qCDebug(workflow_integration_log)
        << "Created transactional composition with isolation level:"
        << static_cast<int>(isolation);
}

qtplugin::expected<QJsonObject, PluginError>
TransactionalComposition::execute_command(std::string_view command,
                                          const QJsonObject& params) {
    qCDebug(workflow_integration_log)
        << "Executing transactional command:" << command.data();

    // Begin transaction
    auto& transaction_manager =
        transactions::PluginTransactionManager::instance();
    auto transaction_result =
        transaction_manager.begin_transaction(m_isolation_level);
    ...
    try {
        ...
        if (result) {
            ...
            if (!commit_result) {
                qCWarning(workflow_integration_log)
                    << "Failed to commit transaction:" << transaction_id;
                return qtplugin::unexpected<PluginError>(commit_result.error());
            }

            qCDebug(workflow_integration_log)
                << "Transactional command completed successfully:"
                << command.data();
            return result;
        } else {
            ...
            if (!rollback_result) {
                qCWarning(workflow_integration_log)
                    << "Failed to rollback transaction:" << transaction_id;
            }
        ...
        QString error_msg =
            QString("Transactional command execution failed: %1").arg(e.what());
        qCWarning(workflow_integration_log) << error_msg;
...
qtplugin::expected<QJsonObject, PluginError>
TransactionalComposition::execute_batch_commands(
    const std::vector<std::pair<std::string, QJsonObject>>& commands) {
    qCDebug(workflow_integration_log)
        << "Executing batch commands in transaction, count:" << commands.size();

    if (commands.empty()) {
        return make_error<QJsonObject>(PluginErrorCode::InvalidParameters,
    ...
    try {
        ...
        for (size_t i = 0; i < commands.size(); ++i) {
            ...
            qCDebug(workflow_integration_log)
                << "Executing batch command" << i + 1 << "of" << commands.size()
                << ":" << command.c_str();
            ...
            if (!result) {
                ...
                qCWarning(workflow_integration_log)
                    << "Batch command failed at index" << i << ":"
                    << command.c_str();
        ...
        if (!commit_result) {
            qCWarning(workflow_integration_log)
                << "Failed to commit batch transaction:" << transaction_id;
            return qtplugin::unexpected<PluginError>(commit_result.error());
        }
        ...
        qCDebug(workflow_integration_log)
            << "Batch commands completed successfully, count:"
            << commands.size();
        return batch_result;
        ...
        QString error_msg =
            QString("Batch command execution failed: %1").arg(e.what());
        qCWarning(workflow_integration_log) << error_msg;

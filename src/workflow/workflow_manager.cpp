...
namespace {
Q_LOGGING_CATEGORY(workflow_manager_log, "qtplugin.workflow.manager")
}  // namespace
...
WorkflowManager::execute_transactional_workflow(
    ...
    try {
        ...
        qCDebug(workflow_manager_log)
            << "Transactional workflow completed successfully:" << workflow_id
        ...
        const QString error_msg =
            QString("Transactional workflow execution failed: %1")
                .arg(e.what());
        emit unified_workflow_failed(workflow_id, error_msg);

        return make_error<QString>(PluginErrorCode::ExecutionFailed,
                                   error_msg.toStdString());
    }
}

qtplugin::expected<QString, PluginError>
WorkflowManager::execute_composite_workflow(const QString& composition_id,
                                            const QString& workflow_id,
                                            const QJsonObject& initial_data) {
    qCDebug(workflow_manager_log)
        << "Executing composite workflow:" << workflow_id
        << "composition:" << composition_id;

    // Get the composition
    auto composition_result =
        composition_manager().get_composition(composition_id);
    ...
    try {
        ...
        qCDebug(workflow_manager_log)
            << "Composite workflow completed successfully:" << workflow_id
            << "execution:" << execution_result.value();
        ...
        const QString error_msg =
            QString("Composite workflow execution failed: %1").arg(e.what());
        emit unified_workflow_failed(workflow_id, error_msg);

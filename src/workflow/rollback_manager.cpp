...
qtplugin::expected<RollbackOperation, PluginError> RollbackOperation::from_json(const QJsonObject& json) {
    RollbackOperation operation;
    
    if (!json.contains("operation_id") || !json["operation_id"].isString()) {
        return make_error<RollbackOperation>(PluginErrorCode::InvalidFormat, "Missing or invalid operation_id");
    }
    ...
    if (json.contains("created_time") && json["created_time"].isString()) {
        operation.created_time = QDateTime::fromString(json["created_time"].toString(), Qt::ISODate);
    }
    
    if (json.contains("executed_time") && json["executed_time"].isString()) {
        operation.executed_time = QDateTime::fromString(json["executed_time"].toString(), Qt::ISODate);
    }
...
qtplugin::expected<RollbackPlanConfig, PluginError> RollbackPlanConfig::from_json(const QJsonObject& json) {
    RollbackPlanConfig config;
    
    if (json.contains("strategy") && json["strategy"].isDouble()) {
        config.strategy = static_cast<RollbackStrategy>(json["strategy"].toInt());
    }
    
    if (json.contains("validation_level") && json["validation_level"].isDouble()) {
        config.validation_level = static_cast<RollbackValidationLevel>(json["validation_level"].toInt());
    }
    ...
    if (json.contains("include_operations") && json["include_operations"].isArray()) {
    ...
    if (json.contains("exclude_operations") && json["exclude_operations"].isArray()) {
    ...
    if (json.contains("retry_delay_ms") && json["retry_delay_ms"].isDouble()) {
        config.retry_delay = std::chrono::milliseconds(json["retry_delay_ms"].toInt());
    }
    ...
    if (json.contains("validate_before_rollback") && json["validate_before_rollback"].isBool()) {
        config.validate_before_rollback = json["validate_before_rollback"].toBool();
    }
    
    if (json.contains("validate_after_rollback") && json["validate_after_rollback"].isBool()) {
        config.validate_after_rollback = json["validate_after_rollback"].toBool();
    }
    
    if (json.contains("continue_on_validation_failure") && json["continue_on_validation_failure"].isBool()) {
        config.continue_on_validation_failure = json["continue_on_validation_failure"].toBool();
    }
    
    if (json.contains("use_compensation_on_failure") && json["use_compensation_on_failure"].isBool()) {
        config.use_compensation_on_failure = json["use_compensation_on_failure"].toBool();
    }
    
    if (json.contains("create_rollback_checkpoint") && json["create_rollback_checkpoint"].isBool()) {
        config.create_rollback_checkpoint = json["create_rollback_checkpoint"].toBool();
    }
    
    if (json.contains("preserve_partial_results") && json["preserve_partial_results"].isBool()) {
        config.preserve_partial_results = json["preserve_partial_results"].toBool();
    }
...
qtplugin::expected<RollbackExecutionResult, PluginError> RollbackExecutionResult::from_json(const QJsonObject& json) {
    ...
    if (json.contains("strategy") && json["strategy"].isDouble()) {
        result.strategy = static_cast<RollbackStrategy>(json["strategy"].toInt());
    }

    if (json.contains("overall_result") && json["overall_result"].isDouble()) {
        result.overall_result = static_cast<RollbackOperationResult>(json["overall_result"].toInt());
    }

    if (json.contains("operation_results") && json["operation_results"].isArray()) {
        QJsonArray operation_results_array = json["operation_results"].toArray();
        for (const QJsonValue& value : operation_results_array) {
            if (value.isObject()) {
                QJsonObject op_result = value.toObject();
                if (op_result.contains("operation_id") && op_result.contains("result")) {
                    QString operation_id = op_result["operation_id"].toString();
                    RollbackOperationResult op_res = static_cast<RollbackOperationResult>(op_result["result"].toInt());
    ...
    if (json.contains("start_time") && json["start_time"].isString()) {
        result.start_time = QDateTime::fromString(json["start_time"].toString(), Qt::ISODate);
    }

    if (json.contains("end_time") && json["end_time"].isString()) {
        result.end_time = QDateTime::fromString(json["end_time"].toString(), Qt::ISODate);
    }

    if (json.contains("total_duration_ms") && json["total_duration_ms"].isDouble()) {
        result.total_duration = std::chrono::milliseconds(json["total_duration_ms"].toInt());
    }

    if (json.contains("total_operations") && json["total_operations"].isDouble()) {
        result.total_operations = json["total_operations"].toInt();
    }

    if (json.contains("successful_operations") && json["successful_operations"].isDouble()) {
        result.successful_operations = json["successful_operations"].toInt();
    }

    if (json.contains("failed_operations") && json["failed_operations"].isDouble()) {
        result.failed_operations = json["failed_operations"].toInt();
    }

    if (json.contains("skipped_operations") && json["skipped_operations"].isDouble()) {
        result.skipped_operations = json["skipped_operations"].toInt();
    }

    if (json.contains("compensated_operations") && json["compensated_operations"].isDouble()) {
    ...
    if (json.contains("pre_validation_passed") && json["pre_validation_passed"].isBool()) {
        result.pre_validation_passed = json["pre_validation_passed"].toBool();
    }

    if (json.contains("post_validation_passed") && json["post_validation_passed"].isBool()) {
        result.post_validation_passed = json["post_validation_passed"].toBool();
    }

    if (json.contains("validation_warnings") && json["validation_warnings"].isArray()) {
    ...
    if (json.contains("recovery_checkpoint_id") && json["recovery_checkpoint_id"].isString()) {
        result.recovery_checkpoint_id = json["recovery_checkpoint_id"].toString();
    }
...
WorkflowRollbackPlan::WorkflowRollbackPlan(const QString& plan_id, const RollbackPlanConfig& config, QObject* parent)
...
void WorkflowRollbackPlan::add_operation(const RollbackOperation& operation) {
    ...
    qCDebug(workflowRollbackLog) << "Added operation to plan:" << m_plan_id << "operation:" << operation.operation_id;
}
...
void WorkflowRollbackPlan::remove_operation(const QString& operation_id) {
    ...
    if (it != m_operations.end()) {
        ...
        qCDebug(workflowRollbackLog) << "Removed operation from plan:" << m_plan_id << "operation:" << operation_id;
    }
}
...
void WorkflowRollbackPlan::clear_operations() {
    ...
    qCDebug(workflowRollbackLog) << "Cleared all operations from plan:" << m_plan_id;
}
...
std::optional<RollbackOperation> WorkflowRollbackPlan::get_operation(const QString& operation_id) const {
    auto it = m_operations.find(operation_id);
    if (it != m_operations.end()) {
        return it->second;
    }
...
qtplugin::expected<void, PluginError> WorkflowRollbackPlan::validate_plan() const {
    // Check for circular dependencies
    if (has_circular_dependencies()) {
        return make_error<void>(PluginErrorCode::CircularDependency, "Rollback plan has circular dependencies");
    }
    ...
    for (const auto& [operation_id, operation] : m_operations) {
        ...
        if (operation.operation_id.isEmpty()) {
            return make_error<void>(PluginErrorCode::InvalidConfiguration, "Operation has empty operation_id");
        }

        if (operation.plugin_id.isEmpty()) {
            return make_error<void>(PluginErrorCode::InvalidConfiguration, "Operation has empty plugin_id: " + operation.operation_id.toStdString());
        }
        ...
        for (const QString& dep : operation.depends_on) {
            if (m_operations.find(dep) == m_operations.end()) {
                return make_error<void>(PluginErrorCode::DependencyMissing,
                                        "Operation " + operation.operation_id.toStdString() + " depends on non-existent operation: " + dep.toStdString());
            }
        }
    }
...
qtplugin::expected<std::vector<QString>, PluginError> WorkflowRollbackPlan::get_execution_order() const {
...
void WorkflowRollbackPlan::sort_by_dependencies() {
    ...
    qCDebug(workflowRollbackLog) << "Sorted operations by dependencies for plan:" << m_plan_id;
}

void WorkflowRollbackPlan::sort_by_priority() {
    // This would sort operations by priority within dependency constraints
    qCDebug(workflowRollbackLog) << "Sorted operations by priority for plan:" << m_plan_id;
}

void WorkflowRollbackPlan::update_config(const RollbackPlanConfig& new_config) {
    m_config = new_config;

    qCDebug(workflowRollbackLog) << "Updated config for rollback plan:" << m_plan_id;
}

bool WorkflowRollbackPlan::has_circular_dependencies() const {
    // Simple cycle detection using DFS
    std::unordered_map<QString, int> state; // 0=unvisited, 1=visiting, 2=visited

    std::function<bool(const QString&)> has_cycle = [&](const QString& operation_id) -> bool {
...
qtplugin::expected<void, PluginError> WorkflowRollbackManager::initialize() {
    if (m_initialized) {
        return make_error<void>(PluginErrorCode::InvalidState, "Workflow rollback manager already initialized");
    }
...
qtplugin::expected<QString, PluginError> WorkflowRollbackManager::create_rollback_plan(const RollbackPlanConfig& config) {
    if (!m_initialized) {
        return make_error<QString>(PluginErrorCode::InvalidState, "Workflow rollback manager not initialized");
    }
...
qtplugin::expected<void, PluginError> WorkflowRollbackManager::update_rollback_plan(const QString& plan_id, const RollbackPlanConfig& config) {
    auto it = m_rollback_plans.find(plan_id);
    if (it == m_rollback_plans.end()) {
        return make_error<void>(PluginErrorCode::NotFound, "Rollback plan not found: " + plan_id.toStdString());
    }
...
qtplugin::expected<void, PluginError> WorkflowRollbackManager::delete_rollback_plan(const QString& plan_id) {
    auto it = m_rollback_plans.find(plan_id);
    if (it == m_rollback_plans.end()) {
        return make_error<void>(PluginErrorCode::NotFound, "Rollback plan not found: " + plan_id.toStdString());
    }
...
std::optional<WorkflowRollbackPlan*> WorkflowRollbackManager::get_rollback_plan(const QString& plan_id) const {
    auto it = m_rollback_plans.find(plan_id);
    if (it != m_rollback_plans.end()) {
...
qtplugin::expected<QString, PluginError> WorkflowRollbackManager::execute_rollback(const QString& plan_id) {
    auto plan_opt = get_rollback_plan(plan_id);
    if (!plan_opt) {
        return make_error<QString>(PluginErrorCode::NotFound, "Rollback plan not found: " + plan_id.toStdString());
    }
    ...
    qCDebug(workflowRollbackLog) << "Executed rollback for plan:" << plan_id << "rollback_id:" << rollback_id;
    return rollback_id;
}
...
qtplugin::expected<QString, PluginError> WorkflowRollbackManager::execute_immediate_rollback(const RollbackPlanConfig& config) {
...
qtplugin::expected<RollbackExecutionResult, PluginError> WorkflowRollbackManager::get_rollback_result(const QString& rollback_id) const {
    auto it = m_rollback_results.find(rollback_id);
    if (it != m_rollback_results.end()) {
        return it->second;
    }
    return make_error<RollbackExecutionResult>(PluginErrorCode::NotFound, "Rollback result not found: " + rollback_id.toStdString());
}
...
qtplugin::expected<void, PluginError> WorkflowRollbackManager::cancel_rollback(const QString& rollback_id) {
    auto it = m_active_rollbacks.find(rollback_id);
    if (it == m_active_rollbacks.end()) {
        return make_error<void>(PluginErrorCode::NotFound, "Active rollback not found: " + rollback_id.toStdString());
    }
...
qtplugin::expected<QString, PluginError> WorkflowRollbackManager::create_recovery_checkpoint(const QString& execution_id) {
    // Stub implementation
    QString checkpoint_id = "checkpoint_" + QUuid::createUuid().toString(QUuid::WithoutBraces);
    emit recovery_checkpoint_created(checkpoint_id);
    qCDebug(workflowRollbackLog) << "Created recovery checkpoint:" << checkpoint_id << "for execution:" << execution_id;
    return checkpoint_id;
}

qtplugin::expected<void, PluginError> WorkflowRollbackManager::restore_from_recovery_checkpoint(const QString& checkpoint_id) {
    // Stub implementation
    qCDebug(workflowRollbackLog) << "Restored from recovery checkpoint:" << checkpoint_id;
    return make_success();
}

qtplugin::expected<void, PluginError> WorkflowRollbackManager::validate_rollback_feasibility(const QString& execution_id, const RollbackPlanConfig& /* config */) {
    // Stub implementation - basic validation
    if (execution_id.isEmpty()) {
        return make_error<void>(PluginErrorCode::InvalidParameters, "Execution ID cannot be empty");
    }

    qCDebug(workflowRollbackLog) << "Validated rollback feasibility for execution:" << execution_id;
    return make_success();
}

qtplugin::expected<std::vector<QString>, PluginError> WorkflowRollbackManager::get_rollback_dependencies(const QString& execution_id) {
    // Stub implementation
    std::vector<QString> dependencies;
    qCDebug(workflowRollbackLog) << "Retrieved rollback dependencies for execution:" << execution_id;
    return dependencies;
}

qtplugin::expected<RollbackExecutionResult, PluginError> WorkflowRollbackManager::execute_rollback_plan(WorkflowRollbackPlan* plan) {
    // Stub implementation
    RollbackExecutionResult result;
    result.rollback_id = generate_rollback_id();
    result.execution_id = "execution_" + QUuid::createUuid().toString(QUuid::WithoutBraces);
    ...
    qCDebug(workflowRollbackLog) << "Executed rollback plan:" << plan->plan_id();
    return result;
}

qtplugin::expected<void, PluginError> WorkflowRollbackManager::execute_rollback_operation(const RollbackOperation& operation, RollbackExecutionResult& /* result */) {
    // Stub implementation
    qCDebug(workflowRollbackLog) << "Executed rollback operation:" << operation.operation_id;
    return make_success();
}

qtplugin::expected<void, PluginError> WorkflowRollbackManager::validate_rollback_operation(const RollbackOperation& operation) {
    // Stub implementation
    if (operation.operation_id.isEmpty()) {
        return make_error<void>(PluginErrorCode::InvalidParameters, "Operation ID cannot be empty");
    }

    return make_success();
}

transactions::PluginTransactionManager* WorkflowRollbackManager::get_transaction_manager() {
    // Stub implementation
    return nullptr;
}

state::WorkflowCheckpointManager* WorkflowRollbackManager::get_checkpoint_manager() {
    // Stub implementation
    return nullptr;
}

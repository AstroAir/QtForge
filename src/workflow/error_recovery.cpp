...
namespace {
Q_LOGGING_CATEGORY(workflow_error_recovery_log,
                   "qtplugin.workflow.error_recovery")
}  // namespace
...
QJsonObject RetryPolicy::to_json() const {
    ...
    for (const PluginErrorCode code : retryable_errors) {
...
qtplugin::expected<RetryPolicy, PluginError> RetryPolicy::from_json(
    ...
    if (json.contains("initial_delay_ms") &&
        json["initial_delay_ms"].isDouble()) {
        policy.initial_delay =
            std::chrono::milliseconds(json["initial_delay_ms"].toInt());
    }

    if (json.contains("max_delay_ms") && json["max_delay_ms"].isDouble()) {
        policy.max_delay =
            std::chrono::milliseconds(json["max_delay_ms"].toInt());
    }
    ...
    if (json.contains("exponential_backoff") &&
        json["exponential_backoff"].isBool()) {
    ...
    if (json.contains("retryable_errors") &&
        json["retryable_errors"].isArray()) {
        const QJsonArray retryable_errors_array =
            json["retryable_errors"].toArray();
        for (const auto& value : retryable_errors_array) {
            if (value.isDouble()) {
                policy.retryable_errors.push_back(
                    static_cast<PluginErrorCode>(value.toInt()));
            }
        }
    }
...
qtplugin::expected<FallbackConfig, PluginError> FallbackConfig::from_json(
    const QJsonObject& json) {
    FallbackConfig config;

    if (json.contains("fallback_plugin_id") &&
        json["fallback_plugin_id"].isString()) {
        config.fallback_plugin_id = json["fallback_plugin_id"].toString();
    }

    if (json.contains("fallback_method") &&
        json["fallback_method"].isString()) {
        config.fallback_method = json["fallback_method"].toString();
    }

    if (json.contains("fallback_parameters") &&
        json["fallback_parameters"].isObject()) {
        config.fallback_parameters = json["fallback_parameters"].toObject();
    }

    if (json.contains("preserve_original_data") &&
        json["preserve_original_data"].isBool()) {
...
qtplugin::expected<CircuitBreakerConfig, PluginError>
CircuitBreakerConfig::from_json(const QJsonObject& json) {
    CircuitBreakerConfig config;

    if (json.contains("failure_threshold") &&
        json["failure_threshold"].isDouble()) {
    ...
    if (json.contains("recovery_timeout_ms") &&
        json["recovery_timeout_ms"].isDouble()) {
        config.recovery_timeout =
            std::chrono::milliseconds(json["recovery_timeout_ms"].toInt());
    }

    if (json.contains("failure_rate_threshold") &&
        json["failure_rate_threshold"].isDouble()) {
        config.failure_rate_threshold =
            json["failure_rate_threshold"].toDouble();
    }

    if (json.contains("minimum_requests") &&
        json["minimum_requests"].isDouble()) {
...
qtplugin::expected<GracefulDegradationConfig, PluginError>
GracefulDegradationConfig::from_json(const QJsonObject& json) {
    GracefulDegradationConfig config;

    if (json.contains("degraded_plugin_id") &&
        json["degraded_plugin_id"].isString()) {
        config.degraded_plugin_id = json["degraded_plugin_id"].toString();
    }

    if (json.contains("degraded_method") &&
        json["degraded_method"].isString()) {
        config.degraded_method = json["degraded_method"].toString();
    }

    if (json.contains("degraded_parameters") &&
        json["degraded_parameters"].isObject()) {
        config.degraded_parameters = json["degraded_parameters"].toObject();
    }

    if (json.contains("notify_degradation") &&
        json["notify_degradation"].isBool()) {
        config.notify_degradation = json["notify_degradation"].toBool();
    }

    if (json.contains("degradation_message") &&
        json["degradation_message"].isString()) {
    ...
    if (json.contains("target_quality") && json["target_quality"].isDouble()) {
        config.target_quality =
            static_cast<GracefulDegradationConfig::QualityLevel>(
                json["target_quality"].toInt());
    }
...
QJsonObject ErrorRecoveryConfig::to_json() const {
    ...
    for (const auto& [error_code, strategy] : error_strategy_map) {
        error_strategy_map_json[QString::number(static_cast<int>(error_code))] =
...
ErrorRecoveryConfig::from_json(const QJsonObject& json) {
    ErrorRecoveryConfig config;

    if (json.contains("primary_strategy") &&
        json["primary_strategy"].isDouble()) {
        config.primary_strategy =
            static_cast<RecoveryStrategy>(json["primary_strategy"].toInt());
    }

    if (json.contains("secondary_strategy") &&
        json["secondary_strategy"].isDouble()) {
        config.secondary_strategy =
            static_cast<RecoveryStrategy>(json["secondary_strategy"].toInt());
    }

    if (json.contains("tertiary_strategy") &&
        json["tertiary_strategy"].isDouble()) {
        config.tertiary_strategy =
            static_cast<RecoveryStrategy>(json["tertiary_strategy"].toInt());
    }

    if (json.contains("retry_policy") && json["retry_policy"].isObject()) {
        auto retry_result =
            RetryPolicy::from_json(json["retry_policy"].toObject());
        if (retry_result) {
            config.retry_policy = retry_result.value();
        }
    }

    if (json.contains("fallback_config") &&
        json["fallback_config"].isObject()) {
        auto fallback_result =
            FallbackConfig::from_json(json["fallback_config"].toObject());
        if (fallback_result) {
            config.fallback_config = fallback_result.value();
        }
    }

    if (json.contains("circuit_breaker_config") &&
        json["circuit_breaker_config"].isObject()) {
        auto circuit_result = CircuitBreakerConfig::from_json(
            json["circuit_breaker_config"].toObject());
        if (circuit_result) {
            config.circuit_breaker_config = circuit_result.value();
        }
    }

    if (json.contains("degradation_config") &&
        json["degradation_config"].isObject()) {
        auto degradation_result = GracefulDegradationConfig::from_json(
            json["degradation_config"].toObject());
    ...
    if (json.contains("escalate_on_failure") &&
        json["escalate_on_failure"].isBool()) {
        config.escalate_on_failure = json["escalate_on_failure"].toBool();
    }

    if (json.contains("log_recovery_attempts") &&
        json["log_recovery_attempts"].isBool()) {
        config.log_recovery_attempts = json["log_recovery_attempts"].toBool();
    }

    if (json.contains("notify_on_recovery") &&
        json["notify_on_recovery"].isBool()) {
        config.notify_on_recovery = json["notify_on_recovery"].toBool();
    }

    if (json.contains("error_strategy_map") &&
        json["error_strategy_map"].isObject()) {
        QJsonObject error_strategy_map_json =
            json["error_strategy_map"].toObject();
        for (auto it = error_strategy_map_json.begin();
             it != error_strategy_map_json.end(); ++it) {
            if (it.value().isDouble()) {
                const PluginErrorCode error_code =
                    static_cast<PluginErrorCode>(it.key().toInt());
                const RecoveryStrategy strategy =
                    static_cast<RecoveryStrategy>(it.value().toInt());
...
CircuitBreaker::CircuitBreaker(const CircuitBreakerConfig& config,
    ...
    connect(m_timeout_timer, &QTimer::timeout, this,
            &CircuitBreaker::on_timeout);

    m_recovery_timer->setSingleShot(true);
    connect(m_recovery_timer, &QTimer::timeout, this,
            &CircuitBreaker::on_recovery_timeout);

    qCDebug(workflow_error_recovery_log)
        << "Created circuit breaker with failure threshold:"
...
void CircuitBreaker::record_failure() {
    ...
    qCDebug(workflow_error_recovery_log)
        << "Circuit breaker recorded failure, count:" << m_failure_count
...
void CircuitBreaker::reset() {
    ...
    qCDebug(workflow_error_recovery_log) << "Circuit breaker reset";
}
...
double CircuitBreaker::failure_rate() const {
    ...
    return static_cast<double>(m_failure_count) /
           static_cast<double>(m_request_count);
}

void CircuitBreaker::update_config(const CircuitBreakerConfig& config) {
    m_config = config;

    qCDebug(workflow_error_recovery_log) << "Updated circuit breaker config";
}
...
void CircuitBreaker::on_timeout() {
    if (m_state == CircuitBreakerState::Open) {
        ...
        m_recovery_timer->start(
            static_cast<int>(m_config.recovery_timeout.count()));
    }
}
...
ErrorRecoveryExecutor::ErrorRecoveryExecutor(QObject* parent)
    : QObject(parent) {
    qCDebug(workflow_error_recovery_log) << "Created error recovery executor";
}

qtplugin::expected<QJsonObject, PluginError>
ErrorRecoveryExecutor::execute_with_recovery(
    const QString& execution_id, const QString& operation_id,
    ...
    auto result =
        try_strategy(context, operation, config.primary_strategy, config);
    if (result) {
    ...
    if (config.escalate_on_failure &&
        config.secondary_strategy != RecoveryStrategy::None) {
        result =
            try_strategy(context, operation, config.secondary_strategy, config);
        if (result) {
            emit recovery_completed(execution_id, true);
            return result;
        }
    ...
    if (config.escalate_on_failure &&
        config.tertiary_strategy != RecoveryStrategy::None) {
        result =
            try_strategy(context, operation, config.tertiary_strategy, config);
        if (result) {
            emit recovery_completed(execution_id, true);
            return result;
        }
...
std::optional<RecoveryExecutionContext>
ErrorRecoveryExecutor::get_recovery_context(const QString& execution_id) const {
...
void ErrorRecoveryExecutor::clear_recovery_context(
    const QString& execution_id) {
    m_recovery_contexts.erase(execution_id);

    qCDebug(workflow_error_recovery_log)
        << "Cleared recovery context for execution:" << execution_id;
}

CircuitBreaker* ErrorRecoveryExecutor::get_circuit_breaker(
...
void ErrorRecoveryExecutor::register_circuit_breaker(
    const QString& operation_id, const CircuitBreakerConfig& config) {
    m_circuit_breakers[operation_id] =
        std::make_unique<CircuitBreaker>(config, this);

    qCDebug(workflow_error_recovery_log)
        << "Registered circuit breaker for operation:" << operation_id;
}

qtplugin::expected<QJsonObject, PluginError>
ErrorRecoveryExecutor::try_strategy(
    RecoveryExecutionContext& context,
    std::function<qtplugin::expected<QJsonObject, PluginError>()> operation,
    RecoveryStrategy strategy, const ErrorRecoveryConfig& config) {
    ...
    switch (strategy) {
        case RecoveryStrategy::Retry:
            result =
                execute_retry_strategy(context, operation, config.retry_policy);
            break;
        ...
        case RecoveryStrategy::GracefulDegradation:
            result = execute_degradation_strategy(context,
        ...
        case RecoveryStrategy::Abort:
            result = make_error<QJsonObject>(
                PluginErrorCode::ExecutionFailed,
                "Operation aborted by recovery strategy");
            break;

        case RecoveryStrategy::None:
        default:
            result = make_error<QJsonObject>(PluginErrorCode::NotSupported,
                                             "Recovery strategy not supported");
            break;
    }
...
qtplugin::expected<QJsonObject, PluginError>
ErrorRecoveryExecutor::execute_retry_strategy(
    RecoveryExecutionContext& /* context */,
    std::function<qtplugin::expected<QJsonObject, PluginError>()> operation,
    ...
    for (int attempt = 0; attempt < policy.max_attempts; ++attempt) {
        if (attempt > 0) {
            ...
            qCDebug(workflow_error_recovery_log)
                << "Retrying operation after delay:" << delay.count()
                << "ms, attempt:" << (attempt + 1);

            // For simplicity, we'll use a blocking delay here
            // In a real implementation, you might want to use QTimer for
            // non-blocking delays
        ...
        if (result) {
            qCDebug(workflow_error_recovery_log)
                << "Retry strategy succeeded on attempt:" << (attempt + 1);
            return result;
        }
        ...
        if (policy.should_retry && !policy.should_retry(result.error())) {
            qCDebug(workflow_error_recovery_log)
                << "Error not retryable, stopping retry attempts";
            return result;
        }
        ...
        if (!policy.retryable_errors.empty()) {
            auto it =
                std::ranges::find(policy.retryable_errors, result.error().code);
            if (it == policy.retryable_errors.end()) {
                qCDebug(workflow_error_recovery_log)
                    << "Error code not in retryable list, stopping retry "
                       "attempts";
                return result;
            }
        }
        ...
        qCDebug(workflow_error_recovery_log)
            << "Retry attempt" << (attempt + 1) << "failed, error:"
...
qtplugin::expected<QJsonObject, PluginError>
ErrorRecoveryExecutor::execute_fallback_strategy(
    RecoveryExecutionContext& context, const FallbackConfig& config) {
    ...
    fallback_result["original_error"] =
        QString::fromStdString(context.original_error.message);
    ...
    qCDebug(workflow_error_recovery_log)
        << "Fallback strategy completed with default result";

    return fallback_result;
}

qtplugin::expected<QJsonObject, PluginError>
ErrorRecoveryExecutor::execute_degradation_strategy(
    RecoveryExecutionContext& context,
    const GracefulDegradationConfig& config) {
    if (config.degradation_func) {
        auto result = config.degradation_func(context.original_parameters,
                                              config.target_quality);
        if (result) {
            qCDebug(workflow_error_recovery_log)
                << "Graceful degradation strategy succeeded";
            return result;
        }
        qCDebug(workflow_error_recovery_log)
    ...
    degraded_result["original_error"] =
        QString::fromStdString(context.original_error.message);
    ...
    qCDebug(workflow_error_recovery_log)
        << "Graceful degradation strategy completed with default result";

    return degraded_result;
}

std::chrono::milliseconds ErrorRecoveryExecutor::calculate_retry_delay(
    ...
    if (policy.exponential_backoff && attempt > 0) {
        ...
        delay = std::chrono::milliseconds(
            static_cast<long long>(delay.count() * multiplier));
...
RecoveryStrategy ErrorRecoveryExecutor::select_strategy(
    const PluginError& error, const ErrorRecoveryConfig& config) {
...
ErrorRecoveryManager::ErrorRecoveryManager(QObject* parent)
    ...
    qCDebug(workflow_error_recovery_log) << "Created error recovery manager";
}

ErrorRecoveryManager::~ErrorRecoveryManager() {
    shutdown();
    qCDebug(workflow_error_recovery_log) << "Destroyed error recovery manager";
}

qtplugin::expected<void, PluginError> ErrorRecoveryManager::initialize() {
    if (m_initialized) {
        return make_error<void>(PluginErrorCode::InvalidState,
                                "Error recovery manager already initialized");
    }
...
void ErrorRecoveryManager::register_recovery_config(
    const QString& operation_id, const ErrorRecoveryConfig& config) {
    m_recovery_configs[operation_id] = config;
    emit recovery_config_registered(operation_id);

    qCDebug(workflow_error_recovery_log)
        << "Registered recovery config for operation:" << operation_id;
}

void ErrorRecoveryManager::unregister_recovery_config(
    const QString& operation_id) {
    ...
    if (it != m_recovery_configs.end()) {
        ...
        qCDebug(workflow_error_recovery_log)
            << "Unregistered recovery config for operation:" << operation_id;
    }
}

std::optional<ErrorRecoveryConfig> ErrorRecoveryManager::get_recovery_config(
...
ErrorRecoveryManager::execute_with_recovery(
    ...
    auto result = m_executor->execute_with_recovery(
        execution_id, operation_id, operation, config_it->second);

...
MessagePublisher::MessagePublisher(const CommunicationConfig::MessageBusConfig& config, QObject* parent)
    ...
    connect(process_timer_, &QTimer::timeout, this, &MessagePublisher::process_message_queue);
    
    qCDebug(messagePublisherLog) << "MessagePublisher created with max queue size:" << config_.max_queue_size;
}
...
Result<void> MessagePublisher::publish(
    ...
    if (!message) {
        return CommunicationError{
            CommunicationError::Type::InvalidMessage,
...
std::future<Result<void>> MessagePublisher::publish_async(
    ...
    if (!message) {
        promise.set_value(CommunicationError{
            CommunicationError::Type::InvalidMessage,
            "Null message provided",
            "Message pointer is null"
        });
...
void MessagePublisher::start() {
    ...
    if (!router_) {
        qCWarning(messagePublisherLog) << "Starting publisher without router - messages will not be delivered";
    }
...
bool MessagePublisher::is_running() const {
...
void MessagePublisher::process_message_queue() {
    ...
    while (!message_queue_.empty()) {
        ...
        auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(now - pending.timestamp);
        
        if (elapsed > config_.delivery_timeout) {
            pending.promise.set_value(CommunicationError{
        ...
        auto result = publish_immediate(pending.message, pending.mode, pending.recipients);
...
Result<void> MessagePublisher::publish_immediate(
    ...
    if (!router_) {
        return CommunicationError{
            CommunicationError::Type::SystemError,
            "No router available",
            "MessageRouter not set"
        };
    }
    
    // Find subscribers
    auto subscribers_result = router_->find_subscribers(*message, mode, recipients);
    if (!std::holds_alternative<std::vector<std::shared_ptr<ISubscription>>>(subscribers_result)) {
        auto error = std::get<CommunicationError>(subscribers_result);
        update_statistics(*message, false, 0);
        emit_signals(*message, false, 0, error.message);
        return error;
    }
    
    auto subscribers = std::get<std::vector<std::shared_ptr<ISubscription>>>(subscribers_result);
    ...
    bool success = std::holds_alternative<void>(delivery_result);
    
    update_statistics(*message, success, subscribers.size());
    emit_signals(*message, success, subscribers.size(), 
                success ? "" : std::get<CommunicationError>(delivery_result).message);
...
void MessagePublisher::enqueue_message(
    ...
    message_queue_.emplace(PendingMessage{
        std::move(message),
        mode,
        recipients,
        std::move(promise),
...
void MessagePublisher::update_statistics(const IMessage& message, bool success, size_t recipient_count) {
    if (statistics_) {
...
void MessagePublisher::emit_signals(const IMessage& message, bool success, size_t recipient_count, const std::string& error) {
    if (success) {

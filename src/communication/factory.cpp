...
CommunicationSystemBuilder::CommunicationSystemBuilder(
    std::shared_ptr<ICommunicationFactory> factory)
    : factory_(std::move(factory)) {}

CommunicationSystemBuilder& CommunicationSystemBuilder::with_config(
    const CommunicationConfig& config) {
    config_ = config;
    return *this;
}

CommunicationSystemBuilder& CommunicationSystemBuilder::with_publisher(
    std::unique_ptr<IMessagePublisher> publisher) {
    publisher_ = std::move(publisher);
    return *this;
}

CommunicationSystemBuilder&
...
CommunicationSystemBuilder& CommunicationSystemBuilder::with_router(
    std::unique_ptr<IMessageRouter> router) {
    router_ = std::move(router);
    return *this;
}

CommunicationSystemBuilder& CommunicationSystemBuilder::with_statistics(
    std::unique_ptr<IStatistics> statistics) {
    statistics_ = std::move(statistics);
    return *this;
}

CommunicationSystemBuilder& CommunicationSystemBuilder::with_event_system(
    std::unique_ptr<IEventSystem> event_system) {
    event_system_ = std::move(event_system);
    return *this;
}

CommunicationSystemBuilder& CommunicationSystemBuilder::with_request_response(
...
CommunicationSystem CommunicationSystemBuilder::build() {
    ...
    if (!subscription_manager_) {
        subscription_manager_ =
            factory_->create_subscription_manager(config_.message_bus);
    }

    if (!router_) {
        // Convert unique_ptr to shared_ptr for router dependency
        auto shared_manager = std::shared_ptr<ISubscriptionManager>(
            subscription_manager_.release());
        router_ = factory_->create_router(shared_manager);
        // Note: We need to handle the shared ownership properly in the actual
        // system
    }
    ...
    if (!request_response_) {
        request_response_ =
            factory_->create_request_response_service(config_.request_response);
    }

    return CommunicationSystem(
        std::move(publisher_), std::move(subscription_manager_),
        std::move(router_), std::move(statistics_), std::move(event_system_),
...
Result<std::shared_ptr<ISubscription>>
CommunicationSystem::subscribe_to_messages(
    ...
    return subscription_manager_->subscribe(
        subscriber_id, message_type, std::move(handler), std::move(filter));
}

Result<void> CommunicationSystem::register_service(
    std::string_view service_name,
    IRequestResponseService::RequestHandler handler) {
    return request_response_->register_service(service_name,
                                               std::move(handler));
}
...
IStatistics::SubscriptionStats CommunicationSystem::get_subscription_stats()
    const {
...
void CommunicationSystem::shutdown() {
    ...
    // Order matters: stop accepting new work first, then clean up in reverse
    // dependency order

    // Phase 1: Stop accepting new work
    // The publisher and request_response service should stop accepting new
    // messages/requests Note: Current implementations don't have explicit "stop
    // accepting" methods, but the shutdown flag prevents new operations from
    // being initiated

    // Phase 2: Wait for pending operations to complete
    // Give a brief moment for in-flight operations to complete
    // In a production system, this would involve proper synchronization
    std::this_thread::sleep_for(std::chrono::milliseconds(100));

    // Phase 3: Clean up resources in reverse dependency order
    // The order is important to avoid use-after-free:
    // 1. Request-response service (depends on nothing)
    // 2. Event system (depends on nothing)
    // 3. Publisher (depends on router, subscription_manager, statistics)
    // 4. Router (depends on subscription_manager)
    // 5. Statistics (depends on nothing)
    // 6. Subscription manager (base dependency)

    request_response_.reset();
    event_system_.reset();
    publisher_.reset();
    router_.reset();
    statistics_.reset();
...
bool CommunicationSystem::is_shutdown() const noexcept {
    return shutdown_flag_.load();
}

// DefaultCommunicationFactory Implementation

std::unique_ptr<IMessagePublisher>
DefaultCommunicationFactory::create_publisher(
    const CommunicationConfig::MessageBusConfig& config) {
    return std::make_unique<MessagePublisher>(config);
}

std::unique_ptr<ISubscriptionManager>
DefaultCommunicationFactory::create_subscription_manager(
    const CommunicationConfig::MessageBusConfig& config) {
...
std::unique_ptr<IRequestResponseService>
DefaultCommunicationFactory::create_request_response_service(
...
CommunicationSystem create_default_communication_system(
    ...
    return CommunicationSystemBuilder(factory).with_config(config).build();
}

 ...
 * @brief Subscription manager implementation following Single Responsibility
 * Principle
 * @version 3.0.0
 */
...
public:
    Subscription(std::string id, std::string subscriber_id,
    ...
    // Internal interface (kept for backward compatibility)
...
public:
    ...
    Result<std::shared_ptr<ISubscription>> subscribe(
        ...
        MessageHandler handler, MessageFilter filter = nullptr) override;
    ...
    find_subscriptions_for_subscriber(std::string_view subscriber_id) const;
...
private:
    ...
    std::unordered_map<std::string, std::shared_ptr<Subscription>>
        subscriptions_by_id_;
    std::unordered_map<std::string, std::vector<std::shared_ptr<Subscription>>>

...
public:
    explicit Private(PluginServiceDiscovery* owner) : q(owner) {
        ...
        QObject::connect(heartbeat_timer, &QTimer::timeout,
                        q, &PluginServiceDiscovery::on_heartbeat_timer);
        QObject::connect(health_check_timer, &QTimer::timeout,
                        q, &PluginServiceDiscovery::on_health_check_timer);
        QObject::connect(network_discovery_timer, &QTimer::timeout,
                        q, &PluginServiceDiscovery::on_network_discovery_timer);
        ...
        QObject::connect(udp_socket, &QUdpSocket::readyRead,
                        q, &PluginServiceDiscovery::on_network_data_received);
    }
    ...
    std::unordered_map<QString, std::chrono::system_clock::time_point> last_heartbeat;
    ...
    bool is_service_healthy(const QString& service_id) {
        ...
        return elapsed.count() < 120;  // Consider healthy if heartbeat within 2 minutes
    }
    
    void update_service_availability(const QString& service_id, ServiceAvailability availability) {
    ...
    void send_network_announcement(const ServiceRegistration& registration) {
        ...
        announcement["service_type"] = registration.service_type;
        announcement["endpoint"] = registration.endpoint;
        announcement["port"] = registration.port;
        ...
        udp_socket->writeDatagram(data, QHostAddress(multicast_group), discovery_port);
    }
    ...
    void process_network_announcement(const QJsonObject& announcement) {
        ...
        registration.service_type = announcement["service_type"].toString();
        registration.endpoint = announcement["endpoint"].toString();
        registration.port = announcement["port"].toInt();
        registration.metadata = announcement["metadata"].toObject();
        
        // Check if this is a new service
        bool is_new = registered_services.find(registration.service_id) == registered_services.end();
        
        registered_services[registration.service_id] = registration;
        last_heartbeat[registration.service_id] = std::chrono::system_clock::now();
        update_service_availability(registration.service_id, ServiceAvailability::Available);
...
void PluginServiceDiscovery::set_discovery_mode(ServiceDiscoveryMode mode) {
    ...
    if (mode == ServiceDiscoveryMode::Network || mode == ServiceDiscoveryMode::Hybrid) {
        d->heartbeat_timer->start();
        d->health_check_timer->start();
        d->network_discovery_timer->start();
        
        // Bind UDP socket for network discovery
        if (!d->udp_socket->bind(QHostAddress::AnyIPv4, d->discovery_port, QUdpSocket::ShareAddress)) {
            qCWarning(serviceDiscoveryLog) << "Failed to bind UDP socket for service discovery";
        }
    } else {
    ...
    qCDebug(serviceDiscoveryLog) << "Discovery mode set to:" << static_cast<int>(mode);
}
...
PluginServiceDiscovery::register_service(const ServiceRegistration& registration) {
    if (registration.service_name.isEmpty() || registration.service_type.isEmpty()) {
        return qtplugin::unexpected(PluginError(PluginErrorCode::InvalidArgument, 
    ...
    emit service_registered(service_id, registration);
...
PluginServiceDiscovery::unregister_service(const QString& service_id) {
    ...
    if (it == d->registered_services.end()) {
        return qtplugin::unexpected(PluginError(PluginErrorCode::NotFound, 
                                               "Service not found"));
    }
...
PluginServiceDiscovery::discover_services(const ServiceDiscoveryQuery& query) {
    ...
    for (const auto& [service_id, registration] : d->registered_services) {
        // Apply filters
        if (!query.service_type.isEmpty() && registration.service_type != query.service_type) {
            continue;
        }
        
        if (!query.service_name.isEmpty() && registration.service_name != query.service_name) {
            continue;
        }
    ...
    result.discovery_time = std::chrono::milliseconds(10);  // Placeholder timing
...
PluginServiceDiscovery::get_service_registration(const QString& service_id) const {
    if (service_id.isEmpty()) {
        return qtplugin::unexpected(PluginError(PluginErrorCode::InvalidArgument, 
                                               "Service ID is required"));
    }
...
void PluginServiceDiscovery::on_health_check_timer() {
    ...
    for (const QString& service_id : unhealthy_services) {
        d->update_service_availability(service_id, ServiceAvailability::Unavailable);
    }
}
...
void PluginServiceDiscovery::on_network_discovery_timer() {
    ...
    d->udp_socket->writeDatagram(data, QHostAddress(d->multicast_group), d->discovery_port);
}
...
void PluginServiceDiscovery::on_network_data_received() {
    while (d->udp_socket->hasPendingDatagrams()) {
        ...
        d->udp_socket->readDatagram(data.data(), data.size(), &sender, &sender_port);

        QJsonParseError error;
        QJsonDocument doc = QJsonDocument::fromJson(data, &error);

        if (error.error != QJsonParseError::NoError) {
            qCWarning(serviceDiscoveryLog) << "Failed to parse network data:" << error.errorString();
            continue;
        }
...
PluginServiceDiscovery::PluginServiceDiscovery(QObject* parent)
    : QObject(parent) {
    qCWarning(serviceDiscoveryLog) << "Network support disabled - service discovery limited to local mode";
}

PluginServiceDiscovery::~PluginServiceDiscovery() = default;

void PluginServiceDiscovery::set_discovery_mode(ServiceDiscoveryMode) {
    qCWarning(serviceDiscoveryLog) << "Network discovery not available - network support disabled";
}
...
PluginServiceDiscovery::register_service(const ServiceRegistration&) {
    return qtplugin::unexpected(PluginError(PluginErrorCode::NotSupported, 
...
PluginServiceDiscovery::discover_services(const ServiceDiscoveryQuery&) {
    return {};
}

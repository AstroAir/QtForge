 ...
 * @brief Message publisher implementation following Single Responsibility Principle
 * @version 3.0.0
 */
...
public:
    explicit MessagePublisher(const CommunicationConfig::MessageBusConfig& config, QObject* parent = nullptr);
...
private:
    ...
    void update_statistics(const IMessage& message, bool success, size_t recipient_count);
    void emit_signals(const IMessage& message, bool success, size_t recipient_count, const std::string& error = {});
};

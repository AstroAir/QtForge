 ...
 */
enum class DockArea { Left, Right, Top, Bottom, Center, Floating, Tabbed };
...
public:
     ...
     */
    virtual std::unique_ptr<QWidget> create_widget(
     ...
     */
    virtual std::vector<QAction*> menu_actions() const { return {}; }

    /**
     * @brief Get toolbar actions for integration
     * @return Vector of toolbar actions
     */
    virtual std::vector<QAction*> toolbar_actions() const { return {}; }

    /**
     * @brief Get context menu actions
     * @return Vector of context menu actions
     */
    virtual std::vector<QAction*> context_menu_actions() const { return {}; }
     ...
     */
    virtual std::unique_ptr<QToolBar> create_toolbar(
        QWidget* parent = nullptr) {
        Q_UNUSED(parent)
        return nullptr;
    }
     ...
     */
    virtual std::vector<QKeySequence> keyboard_shortcuts() const { return {}; }
    ...
    virtual qtplugin::expected<void, PluginError> apply_theme(
        std::string_view theme_name, const QJsonObject& theme_data = {}) {
        Q_UNUSED(theme_name)
        Q_UNUSED(theme_data)
        return make_success();
    }
     ...
     */
    virtual std::string current_theme() const { return "default"; }
...
public:
     ...
     */
    virtual std::unique_ptr<IUIPlugin> create_ui_plugin(
        QWidget* parent = nullptr, const QJsonObject& config = {}) = 0;
     ...
     */
    virtual std::vector<UIIntegrationMode> supported_integration_modes()
        const = 0;
};

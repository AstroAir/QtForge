...
public:
    ...
    virtual QString load_remote_async(
        ...
        std::function<void(const qtplugin::expected<RemotePluginLoadResult, PluginError>&)> completion_callback = nullptr) = 0;
     ...
     */
    virtual qtplugin::expected<void, PluginError> cancel_remote_load(const QString& operation_id) = 0;
     ...
     * @return Success or error
     */
    virtual qtplugin::expected<void, PluginError> add_source(const RemotePluginSource& source) = 0;
     ...
     * @return Success or error
     */
    virtual qtplugin::expected<void, PluginError> remove_source(const QString& source_id) = 0;
     ...
     */
    virtual std::optional<RemotePluginSource> find_source_for_url(const QUrl& url) const = 0;
     ...
     */
    virtual qtplugin::expected<std::vector<QJsonObject>, PluginError> discover_plugins(
     ...
     */
    virtual qtplugin::expected<std::vector<QJsonObject>, PluginError> search_plugins(
        const QString& query, int max_results = 50) = 0;
     ...
     */
    virtual void set_configuration(std::shared_ptr<RemotePluginConfiguration> configuration) = 0;
    
    /**
     * @brief Get remote plugin configuration
     * @return Current configuration
     */
    virtual std::shared_ptr<RemotePluginConfiguration> configuration() const = 0;
    
    /**
     * @brief Set download manager
     * @param download_manager Download manager to use
     */
    virtual void set_download_manager(std::shared_ptr<PluginDownloadManager> download_manager) = 0;
     ...
     */
    virtual void set_validator(std::shared_ptr<RemotePluginValidator> validator) = 0;
...
public:
    ...
    qtplugin::expected<void, PluginError> add_source(const RemotePluginSource& source) override;
    qtplugin::expected<void, PluginError> remove_source(const QString& source_id) override;
    std::vector<RemotePluginSource> get_sources() const override;
    std::optional<RemotePluginSource> find_source_for_url(const QUrl& url) const override;
    
    void set_configuration(std::shared_ptr<RemotePluginConfiguration> configuration) override;
    std::shared_ptr<RemotePluginConfiguration> configuration() const override;
    void set_download_manager(std::shared_ptr<PluginDownloadManager> download_manager) override;
    std::shared_ptr<PluginDownloadManager> download_manager() const override;
    void set_validator(std::shared_ptr<RemotePluginValidator> validator) override;
...
protected:
    ...
    // Helper methods
    virtual qtplugin::expected<RemotePluginLoadResult, PluginError> perform_remote_load(
        const RemotePluginSource& source,
        const RemotePluginLoadOptions& options) = 0;

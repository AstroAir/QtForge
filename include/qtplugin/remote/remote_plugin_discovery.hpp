...
struct DiscoveryResult {
    ...
    bool is_successful() const {
        return !plugins.empty() || failed_sources.empty();
    }
    ...
    double success_rate() const {
        if (total_sources_queried == 0)
            return 0.0;
        return static_cast<double>(total_sources_queried -
                                   failed_sources.size()) /
...
using DiscoveryCompletionCallback = std::function<void(
...
public:
    ...
    void set_max_concurrent_requests(int max_requests) {
        m_max_concurrent_requests = max_requests;
    }
...
private:
    ...
    std::unordered_map<QString, std::unique_ptr<DiscoveryOperation>>
        m_active_operations;

    // Helper methods
    QString generate_operation_id() const;
    QNetworkRequest create_discovery_request(
        const RemotePluginSource& source,
        const PluginDiscoveryFilter& filter) const;
    qtplugin::expected<std::vector<RemotePluginDiscoveryResult>, PluginError>
    parse_discovery_response(const QByteArray& response,
...
public:
     ...
     */
    qtplugin::expected<void, PluginError> cancel_discovery(
...
signals:
     ...
     */
    void discovery_progress(const QString& operation_id,
                            const QJsonObject& progress);

    /**
     * @brief Emitted when discovery completes
     */
    void discovery_completed(const QString& operation_id, bool success,
                             const QString& error_message);

private:
    std::unordered_map<QString, std::shared_ptr<IPluginDiscoveryEngine>>
        m_engines;
    ...
    struct ManagedDiscoveryOperation {
        ...
        std::unordered_map<QString, QString>
            source_operation_ids;  // source_id -> engine_operation_id
        DiscoveryResult partial_result;
        int completed_sources = 0;
    };

    std::unordered_map<QString, std::unique_ptr<ManagedDiscoveryOperation>>
        m_active_operations;
    mutable std::mutex m_operations_mutex;

    // Helper methods
    QString generate_operation_id() const;
    std::shared_ptr<IPluginDiscoveryEngine> find_engine_for_source(
        const RemotePluginSource& source) const;
    void handle_source_discovery_completed(

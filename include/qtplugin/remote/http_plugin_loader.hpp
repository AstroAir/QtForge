...
public:
    ...
    QString load_remote_async(
        ...
        std::function<void(const qtplugin::expected<RemotePluginLoadResult, PluginError>&)> completion_callback = nullptr) override;
    
    qtplugin::expected<void, PluginError> cancel_remote_load(const QString& operation_id) override;
     ...
     */
    void set_network_manager(std::unique_ptr<QNetworkAccessManager> network_manager);
    ...
    qtplugin::expected<QJsonObject, PluginError> get_plugin_metadata(
        const QUrl& url, std::chrono::seconds timeout = std::chrono::seconds{30});
     ...
     */
    QUrl get_registry_endpoint(const RemotePluginSource& source, const QString& endpoint) const;
...
private slots:
    void on_async_load_progress(const QString& download_id, const DownloadProgress& progress);
    void on_async_load_completed(const QString& download_id, const DownloadResult& result);
    void on_async_load_failed(const QString& download_id, const PluginError& error);
...
private:
    ...
    std::unordered_map<QString, std::unique_ptr<AsyncOperation>> m_async_operations;
    std::unordered_map<QString, QString> m_download_to_operation_map;  // download_id -> operation_id
    ...
    qtplugin::expected<DownloadResult, PluginError> download_plugin_file(
        const RemotePluginSource& source, const RemotePluginLoadOptions& options);
    qtplugin::expected<std::shared_ptr<IPlugin>, PluginError> load_downloaded_plugin(
    ...
    void complete_async_operation(const QString& operation_id, 
    ...
    qtplugin::expected<QJsonArray, PluginError> parse_plugin_list_response(const QJsonObject& response);
    qtplugin::expected<QJsonObject, PluginError> parse_plugin_info_response(const QJsonObject& response);
    ...
    void setup_network_request(QNetworkRequest& request, const RemotePluginSource& source) const;
    void apply_authentication(QNetworkRequest& request, const RemotePluginSource& source) const;
    void apply_custom_headers(QNetworkRequest& request, const QJsonObject& headers) const;
    
    // URL manipulation helpers
    QUrl resolve_plugin_url(const RemotePluginSource& source, const QString& plugin_path) const;
    QUrl build_search_url(const RemotePluginSource& source, const QString& query, int max_results) const;
    QUrl build_discovery_url(const RemotePluginSource& source) const;
    
    // Response parsing helpers
    qtplugin::expected<QJsonObject, PluginError> parse_json_response(const QByteArray& data) const;
...
public:
     ...
     */
    static std::unique_ptr<HttpPluginLoader> create_default(QObject* parent = nullptr);
     ...
     */
    static std::unique_ptr<HttpPluginLoader> create_enterprise(QObject* parent = nullptr);

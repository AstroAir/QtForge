...
struct RemotePluginInfo : public PluginInfo {
    ...
    std::optional<RemotePluginSource> remote_source;  ///< Source of remote plugin
    std::optional<QUrl> original_url;                 ///< Original download URL
    std::optional<std::filesystem::path> cached_path; ///< Local cache path
    std::chrono::system_clock::time_point download_time; ///< When plugin was downloaded
    std::chrono::system_clock::time_point last_update_check; ///< Last update check time
    std::optional<std::string> remote_version;        ///< Latest available remote version
     ...
     */
    static qtplugin::expected<RemotePluginInfo, PluginError> from_json(const QJsonObject& json);
...
struct RemotePluginDiscoveryResult {
     ...
     * @brief Create from JSON representation
     */
    static qtplugin::expected<RemotePluginDiscoveryResult, PluginError> from_json(const QJsonObject& json);
};
...
public:
     ...
     */
    virtual std::vector<RemotePluginInfo> get_all_remote_plugin_info() const = 0;
    ...
    virtual qtplugin::expected<std::vector<RemotePluginDiscoveryResult>, PluginError> 
        discover_remote_plugins(const RemotePluginSearchCriteria& criteria = {}) const = 0;
     ...
     */
    virtual qtplugin::expected<std::vector<RemotePluginDiscoveryResult>, PluginError>
        search_remote_plugins(const std::string& query, int max_results = 50) const = 0;
     ...
     * @param older_than_days Clear plugins cached longer than specified days (0 = all)
    ...
    virtual qtplugin::expected<bool, PluginError> 
        check_plugin_update(const std::string& plugin_id) const = 0;
 ...
 */
class RemotePluginRegistry : public PluginRegistry, public IRemotePluginRegistry {
    Q_OBJECT
...
public:
    ...
    qtplugin::expected<std::vector<RemotePluginDiscoveryResult>, PluginError> 
        discover_remote_plugins(const RemotePluginSearchCriteria& criteria = {}) const override;
    
    qtplugin::expected<std::vector<RemotePluginDiscoveryResult>, PluginError>
        search_remote_plugins(const std::string& query, int max_results = 50) const override;
    ...
    qtplugin::expected<bool, PluginError> 
        check_plugin_update(const std::string& plugin_id) const override;
...
signals:
     ...
     */
    void remote_plugin_discovered(const QString& plugin_id, const QJsonObject& metadata);
    
    /**
     * @brief Emitted when remote plugin update is available
     */
    void remote_plugin_update_available(const QString& plugin_id, const QString& new_version);
...
private:
    ...
    std::unordered_map<std::string, std::unique_ptr<RemotePluginInfo>> m_remote_plugins;
    ...
    // Helper methods
    RemotePluginInfo create_remote_plugin_info_copy(const RemotePluginInfo& original) const;

...
struct ValidationIssue {
    ...
    static qtplugin::expected<ValidationIssue, PluginError> from_json(
...
struct ValidationResult {
    ...
    std::unordered_map<ValidationRuleType, std::vector<ValidationIssue>>
        issues_by_type;
    ...
    // Serialization
    QJsonObject to_json() const;
    static qtplugin::expected<ValidationResult, PluginError> from_json(
        const QJsonObject& json);
};
...
struct ValidationRule {
    ...
    // Rule function
    std::function<std::vector<ValidationIssue>(
        const qtplugin::workflow::orchestration::Workflow&, const QJsonObject&)>
        validation_func;

    // Conditions
    std::function<bool(const qtplugin::workflow::orchestration::Workflow&)>
        should_apply;

    // Serialization
    QJsonObject to_json() const;
    static qtplugin::expected<ValidationRule, PluginError> from_json(
        const QJsonObject& json);
};
...
public:
    ...
    std::vector<ValidationIssue> validate_dependencies(
        const qtplugin::workflow::orchestration::Workflow& workflow) const;
    std::vector<ValidationIssue> validate_step_dependencies(
        const qtplugin::workflow::orchestration::WorkflowStep& step,
        const qtplugin::workflow::orchestration::Workflow& workflow) const;
    std::vector<ValidationIssue> validate_plugin_dependencies(
        const QString& plugin_id) const;

    // Circular dependency detection
    bool has_circular_dependencies(
        const qtplugin::workflow::orchestration::Workflow& workflow) const;
    std::vector<QString> find_dependency_cycle(
        const qtplugin::workflow::orchestration::Workflow& workflow) const;

    // Dependency graph analysis
    std::vector<QString> get_execution_order(
        const qtplugin::workflow::orchestration::Workflow& workflow) const;
    std::unordered_map<QString, std::vector<QString>> build_dependency_graph(
        const qtplugin::workflow::orchestration::Workflow& workflow) const;

private:
    // Helper methods
    bool has_cycle_dfs(
        const std::unordered_map<QString, std::vector<QString>>& graph,
        const QString& node, std::unordered_set<QString>& visited,
        std::unordered_set<QString>& rec_stack,
        std::vector<QString>& cycle_path) const;

    std::vector<QString> topological_sort(
        const std::unordered_map<QString, std::vector<QString>>& graph) const;
};
...
public:
    ...
    // Plugin validation
    std::vector<ValidationIssue> validate_plugin_availability(
        const qtplugin::workflow::orchestration::Workflow& workflow) const;
    std::vector<ValidationIssue> validate_plugin_compatibility(
        const QString& plugin_id, const QJsonObject& config) const;
    std::vector<ValidationIssue> validate_plugin_methods(
        const QString& plugin_id,
        const std::vector<QString>& required_methods) const;
...
private:
    // Helper methods
    bool check_version_compatibility(const QString& required_version,
                                     const QString& available_version) const;
    bool check_method_signature(const QString& plugin_id,
                                const QString& method_name,
                                const QJsonObject& expected_signature) const;
};
...
public:
    ...
    std::vector<ValidationIssue> validate_workflow_configuration(
        const qtplugin::workflow::orchestration::Workflow& workflow) const;
    std::vector<ValidationIssue> validate_step_configuration(
        const qtplugin::workflow::orchestration::WorkflowStep& step) const;
    std::vector<ValidationIssue> validate_plugin_configuration(
        const QString& plugin_id, const QJsonObject& config) const;

    // Schema validation
    std::vector<ValidationIssue> validate_against_schema(
        const QJsonObject& data, const QJsonObject& schema,
        const QString& context) const;

    // Configuration completeness
    std::vector<ValidationIssue> check_required_parameters(
        const QJsonObject& config, const std::vector<QString>& required_params,
        const QString& context) const;
    std::vector<ValidationIssue> check_parameter_types(
        const QJsonObject& config,
        const std::unordered_map<QString, QString>& expected_types,
        const QString& context) const;

private:
    // Schema validation helpers
    bool validate_json_type(const QJsonValue& value,
                            const QString& expected_type) const;
    std::vector<ValidationIssue> validate_object_properties(
        const QJsonObject& object, const QJsonObject& schema,
        const QString& context) const;
};
...
public:
    ...
    // Resource validation
    std::vector<ValidationIssue> validate_resource_availability(
        const qtplugin::workflow::orchestration::Workflow& workflow) const;
    std::vector<ValidationIssue> validate_file_resources(
        const std::vector<QString>& file_paths) const;
    std::vector<ValidationIssue> validate_network_resources(
        const std::vector<QString>& urls) const;
    std::vector<ValidationIssue> validate_database_resources(
        const std::vector<QString>& connection_strings) const;

    // Resource capacity
    std::vector<ValidationIssue> check_memory_requirements(
        const qtplugin::workflow::orchestration::Workflow& workflow) const;
    std::vector<ValidationIssue> check_disk_space_requirements(
        const qtplugin::workflow::orchestration::Workflow& workflow) const;
    std::vector<ValidationIssue> check_network_bandwidth_requirements(
        const qtplugin::workflow::orchestration::Workflow& workflow) const;
...
private:
    ...
    qint64 estimate_memory_usage(
        const qtplugin::workflow::orchestration::Workflow& workflow) const;
    qint64 estimate_disk_usage(
        const qtplugin::workflow::orchestration::Workflow& workflow) const;
    qint64 estimate_bandwidth_usage(
        const qtplugin::workflow::orchestration::Workflow& workflow) const;
};
...
public:
    ...
    qtplugin::expected<ValidationResult, PluginError> validate_workflow(
        const qtplugin::workflow::orchestration::Workflow& workflow) const;
    qtplugin::expected<ValidationResult, PluginError>
    validate_workflow_with_config(
        const qtplugin::workflow::orchestration::Workflow& workflow,
        const QJsonObject& validation_config) const;

    // Quick validation
    qtplugin::expected<bool, PluginError> is_workflow_valid(
        const qtplugin::workflow::orchestration::Workflow& workflow) const;
    qtplugin::expected<std::vector<ValidationIssue>, PluginError>
    get_critical_issues(
        const qtplugin::workflow::orchestration::Workflow& workflow) const;
    ...
    std::optional<ValidationRule> get_validation_rule(
        const QString& rule_id) const;
    ...
    qtplugin::expected<qtplugin::workflow::orchestration::Workflow, PluginError>
    auto_fix_workflow(
        const qtplugin::workflow::orchestration::Workflow& workflow,
        const ValidationResult& validation_result) const;
    std::vector<ValidationIssue> get_auto_fixable_issues(
        const ValidationResult& validation_result) const;
...
signals:
    ...
    void critical_issue_detected(const QString& workflow_id,
...
private:
    ...
    std::vector<ValidationIssue> validate_basic_workflow_structure(
        const qtplugin::workflow::orchestration::Workflow& workflow) const;
    std::vector<ValidationIssue> validate_workflow_metadata(
        const qtplugin::workflow::orchestration::Workflow& workflow) const;
};

...
struct WorkflowStep {
    ...
    std::chrono::milliseconds timeout{
        std::chrono::milliseconds{60000}};  // Step timeout
    int max_retries{0};                     // Maximum retry attempts
    std::chrono::milliseconds retry_delay{
        std::chrono::milliseconds{1000}};  // Delay between retries
    bool critical{true};   // Whether failure should stop workflow
    QJsonObject metadata;  // Additional metadata

    WorkflowStep() = default;
    WorkflowStep(const QString& step_id, const QString& plugin,
...
public:
    ...
    Workflow& add_rollback_step(const QString& step_id,
                                const WorkflowStep& rollback_step) {
        m_rollback_steps[step_id] = rollback_step;
        return *this;
    }

    Workflow& set_global_condition(
        std::function<bool(const QJsonObject&)> condition) {
    ...
    const std::unordered_map<QString, WorkflowStep>& steps() const noexcept {
        return m_steps;
    }
    const std::unordered_map<QString, WorkflowStep>& rollback_steps()
        const noexcept {
        return m_rollback_steps;
    }
    ...
    static qtplugin::expected<Workflow, PluginError> from_json(
...
private:
    ...
    std::chrono::milliseconds m_timeout{
        std::chrono::milliseconds{300000}};  // 5 minutes default
...
public:
    ...
    qtplugin::expected<void, PluginError> unregister_workflow(
        const QString& workflow_id);

    qtplugin::expected<Workflow, PluginError> get_workflow(
    ...
    qtplugin::expected<QString, PluginError> execute_workflow(
        const QString& workflow_id, const QJsonObject& initial_data = {},
        bool async = false);

    std::future<qtplugin::expected<QJsonObject, PluginError>>
    execute_workflow_async(const QString& workflow_id,
                           const QJsonObject& initial_data = {});

    qtplugin::expected<void, PluginError> cancel_workflow(
        const QString& execution_id);

    // === Execution Monitoring ===

    qtplugin::expected<QJsonObject, PluginError> get_execution_status(
    ...
    qtplugin::expected<void, PluginError> begin_transaction(
        const QString& transaction_id);
    qtplugin::expected<void, PluginError> commit_transaction(
        const QString& transaction_id);
    qtplugin::expected<void, PluginError> rollback_transaction(
        const QString& transaction_id);

signals:
    void workflow_started(const QString& execution_id,
                          const QString& workflow_id);
    void workflow_completed(const QString& execution_id,
    ...
    void step_completed(const QString& execution_id, const QString& step_id,
                        const QJsonObject& result);
    void step_failed(const QString& execution_id, const QString& step_id,
                     const QString& error);
...
private:
    ...
    std::unordered_map<QString, std::unique_ptr<ExecutionState>>
        m_active_executions;
    std::unordered_map<QString, std::vector<QString>>
        m_transactions;  // transaction_id -> execution_ids

    // Execution methods
    qtplugin::expected<QJsonObject, PluginError> execute_workflow_impl(
        const Workflow& workflow, WorkflowContext& context);
    ...
    bool check_step_dependencies(const WorkflowStep& step,
                                 const WorkflowContext& context) const;
    QJsonObject merge_step_data(const QJsonObject& shared_data,
                                const QJsonObject& step_params) const;
};

}  // namespace qtplugin::workflow::orchestration

Q_DECLARE_METATYPE(qtplugin::workflow::orchestration::StepStatus)
Q_DECLARE_METATYPE(qtplugin::workflow::orchestration::ExecutionMode)
Q_DECLARE_METATYPE(qtplugin::workflow::orchestration::WorkflowStep)
Q_DECLARE_METATYPE(qtplugin::workflow::orchestration::StepResult)

 ...
 */
template<typename T>
using Result = std::variant<T, CommunicationError>;
...
/**
 * @brief Message subscription manager interface (Single Responsibility Principle)
 */
...
public:
    ...
    virtual Result<std::vector<std::shared_ptr<ISubscription>>> find_subscribers(
...
public:
    ...
    Result<std::shared_ptr<ISubscription>> subscribe_to_event(
        ...
        auto type_erased_handler = [handler](const IMessage& msg) {
            if (const auto* typed_event = dynamic_cast<const EventType*>(&msg)) {
                handler(*typed_event);
            }
        };
        ...
        return subscribe_event_impl(subscriber_id, std::type_index(typeid(EventType)),
                                   type_erased_handler, type_erased_filter);
    }

protected:
    virtual Result<void> publish_event_impl(std::shared_ptr<IMessage> event) = 0;
...
public:
    ...
    virtual Result<void> register_service(

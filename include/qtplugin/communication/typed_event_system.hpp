 ...
 * @brief Typed event system using Qt's meta-object system for plugin
 * communication
 * @version 3.0.0
 */
...
struct EventSubscription {
    ...
    EventPriority min_priority =
        EventPriority::Lowest;  ///< Minimum priority filter
    bool is_active = true;      ///< Whether subscription is active
    std::chrono::system_clock::time_point
        created_time;      ///< Subscription creation time
...
struct EventStatistics {
    ...
    std::chrono::milliseconds average_delivery_time{
        0};  ///< Average delivery time
    std::unordered_map<QString, uint64_t>
        events_by_type;  ///< Events count by type
    std::unordered_map<QString, uint64_t>
        events_by_source;  ///< Events count by source
 ...
 * for type-safe plugin communication with filtering, routing, and delivery
 * guarantees.
 */
...
public:
     ...
     */
    qtplugin::expected<EventDeliveryResult, PluginError> publish_event(
     ...
     */
    template <typename T>
    qtplugin::expected<EventDeliveryResult, PluginError> publish(
        const QString& source, const T& data,
        EventDeliveryMode delivery_mode = EventDeliveryMode::Immediate,
        EventRoutingMode routing_mode = EventRoutingMode::Broadcast,
        const std::vector<QString>& recipients = {}) {
        auto event = std::make_unique<TypedEvent<T>>(source, data);
        return publish_event(std::move(event), delivery_mode, routing_mode,
                             recipients);
    }
    ...
    qtplugin::expected<QString, PluginError> subscribe(
        ...
        auto generic_handler = [handler](const IEvent& event) {
            if (const auto* typed_event =
                    dynamic_cast<const TypedEvent<T>*>(&event)) {
                handler(*typed_event);
            }
        };
        ...
        return subscribe(subscriber_id, event_type, generic_handler,
                         generic_filter, min_priority);
    }
     ...
     */
    std::vector<EventSubscription> get_subscriptions(
     ...
     * @return Success or error
     */
    qtplugin::expected<void, PluginError> set_subscription_enabled(
        const QString& subscription_id, bool enabled);
     ...
     */
    void set_event_history_enabled(bool enabled,
     ...
     */
    std::vector<QJsonObject> get_event_history(
        const QString& event_type = QString(), size_t max_events = 100) const;
...
signals:
     ...
     */
    void event_published(const QString& event_type, const QString& source,
     ...
     */
    void event_delivered(const QString& event_id, const QString& recipient,
                         bool success);
     ...
     * @param event_type Event type
     */
    void subscription_created(const QString& subscription_id,
                              const QString& subscriber_id,

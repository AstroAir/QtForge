/**
 * @file factory.hpp
 * @brief Factory interfaces for communication components (Dependency Inversion Principle)
 * @version 3.0.0
 */
 ...
 * Implements Dependency Inversion Principle by allowing injection of dependencies
 */
...
public:
     ...
     */
    virtual std::unique_ptr<IRequestResponseService> create_request_response_service(
...
public:
    explicit CommunicationSystemBuilder(std::shared_ptr<ICommunicationFactory> factory);
    
    CommunicationSystemBuilder& with_config(const CommunicationConfig& config);
    CommunicationSystemBuilder& with_publisher(std::unique_ptr<IMessagePublisher> publisher);
    CommunicationSystemBuilder& with_subscription_manager(std::unique_ptr<ISubscriptionManager> manager);
    CommunicationSystemBuilder& with_router(std::unique_ptr<IMessageRouter> router);
    CommunicationSystemBuilder& with_statistics(std::unique_ptr<IStatistics> statistics);
    CommunicationSystemBuilder& with_event_system(std::unique_ptr<IEventSystem> event_system);
    CommunicationSystemBuilder& with_request_response(std::unique_ptr<IRequestResponseService> service);
...
public:
    ...
    Result<std::shared_ptr<ISubscription>> subscribe_to_event(
        ...
        return event_system_->subscribe_to_event(subscriber_id, handler, filter);
    }

...
public:
     ...
     */
    virtual qtplugin::expected<void, PluginError> register_pool(
        std::shared_ptr<IComponentResourcePool> pool) = 0;
     ...
     * @return Success or error
     */
    virtual qtplugin::expected<void, PluginError> unregister_pool(
     ...
     */
    virtual qtplugin::expected<AllocationRecord, PluginError> allocate_resource(
     ...
     * @return Success or error
     */
    virtual qtplugin::expected<void, PluginError> deallocate_resource(
        const std::string& allocation_id) = 0;
     ...
     */
    virtual void set_allocation_policy(ResourceType resource_type,
                                       const AllocationPolicy& policy) = 0;
     ...
     */
    virtual AllocationPolicy get_allocation_policy(
        ResourceType resource_type) const = 0;
     ...
     */
    virtual ResourceUsageStats get_allocation_statistics(
     ...
     */
    virtual std::vector<AllocationRecord> get_active_allocations(
        std::string_view plugin_id = {}) const = 0;
     ...
     */
    virtual std::vector<std::string> get_available_pools(
        ResourceType resource_type) const = 0;
...
public:
    ...
    qtplugin::expected<void, PluginError> register_pool(
        std::shared_ptr<IComponentResourcePool> pool) override;

    qtplugin::expected<void, PluginError> unregister_pool(
        std::string_view pool_name) override;

    qtplugin::expected<AllocationRecord, PluginError> allocate_resource(
        ResourceType resource_type, std::string_view plugin_id,
        ResourcePriority priority = ResourcePriority::Normal,
        const std::unordered_map<std::string, std::string>& metadata = {})
        override;

    qtplugin::expected<void, PluginError> deallocate_resource(
        const std::string& allocation_id) override;

    void set_allocation_policy(ResourceType resource_type,
                               const AllocationPolicy& policy) override;
    AllocationPolicy get_allocation_policy(
        ResourceType resource_type) const override;

    ResourceUsageStats get_allocation_statistics(
        std::optional<ResourceType> resource_type = std::nullopt,
    ...
    bool can_allocate(ResourceType resource_type, std::string_view plugin_id,
                      ResourcePriority priority) const override;

    std::vector<std::string> get_available_pools(
        ResourceType resource_type) const override;
...
signals:
     ...
     */
    void resource_allocated(const QString& allocation_id,
                            const QString& plugin_id, int resource_type);
     ...
     */
    void resource_deallocated(const QString& allocation_id,
     ...
     */
    void allocation_failed(const QString& plugin_id, int resource_type,
...
private:
    ...
    std::unordered_map<std::string, std::shared_ptr<IComponentResourcePool>>
        m_pools;
    ...
    std::string select_pool(ResourceType resource_type,
                            std::string_view plugin_id,
                            ResourcePriority priority,
                            const AllocationPolicy& policy) const;
    bool check_allocation_limits(ResourceType resource_type,
                                 std::string_view plugin_id,
                                 const AllocationPolicy& policy) const;
    void initialize_default_policies();
    size_t count_plugin_allocations(std::string_view plugin_id,

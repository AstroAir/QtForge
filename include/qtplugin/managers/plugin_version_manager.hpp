...
struct PluginVersionInfo {
    ...
    CompatibilityLevel
        compatibility_level;  ///< Compatibility with previous version
    bool is_active;           ///< Whether this version is currently active
    size_t usage_count;       ///< Number of times this version was used
    QJsonObject
        configuration_schema;  ///< Configuration schema for this version
...
struct MigrationContext {
    ...
    MigrationContext(std::string_view id, const Version& from,
                     const Version& to)
        : plugin_id(id),
          from_version(from),
          to_version(to),
...
struct RollbackInfo {
    ...
    RollbackInfo(std::string_view id, const Version& current,
...
struct VersionError {
    ...
    VersionError(VersionErrorCode c, std::string_view msg,
                 std::string_view id = {},
                 std::optional<Version> ver = std::nullopt)
...
public:
     ...
     */
    virtual bool is_migration_available(std::string_view plugin_id,
     ...
     */
    virtual qtplugin::expected<PluginVersionInfo, VersionError>
    get_version_info(std::string_view plugin_id,
                     std::optional<Version> version = std::nullopt) const = 0;
     ...
     */
    virtual QJsonObject get_storage_usage(
        std::string_view plugin_id = {}) const = 0;
     ...
     * @param subscription_id Subscription ID from
     * register_version_event_callback
     */
    virtual void unregister_version_event_callback(

...
struct LifecycleEvent {
    ...
    LifecycleEvent(std::string_view res_id, ResourceType type,
                   std::string_view plugin, LifecycleState old_st,
                   LifecycleState new_st, const QJsonObject& meta = {})
        : resource_id(res_id),
          resource_type(type),
          plugin_id(plugin),
          old_state(old_st),
...
struct CleanupPolicy {
    ...
    bool should_cleanup_resource(const ResourceHandle& handle,
        ...
        if (state == LifecycleState::Idle && max_idle_time.count() > 0 &&
...
struct ResourceDependency {
    ...
    std::string relationship_type;  ///< Type of dependency (e.g., "parent",
                                    ///< "shared", "weak")
...
public:
     ...
     */
    virtual qtplugin::expected<void, PluginError> register_resource(
        const ResourceHandle& handle,
        LifecycleState initial_state = LifecycleState::Created) = 0;
     ...
     * @return Success or error information
     */
    virtual qtplugin::expected<void, PluginError> unregister_resource(
     ...
     * @return Success or error information
     */
    virtual qtplugin::expected<void, PluginError> update_state(
     ...
     */
    virtual qtplugin::expected<LifecycleState, PluginError> get_state(
     ...
     * @return Success or error information
     */
    virtual qtplugin::expected<void, PluginError> add_dependency(
        const ResourceDependency& dependency) = 0;
     ...
     * @return Success or error information
     */
    virtual qtplugin::expected<void, PluginError> remove_dependency(
        const std::string& dependent_id, const std::string& dependency_id) = 0;
     ...
     * @return Success or error information
     */
    virtual qtplugin::expected<void, PluginError> force_cleanup(
        const std::string& resource_id, bool force_cleanup = false) = 0;
     ...
     */
    virtual std::vector<std::string> get_resources_in_state(
        LifecycleState state) const = 0;
     ...
     */
    virtual std::vector<std::string> get_cleanup_candidates(
        size_t max_candidates = 100) const = 0;

...
int main(int argc, char** argv) {
    ...
    if (!set_ok) {
        qWarning() << "Failed to set thread pool config:" << static_cast<int>(set_ok.error().code);
    }
    ...
    auto id2 = pool.submit_task("delayed_task", "examples.threading", [](){
        QThread::msleep(50);
    }, qtplugin::TaskPriority::High);

    if (!id1) qWarning() << "Submit id1 failed:" << static_cast<int>(id1.error().code);
    if (!id2) qWarning() << "Submit id2 failed:" << static_cast<int>(id2.error().code);
    ...
    qInfo() << "Threading example complete. Executed tasks:" << stats.total_tasks_executed

 ...
 * @brief Implementation of monitoring plugin demonstrating QtForge monitoring
 * features
 * @version 3.0.0
 */
...
MonitoringPlugin::MonitoringPlugin(QObject* parent)
    ...
    m_required_dependencies = {"qtplugin.PluginHotReloadManager",
                               "qtplugin.PluginMetricsCollector"};
...
qtplugin::PluginMetadata MonitoringPlugin::metadata() const {
    ...
    meta.description =
        "Comprehensive monitoring plugin demonstrating QtForge monitoring "
        "features";
    meta.author = "QtForge Team";
    meta.license = "MIT";
    meta.category = "Monitoring";
    meta.tags = {"monitoring", "hot-reload", "metrics", "performance",
                 "example"};

    // Monitoring-specific metadata
    QJsonObject custom_data;
    custom_data[QLatin1String("hot_reload_enabled")] = m_hot_reload_enabled;
    custom_data[QLatin1String("metrics_collection_enabled")] =
        m_metrics_collection_enabled;
...
qtplugin::PluginPriority MonitoringPlugin::priority() const noexcept {
    return qtplugin::PluginPriority::High;  // Monitoring plugins should have
                                            // high priority
}

bool MonitoringPlugin::is_thread_safe() const noexcept { return true; }
...
std::optional<QJsonObject> MonitoringPlugin::default_configuration() const {
    return QJsonObject{
        ...
        {QLatin1String("watched_directories"),
         QJsonArray{QString(QCoreApplication::applicationDirPath() +
                            QLatin1String("/plugins"))}},
        {QLatin1String("metric_types"),
         QJsonArray{QLatin1String("cpu_usage"), QLatin1String("memory_usage"),
                    QLatin1String("plugin_count"),
                    QLatin1String("error_rate")}},
        {QLatin1String("alert_thresholds"),
         QJsonObject{
...
qtplugin::expected<void, qtplugin::PluginError> MonitoringPlugin::configure(
    ...
    if (config.contains(QLatin1String("hot_reload_enabled"))) {
        m_hot_reload_enabled =
            config[QLatin1String("hot_reload_enabled")].toBool();
    }

    if (config.contains(QLatin1String("metrics_collection_enabled"))) {
        m_metrics_collection_enabled =
            config[QLatin1String("metrics_collection_enabled")].toBool();
    }
    ...
    if (config.contains(QLatin1String("monitoring_interval"))) {
        m_monitoring_interval =
            config[QLatin1String("monitoring_interval")].toInt();
    ...
    if (config.contains(QLatin1String("metrics_collection_interval"))) {
        m_metrics_collection_interval =
            config[QLatin1String("metrics_collection_interval")].toInt();
    ...
    if (config.contains(QLatin1String("alert_check_interval"))) {
        m_alert_check_interval =
            config[QLatin1String("alert_check_interval")].toInt();
    ...
    if (config.contains(QLatin1String("metrics_history_size"))) {
        m_metrics_history_size =
            config[QLatin1String("metrics_history_size")].toInt();
    ...
    if (config.contains(QLatin1String("watched_directories"))) {
        QJsonArray dirs =
            config[QLatin1String("watched_directories")].toArray();
...
MonitoringPlugin::execute_command(std::string_view command,
...
std::vector<std::string> MonitoringPlugin::available_commands() const {
    return {"hot_reload", "metrics", "dashboard",
...
QJsonObject MonitoringPlugin::performance_metrics() const {
    ...
    return QJsonObject{
        {QLatin1String("uptime_ms"),
         static_cast<qint64>(current_uptime.count())},
        {QLatin1String("monitoring_cycles"),
         static_cast<qint64>(m_monitoring_cycles.load())},
        {QLatin1String("metrics_collections"),
         static_cast<qint64>(m_metrics_collections.load())},
        {QLatin1String("file_changes_detected"),
         static_cast<qint64>(m_file_changes_detected.load())},
        {QLatin1String("reload_count"),
         static_cast<qint64>(m_reload_count.load())},
        {QLatin1String("alert_count"),
         static_cast<qint64>(m_alert_count.load())},
        ...
        {QLatin1String("monitored_plugins_count"),
         static_cast<int>(m_monitored_plugins.size())},
        {QLatin1String("metrics_history_size"),
         static_cast<int>(m_metrics_history.size())}};
}
...
QJsonObject MonitoringPlugin::resource_usage() const {
    ...
    auto cpu_estimate =
        (m_monitoring_timer && m_monitoring_timer->isActive()) ? 2.0 : 0.1;

    return QJsonObject{
        {QLatin1String("estimated_memory_kb"),
         static_cast<qint64>(memory_estimate)},
        {QLatin1String("estimated_cpu_percent"), cpu_estimate},
        {QLatin1String("thread_count"), 1},
        {QLatin1String("monitoring_timer_active"),
         m_monitoring_timer && m_monitoring_timer->isActive()},
        {QLatin1String("metrics_timer_active"),
         m_metrics_timer && m_metrics_timer->isActive()},
        {QLatin1String("alert_timer_active"),
         m_alert_timer && m_alert_timer->isActive()},
        {QLatin1String("watched_files_count"), m_file_watcher->files().size()},
        {QLatin1String("watched_directories_count"),
         m_file_watcher->directories().size()},
        {QLatin1String("error_log_size"),
...
MonitoringPlugin::enable_hot_reload(const QString& plugin_id,
    ...
    try {
        ...
        auto result = m_hot_reload_manager->enable_hot_reload(
            plugin_id.toStdString(), path);
        ...
        if (result) {
            ...
            log_info("Hot reload enabled for plugin: " +
        ...
        std::string error_msg =
            "Failed to enable hot reload: " + std::string(e.what());
        log_error(error_msg);
        return qtplugin::make_error<void>(
            qtplugin::PluginErrorCode::ExecutionFailed, error_msg);
    }
...
qtplugin::expected<void, qtplugin::PluginError>
MonitoringPlugin::disable_hot_reload(const QString& plugin_id) {
    if (!m_hot_reload_manager) {
        return qtplugin::make_error<void>(
            qtplugin::PluginErrorCode::InitializationFailed,
            "Hot reload manager not initialized");
    ...
    try {
...

        {
            ...
            QString file_path =
                (it != m_monitored_plugins.end()) ? it->second : QString();
            m_monitored_plugins.erase(plugin_id);
            m_last_reload_times.erase(plugin_id);

            // Remove from file watcher if no other plugins are watching this
            // file
            ...
            log_info("Hot reload disabled for plugin: " +
                     plugin_id.toStdString());
        }
        ...
        std::string error_msg =
            "Failed to disable hot reload: " + std::string(e.what());
        log_error(error_msg);
        return qtplugin::make_error<void>(
            qtplugin::PluginErrorCode::ExecutionFailed, error_msg);
    }
...
QJsonObject MonitoringPlugin::collect_plugin_metrics(const QString& plugin_id) {
    if (!m_metrics_collector) {
        return QJsonObject{
            {QLatin1String("error"),
             QLatin1String("Metrics collector not initialized")}};
    }

    try {
        auto metrics = m_metrics_collector->get_plugin_metrics(
            plugin_id.toStdString(), nullptr);

        // Convert to QJsonObject
        QJsonObject result;
        result[QLatin1String("plugin_id")] = plugin_id;
        result[QLatin1String("timestamp")] =
            QDateTime::currentDateTime().toString(Qt::ISODate);
        result[QLatin1String("metrics")] =
            metrics;  // Assuming metrics is already a QJsonObject
...
QJsonObject MonitoringPlugin::get_monitoring_dashboard() const {
    ...
    dashboard[QLatin1String("timestamp")] =
        QDateTime::currentDateTime().toString(Qt::ISODate);
    dashboard[QLatin1String("uptime_ms")] =
        static_cast<qint64>(uptime().count());

    // System overview
    QJsonObject system_overview;
    system_overview[QLatin1String("monitored_plugins")] =
        static_cast<int>(m_monitored_plugins.size());
    system_overview[QLatin1String("total_reloads")] =
        static_cast<qint64>(m_reload_count.load());
    system_overview[QLatin1String("active_alerts")] =
        static_cast<int>(m_active_alerts.size());
    system_overview[QLatin1String("metrics_collections")] =
        static_cast<qint64>(m_metrics_collections.load());
    ...
    for (const auto& [plugin_id, metrics] : m_plugin_metrics) {
        ...
        summary[QLatin1String("last_update")] =
            metrics.value(QLatin1String("timestamp"));
        summary[QLatin1String("status")] =
            QLatin1String("active");  // Could be enhanced with actual status
...
qtplugin::expected<void, qtplugin::PluginError> MonitoringPlugin::setup_alerts(
    ...
    try {
        ...
        std::string error_msg =
            "Failed to setup alerts: " + std::string(e.what());
        log_error(error_msg);
        return qtplugin::make_error<void>(
            qtplugin::PluginErrorCode::ExecutionFailed, error_msg);
    }
...
QJsonObject MonitoringPlugin::get_historical_metrics(
    ...
    for (const auto& metric : m_metrics_history) {
        ...
        if (metric_time >= start_time && metric_time <= end_time) {
            if (plugin_id.isEmpty() ||
                metric.value(QLatin1String("plugin_id")).toString() ==
                    plugin_id) {
...
QJsonObject MonitoringPlugin::handle_hot_reload_command(
    const QJsonObject& params) {
    QString action =
        params.value(QLatin1String("action")).toString(QLatin1String("status"));

    if (action == QLatin1String("enable")) {
        if (!params.contains(QLatin1String("plugin_id")) ||
            !params.contains(QLatin1String("file_path"))) {
            return QJsonObject{
                {QLatin1String("error"),
                 QLatin1String(
                     "Missing required parameters: plugin_id, file_path")},
        ...
        if (!params.contains(QLatin1String("plugin_id"))) {
            return QJsonObject{
                {QLatin1String("error"),
                 QLatin1String("Missing required parameter: plugin_id")},
                {QLatin1String("success"), false}};
        }
        ...
        for (auto it = m_monitored_plugins.begin();
             it != m_monitored_plugins.end(); ++it) {
            ...
            if (reload_time_it != m_last_reload_times.end()) {
                ...
                plugin_info[QLatin1String("last_reload")] =
                    QDateTime::fromSecsSinceEpoch(time_t).toString(Qt::ISODate);
            }
        ...
        return QJsonObject{
            {QLatin1String("error"),
             QLatin1String(
                 "Invalid action. Supported: enable, disable, status")},
            {QLatin1String("success"), false}};
    }
}

QJsonObject MonitoringPlugin::handle_metrics_command(
    const QJsonObject& params) {
    ...
    if (plugin_id.isEmpty()) {
        ...
        return QJsonObject{{QLatin1String("action"), QLatin1String("get_all")},
        ...
        return QJsonObject{
            ...
            {QLatin1String("success"),
             !metrics.contains(QLatin1String("error"))}};
    }
}

QJsonObject MonitoringPlugin::handle_dashboard_command(
    const QJsonObject& params) {
    Q_UNUSED(params)

    auto dashboard = get_monitoring_dashboard();

    return QJsonObject{{QLatin1String("action"), QLatin1String("dashboard")},
...
QJsonObject MonitoringPlugin::handle_alerts_command(const QJsonObject& params) {
    QString action =
        params.value(QLatin1String("action")).toString(QLatin1String("get"));
    ...
    if (action == QLatin1String("get")) {
        ...
        return QJsonObject{{QLatin1String("action"), QLatin1String("get")},
        ...
        QJsonObject alert_config =
            params.value(QLatin1String("config")).toObject();
        ...
        return QJsonObject{
            {QLatin1String("error"),
             QLatin1String("Invalid action. Supported: get, setup, clear")},
            {QLatin1String("success"), false}};
    }
}
...
QJsonObject MonitoringPlugin::handle_status_command(const QJsonObject& params) {
    ...
    status[QLatin1String("metrics_collection_enabled")] =
        m_metrics_collection_enabled;
    status[QLatin1String("alerts_enabled")] = m_alerts_enabled;

    // Component status
    QJsonObject components;
    components[QLatin1String("hot_reload_manager")] =
        m_hot_reload_manager != nullptr;
    components[QLatin1String("metrics_collector")] =
        m_metrics_collector != nullptr;
    components[QLatin1String("file_watcher")] = m_file_watcher != nullptr;
    status[QLatin1String("components")] = components;

    // Statistics
    QJsonObject stats;
    stats[QLatin1String("monitoring_cycles")] =
        static_cast<qint64>(m_monitoring_cycles.load());
    stats[QLatin1String("metrics_collections")] =
        static_cast<qint64>(m_metrics_collections.load());
    stats[QLatin1String("file_changes_detected")] =
        static_cast<qint64>(m_file_changes_detected.load());
    stats[QLatin1String("reload_count")] =
        static_cast<qint64>(m_reload_count.load());
    stats[QLatin1String("alert_count")] =
        static_cast<qint64>(m_alert_count.load());
    status[QLatin1String("statistics")] = stats;

    // Current monitoring state
    QJsonObject monitoring_state;
    monitoring_state[QLatin1String("monitored_plugins")] =
        static_cast<int>(m_monitored_plugins.size());
    monitoring_state[QLatin1String("active_alerts")] =
        static_cast<int>(m_active_alerts.size());
    monitoring_state[QLatin1String("metrics_history_size")] =
        static_cast<int>(m_metrics_history.size());
    monitoring_state[QLatin1String("watched_files")] =
        m_file_watcher->files().size();
    monitoring_state[QLatin1String("watched_directories")] =
        m_file_watcher->directories().size();
    status[QLatin1String("monitoring_state")] = monitoring_state;

    status[QLatin1String("timestamp")] =
        QDateTime::currentDateTime().toString(Qt::ISODate);
...
QJsonObject MonitoringPlugin::handle_history_command(
    const QJsonObject& params) {
    QJsonObject time_range =
        params.value(QLatin1String("time_range")).toObject();
    QString plugin_id = params.value(QLatin1String("plugin_id")).toString();

    auto history = get_historical_metrics(time_range, plugin_id);

    return QJsonObject{{QLatin1String("action"), QLatin1String("history")},
...
void MonitoringPlugin::collect_system_metrics() {
    ...
    system_metrics[QLatin1String("timestamp")] =
        QDateTime::currentDateTime().toString(Qt::ISODate);
    system_metrics[QLatin1String("uptime_ms")] =
        static_cast<qint64>(uptime().count());
    system_metrics[QLatin1String("monitored_plugins")] =
        static_cast<int>(m_monitored_plugins.size());
    system_metrics[QLatin1String("active_alerts")] =
        static_cast<int>(m_active_alerts.size());
    system_metrics[QLatin1String("file_changes")] =
        static_cast<qint64>(m_file_changes_detected.load());
    system_metrics[QLatin1String("reload_count")] =
        static_cast<qint64>(m_reload_count.load());
    ...
    for (auto it = m_alert_config.begin(); it != m_alert_config.end(); ++it) {
        ...
        if (condition_value.isObject()) {
            ...
            if (evaluate_alert_condition(condition, current_metrics)) {
                ...
                alert[QLatin1String("triggered_at")] =
                    QDateTime::currentDateTime().toString(Qt::ISODate);
                alert[QLatin1String("metrics")] = current_metrics;
                alert[QLatin1String("severity")] =
                    condition.value(QLatin1String("severity"))
                        .toString(QLatin1String("warning"));

                // Check if this alert is already active
                bool already_active = false;
                for (const auto& active_alert : m_active_alerts) {
                    if (active_alert.value(QLatin1String("condition"))
                            .toString() == condition_name) {
                        already_active = true;
                        break;
                    }
                }
                ...
                if (!already_active) {
                    ...
                    log_error("Alert triggered: " +
                              condition_name.toStdString());
                }
            }
        }
    }
...
void MonitoringPlugin::process_file_change(const QString& file_path) {
    ...
    for (auto it = m_monitored_plugins.begin(); it != m_monitored_plugins.end();
         ++it) {
        if (it->second == file_path) {
            ...
            if (m_hot_reload_enabled && m_hot_reload_manager) {
                // Hot reload manager uses callback mechanism, so we just log
                // the file change The actual reload will be handled by the
                // callback set during initialization
                m_reload_count.fetch_add(1);
                m_last_reload_times[plugin_id] =
                    std::chrono::system_clock::now();
                log_info("File change detected for plugin: " +
                         plugin_id.toStdString());
            }
            break;
        }
...
QJsonObject MonitoringPlugin::create_metric_entry(const QString& plugin_id,
                                                  const QJsonObject& data) {
    QJsonObject entry;
    entry[QLatin1String("plugin_id")] = plugin_id;
    entry[QLatin1String("timestamp")] =
        QDateTime::currentDateTime().toString(Qt::ISODate);
...
void MonitoringPlugin::maintain_metrics_history() {
    ...
    while (static_cast<int>(m_metrics_history.size()) >
           m_metrics_history_size) {
        m_metrics_history.erase(m_metrics_history.begin());
    }
}

bool MonitoringPlugin::evaluate_alert_condition(const QJsonObject& condition,
                                                const QJsonObject& metrics) {
    QString metric_name = condition.value(QLatin1String("metric")).toString();
    QString operator_str =
        condition.value(QLatin1String("operator")).toString();
    ...
    // Evaluate condition
    if (operator_str == QLatin1String("greater_than") ||
        operator_str == QLatin1String(">")) {
        return value > threshold;
    } else if (operator_str == QLatin1String("less_than") ||
               operator_str == QLatin1String("<")) {
        return value < threshold;
    } else if (operator_str == QLatin1String("equals") ||
               operator_str == QLatin1String("==")) {
        return qAbs(value - threshold) <
               0.001;  // Float comparison with tolerance
    } else if (operator_str == QLatin1String("greater_equal") ||
               operator_str == QLatin1String(">=")) {
        return value >= threshold;
    } else if (operator_str == QLatin1String("less_equal") ||
               operator_str == QLatin1String("<=")) {
        return value <= threshold;
    }
...
std::string_view MonitoringPlugin::description() const noexcept {
    return "Advanced monitoring plugin with hot reload, metrics collection, "
           "and alerting";
}

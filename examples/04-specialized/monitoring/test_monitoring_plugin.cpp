...
public:
    explicit MonitoringPluginTester(QObject* parent = nullptr)
        : QObject(parent) {}
    ...
    int run_tests(const QString& test_type) {
        ...
        if (test_type == "basic") {
            ...
            success =
                test_basic_functionality() && test_hot_reload_functionality() &&
                test_metrics_functionality() && test_alerts_functionality() &&
                test_dashboard_functionality();
        } else {
            qCritical() << "Unknown test type:" << test_type;
            qInfo() << "Available test types: basic, hot_reload, metrics, "
                       "alerts, all";
            return 1;
        }
...
private:
    ...
    bool test_hot_reload_functionality() {
        ...
        QJsonObject enable_params{{"action", "enable"},
                                  {"plugin_id", "test_plugin"},
                                  {"file_path", temp_path}};

        auto enable_result =
            m_plugin->execute_command("hot_reload", enable_params);
        if (!enable_result) {
            qCritical() << "Hot reload enable failed:"
                        << QString::fromStdString(
                               enable_result.error().message);
            return false;
        }
        ...
        auto status_result =
            m_plugin->execute_command("hot_reload", status_params);
        if (!status_result) {
        ...
        QJsonObject disable_params{{"action", "disable"},
                                   {"plugin_id", "test_plugin"}};

        auto disable_result =
            m_plugin->execute_command("hot_reload", disable_params);
        if (!disable_result) {
            qCritical() << "Hot reload disable failed:"
                        << QString::fromStdString(
                               disable_result.error().message);
            return false;
        }
    ...
    bool test_metrics_functionality() {
        ...
        auto all_metrics_result =
            m_plugin->execute_command("metrics", QJsonObject{});
        if (!all_metrics_result) {
            qCritical() << "All metrics collection failed:"
                        << QString::fromStdString(
                               all_metrics_result.error().message);
            return false;
        }
        ...
        auto specific_result =
            m_plugin->execute_command("metrics", specific_params);
        if (!specific_result) {
            qCritical() << "Specific metrics collection failed:"
                        << QString::fromStdString(
                               specific_result.error().message);
            return false;
        }
        ...
        QJsonObject history_params{
            {"time_range",
             QJsonObject{
                 {"start", QDateTime::currentDateTime().addSecs(-3600).toString(
                               Qt::ISODate)},
                 {"end", QDateTime::currentDateTime().toString(Qt::ISODate)}}}};

        auto history_result =
            m_plugin->execute_command("history", history_params);
        if (!history_result) {
            qCritical() << "Historical metrics failed:"
                        << QString::fromStdString(
                               history_result.error().message);
            return false;
        }
    ...
    bool test_alerts_functionality() {
        ...
        QJsonObject alert_config{
            {"cpu_usage_max", QJsonObject{{"metric", "cpu_usage"},
                                          {"operator", "greater_than"},
                                          {"threshold", 80.0},
                                          {"severity", "warning"}}},
            {"memory_usage_max", QJsonObject{{"metric", "memory_usage"},
                                             {"operator", "greater_than"},
                                             {"threshold", 1024.0},
                                             {"severity", "critical"}}}};

        QJsonObject setup_params{{"action", "setup"}, {"config", alert_config}};
    ...
    bool test_dashboard_functionality() {
        ...
        auto dashboard_result =
            m_plugin->execute_command("dashboard", QJsonObject{});
        if (!dashboard_result) {
            qCritical() << "Dashboard command failed:"
                        << QString::fromStdString(
                               dashboard_result.error().message);
            return false;
        }

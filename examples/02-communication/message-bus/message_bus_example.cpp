...
AdvancedMessageBusExample::AdvancedMessageBusExample(QObject* parent)
    : QObject(parent),
      ...
      m_batch_processor(
          std::make_unique<utils::MessageBatchProcessor>(m_batch_size)),
      m_monitoring_timer(new QTimer(this)),
      m_start_time(std::chrono::steady_clock::now()) {
    // Connect monitoring timer
    connect(m_monitoring_timer, &QTimer::timeout, this,
            &AdvancedMessageBusExample::on_monitoring_timer);
...
QJsonObject AdvancedMessageBusExample::get_statistics() const {
    ...
    stats["messages_received"] =
        static_cast<qint64>(m_messages_received.load());
...
void AdvancedMessageBusExample::demonstrate_basic_messaging() {
    ...
    auto message = utils::create_test_system_event(
        SystemEventMessage::EventType::SystemStartup, "example_app");
    ...
    if (result.has_value()) {
        ...
        m_messages_failed++;
        m_statistics_collector->record_message_failed("system_event",
                                                      result.error().message);
        log_message_activity("Failed to publish system startup event");
    }

    // Publish performance metrics
    auto metrics = utils::create_test_performance_metrics("example_app", 25.5,
                                                          1024 * 1024 * 50);
...
void AdvancedMessageBusExample::demonstrate_priority_messaging() {
    ...
    std::vector<SystemEventMessage::Priority> priorities = {
        SystemEventMessage::Priority::Low, SystemEventMessage::Priority::Normal,
    ...
    for (auto priority : priorities) {
        ...
        data["message"] =
            QString("Priority %1 message").arg(static_cast<int>(priority));
        ...
        if (result.has_value()) {
            m_messages_sent++;
            m_statistics_collector->record_message_sent("system_event");
            log_message_activity(QString("Priority %1 message published")
...
void AdvancedMessageBusExample::demonstrate_filtered_subscriptions() {
    ...
    PriorityMessageFilter high_priority_filter(
        SystemEventMessage::Priority::High);
    ...
    std::vector<SystemEventMessage::Priority> test_priorities = {
        SystemEventMessage::Priority::Low, SystemEventMessage::Priority::High,
        SystemEventMessage::Priority::Normal,
        SystemEventMessage::Priority::Critical};

    for (auto priority : test_priorities) {
        auto message = utils::create_test_system_event(
            SystemEventMessage::EventType::ErrorOccurred, "error_handler");
    ...
    log_message_activity(QString("Published %1 messages with mixed priorities")
                             .arg(test_priorities.size()));
...
void AdvancedMessageBusExample::demonstrate_message_statistics() {
    ...
    if (stats.contains("detailed_stats")) {
        ...
        if (detailed.contains("runtime_seconds")) {
            std::cout << "   - Runtime: " << detailed["runtime_seconds"].toInt()
                      << " seconds\n";
        }
    }
    ...
    if (stats.contains("bus_stats")) {
        ...
        if (bus_stats.contains("total_messages")) {
            std::cout << "   - Bus total messages: "
                      << bus_stats["total_messages"].toInt() << "\n";
        }
        if (bus_stats.contains("active_subscriptions")) {
            std::cout << "   - Active subscriptions: "
                      << bus_stats["active_subscriptions"].toInt() << "\n";
        }
    }
}
...
void AdvancedMessageBusExample::on_system_event_received(
    std::shared_ptr<SystemEventMessage> message) {
    m_messages_received++;
...
void AdvancedMessageBusExample::on_performance_metrics_received(
    ...
    if (m_verbose_logging) {
        log_message_activity(
            QString("Performance metrics received: CPU=%1%, Memory=%2MB")
...
void AdvancedMessageBusExample::on_monitoring_timer() {
    ...
    auto metrics = utils::create_test_performance_metrics(
        "monitoring_system", static_cast<double>(rand() % 100),
...
void AdvancedMessageBusExample::demonstrate_error_handling() {
    ...
    auto message = utils::create_test_system_event(
        SystemEventMessage::EventType::ErrorOccurred, "error_simulator");
    ...
    if (!result.has_value()) {
        ...
        m_statistics_collector->record_message_failed("system_event",
                                                      result.error().message);
        log_message_activity(QString("Expected error occurred: %1")
...
void AdvancedMessageBusExample::demonstrate_performance_monitoring() {
    ...
    for (int i = 0; i < 50; ++i) {
        auto message = utils::create_test_system_event(
            SystemEventMessage::EventType::PluginLoaded, "load_tester");
...
void AdvancedMessageBusExample::demonstrate_subscription_management() {
    ...
    auto subscribers =
        m_message_bus->subscribers(std::type_index(typeid(SystemEventMessage)));
    log_message_activity(QString("Current system event subscribers: %1")
                             .arg(subscribers.size()));
...
void AdvancedMessageBusExample::demonstrate_message_batching() {
    ...
    // Publish batch
    log_message_activity(QString("Publishing batch of %1 messages...")
                             .arg(batch_messages.size()));
...
void AdvancedMessageBusExample::publish_test_messages() {
    ...
    for (int i = 0; i < 5; ++i) {
        auto system_event = utils::create_test_system_event(
            SystemEventMessage::EventType::PluginLoaded, "test_publisher");

        auto metrics = utils::create_test_performance_metrics(
            "test_publisher", static_cast<double>(rand() % 100),
            1024 * 1024 * (rand() % 100));

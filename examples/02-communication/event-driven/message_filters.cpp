...
PriorityMessageFilter::PriorityMessageFilter(
    SystemEventMessage::Priority min_priority)
    : m_min_priority(min_priority) {}

bool PriorityMessageFilter::operator()(
    const SystemEventMessage& message) const {
    return message.priority() >= m_min_priority;
}

// EventTypeMessageFilter Implementation
EventTypeMessageFilter::EventTypeMessageFilter(
    SystemEventMessage::EventType target_type)
    : m_target_type(target_type) {}

bool EventTypeMessageFilter::operator()(
    const SystemEventMessage& message) const {

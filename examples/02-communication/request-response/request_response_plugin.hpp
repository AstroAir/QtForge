 ...
 * 
 * Demonstrates synchronous and asynchronous request-response patterns including:
...
class RequestResponsePlugin : public QObject, public qtplugin::IPlugin {
    Q_OBJECT
    Q_PLUGIN_METADATA(IID "qtplugin.IPlugin/3.0" FILE "request_response_plugin.json")
...
public:
    ...
    QString description() const override { return "Request-Response communication pattern example"; }
...
public slots:
    void on_request_received(const QString& request_id, const QJsonObject& request);
    void on_response_received(const QString& request_id, const QJsonObject& response);
...
private:
    ...
    // Request handling
    QString send_request(const QJsonObject& request, const QString& target, bool async = false, int priority = 0);
    void handle_request(const QString& request_id, const QJsonObject& request);
    void send_response(const QString& request_id, const QJsonObject& response, bool success = true);
    void handle_response(const QString& request_id, const QJsonObject& response);
    ...
    void update_statistics(const QString& request_id, bool success, double response_time);

...
public:
    explicit RequestResponseTester(QObject* parent = nullptr) : QObject(parent) {}
...
public slots:
    void run_tests() {
        ...
        qDebug() << "Plugin metadata:" << QJsonDocument(plugin->metadata()).toJson(QJsonDocument::Compact);
        ...
        qDebug() << "\n--- Test 2: Send Simple Request ---";
        QJsonObject send_result = plugin->execute_command("send_request", QJsonObject{
            {"target", "test_service"},
            {"request", QJsonObject{
                {"action", "get_data"},
        ...
        qDebug() << "Send request result:" << QJsonDocument(send_result).toJson(QJsonDocument::Compact);
        
        // Test 3: Send multiple requests
        qDebug() << "\n--- Test 3: Send Multiple Requests ---";
        for (int i = 0; i < 5; ++i) {
            QJsonObject multi_result = plugin->execute_command("send_request", QJsonObject{
            ...
            qDebug() << "Batch request" << i << "result:" << QJsonDocument(multi_result).toJson(QJsonDocument::Compact);
        }
        
        // Test 4: List pending requests
        qDebug() << "\n--- Test 4: List Pending Requests ---";
        QJsonObject pending_result = plugin->execute_command("list_pending");
        qDebug() << "Pending requests:" << QJsonDocument(pending_result).toJson(QJsonDocument::Compact);
        
        // Test 5: Get statistics
        qDebug() << "\n--- Test 5: Get Statistics ---";
        QJsonObject stats_result = plugin->execute_command("get_statistics");
        qDebug() << "Statistics:" << QJsonDocument(stats_result).toJson(QJsonDocument::Compact);
        ...
        if (!request_id_to_cancel.isEmpty()) {
            QJsonObject cancel_result = plugin->execute_command("cancel_request", QJsonObject{
                {"request_id", request_id_to_cancel}
            });
            qDebug() << "Cancel request result:" << QJsonDocument(cancel_result).toJson(QJsonDocument::Compact);
        }
        
        // Test 7: Invalid request (missing parameters)
        qDebug() << "\n--- Test 7: Invalid Request ---";
        QJsonObject invalid_result = plugin->execute_command("send_request", QJsonObject{
            {"target", ""}  // Empty target
        });
        qDebug() << "Invalid request result:" << QJsonDocument(invalid_result).toJson(QJsonDocument::Compact);
        
        // Test 8: Unknown command
        qDebug() << "\n--- Test 8: Unknown Command ---";
        QJsonObject unknown_result = plugin->execute_command("unknown_command");
        qDebug() << "Unknown command result:" << QJsonDocument(unknown_result).toJson(QJsonDocument::Compact);
        ...
        QTimer::singleShot(5000, [this, &plugin]() {
            ...
            qDebug() << "Final statistics:" << QJsonDocument(final_stats).toJson(QJsonDocument::Compact);
            
            // Test 10: List remaining pending requests
            qDebug() << "\n--- Test 10: Remaining Pending Requests ---";
            QJsonObject remaining_pending = plugin->execute_command("list_pending");
            qDebug() << "Remaining pending:" << QJsonDocument(remaining_pending).toJson(QJsonDocument::Compact);
            
            // Test 11: Clear statistics
            qDebug() << "\n--- Test 11: Clear Statistics ---";
            QJsonObject clear_result = plugin->execute_command("clear_statistics");
            qDebug() << "Clear statistics result:" << QJsonDocument(clear_result).toJson(QJsonDocument::Compact);
            
            // Test 12: Verify statistics cleared
            qDebug() << "\n--- Test 12: Verify Statistics Cleared ---";
            QJsonObject cleared_stats = plugin->execute_command("get_statistics");
            qDebug() << "Cleared statistics:" << QJsonDocument(cleared_stats).toJson(QJsonDocument::Compact);
            
            // Test 13: Final metadata check
            qDebug() << "\n--- Test 13: Final Metadata Check ---";
            QJsonObject final_metadata = plugin->metadata();
            qDebug() << "Final metadata:" << QJsonDocument(final_metadata).toJson(QJsonDocument::Compact);
            
            // Shutdown
            qDebug() << "\n--- Shutdown ---";
            plugin->shutdown();
            qDebug() << "Plugin state after shutdown:" << static_cast<int>(plugin->state());

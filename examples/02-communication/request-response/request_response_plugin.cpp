...
RequestResponsePlugin::RequestResponsePlugin(QObject* parent)
    : QObject(parent), m_stats_start_time(QDateTime::currentDateTime()) {
...
bool RequestResponsePlugin::initialize(const QJsonObject& config) {
    ...
    try {
        ...
        if (config.contains("enable_request_queuing")) {
            m_enable_request_queuing =
                config["enable_request_queuing"].toBool();
        }
        ...
        connect(m_cleanup_timer.get(), &QTimer::timeout, this,
                &RequestResponsePlugin::cleanup_expired_requests);
        ...
        if (m_enable_request_queuing) {
            ...
            connect(m_queue_processor.get(), &QTimer::timeout, this,
                    &RequestResponsePlugin::process_request_queue);
        ...
        qCritical() << "RequestResponsePlugin: Initialization failed:"
                    << e.what();
...
QJsonObject RequestResponsePlugin::metadata() const {
    ...
    return QJsonObject{
        ...
        {"configuration",
         QJsonObject{{"default_timeout_ms", m_default_timeout_ms},
        ...
        {"statistics",
         QJsonObject{
             ...
             {"uptime_seconds",
              m_stats_start_time.secsTo(QDateTime::currentDateTime())}}},
        {"commands",
         QJsonArray{"send_request", "get_statistics", "clear_statistics",
                    "list_pending", "cancel_request"}}};
}

QJsonObject RequestResponsePlugin::execute_command(const QString& command,
...
QString RequestResponsePlugin::send_request(const QJsonObject& request,
                                            const QString& target, bool async,
                                            int priority) {
    ...
    pending_request.timeout =
        pending_request.timestamp.addMSecs(m_default_timeout_ms);
    ...
    qDebug() << "RequestResponsePlugin: Sending request" << request_id << "to"
             << target;
...
void RequestResponsePlugin::handle_response(const QString& request_id,
    ...
    if (!pending) {
        qWarning()
            << "RequestResponsePlugin: Received response for unknown request:"
            << request_id;
        return;
    }

    double response_time =
        pending->timestamp.msecsTo(QDateTime::currentDateTime());
    bool success = response["status"].toString() == "success";

    update_statistics(request_id, success, response_time);

    qDebug() << "RequestResponsePlugin: Received response for request"
             << request_id << "in" << response_time
             << "ms, success:" << success;
...
void RequestResponsePlugin::add_pending_request(const PendingRequest& request) {
    ...
    if (m_pending_requests.size() >=
        static_cast<size_t>(m_max_pending_requests)) {
        qWarning() << "RequestResponsePlugin: Maximum pending requests "
                      "reached, dropping oldest";
...
RequestResponsePlugin::PendingRequest*
RequestResponsePlugin::find_pending_request(const QString& request_id) {
    auto it = m_pending_requests.find(request_id);
    return (it != m_pending_requests.end()) ? &it->second : nullptr;
}

void RequestResponsePlugin::update_statistics(const QString& request_id,
                                              bool success,
                                              double response_time) {
    Q_UNUSED(request_id)
    ...
    int total_responses =
        m_stats.successful_responses.load() + m_stats.failed_responses.load();
    double new_avg = ((current_avg * (total_responses - 1)) + response_time) /
                     total_responses;
    m_stats.average_response_time.store(new_avg);
}

QJsonObject RequestResponsePlugin::execute_send_request_command(
    const QJsonObject& params) {
...
QJsonObject RequestResponsePlugin::execute_get_statistics_command(
    const QJsonObject& params) {
    Q_UNUSED(params)
        ...
        {"statistics",
         QJsonObject{
             ...
             {"uptime_seconds",
              m_stats_start_time.secsTo(QDateTime::currentDateTime())},
             {"success_rate",
              m_stats.total_requests.load() > 0
                  ? (static_cast<double>(m_stats.successful_responses.load()) /
                     m_stats.total_requests.load()) *
                        100.0
                  : 0.0}}},
        {"timestamp", QDateTime::currentDateTime().toString(Qt::ISODate)}};
}

QJsonObject RequestResponsePlugin::execute_clear_statistics_command(
    const QJsonObject& params) {
    Q_UNUSED(params)
...
QJsonObject RequestResponsePlugin::execute_list_pending_command(
    const QJsonObject& params) {
    Q_UNUSED(params)
    ...
    for (const auto& [id, request] : m_pending_requests) {
        pending_array.append(QJsonObject{
            ...
            {"age_ms",
             request.timestamp.msecsTo(QDateTime::currentDateTime())}});
    }
...
QJsonObject RequestResponsePlugin::execute_cancel_request_command(
    const QJsonObject& params) {
...
void RequestResponsePlugin::on_request_received(const QString& request_id,
                                                const QJsonObject& request) {
    qDebug() << "RequestResponsePlugin: Received request:" << request_id;
    handle_request(request_id, request);
}

void RequestResponsePlugin::on_response_received(const QString& request_id,
                                                 const QJsonObject& response) {
...
void RequestResponsePlugin::cleanup_expired_requests() {
    ...
    while (it != m_pending_requests.end()) {
        if (now > it->second.timeout) {
            qDebug() << "RequestResponsePlugin: Cleaning up expired request:"
                     << it->first;
            m_stats.timeout_responses.fetch_add(1);
...
void RequestResponsePlugin::handle_request(const QString& request_id,
                                           const QJsonObject& request) {
...
void RequestResponsePlugin::send_response(const QString& request_id,
                                          const QJsonObject& response,
                                          bool success) {
    Q_UNUSED(request_id)
    Q_UNUSED(response)

 ...
 * @brief Enhanced basic plugin demonstrating ALL QtPlugin system capabilities
 * @version 3.0.0
 *
 * This enhanced basic plugin serves as a comprehensive example and template
 * for all QtPlugin system features, demonstrating every public API method
 * with proper error handling, thread safety, and documentation.
 */
...
public:
    ...
    std::string_view category() const noexcept override { return "Example"; }
...
private:
    ...
    QJsonObject handle_lifecycle_command(const QJsonObject& params);
    QJsonObject handle_monitoring_command(const QJsonObject& params);
    QJsonObject handle_dependencies_command(const QJsonObject& params);
    QJsonObject handle_capabilities_command(const QJsonObject& params);
};

/**
 * @brief Enhanced plugin factory for creating BasicPlugin instances
 *
 * This factory demonstrates plugin creation patterns and provides
 * comprehensive plugin instantiation with validation and configuration.
 */
...
public:
    ...
    /**
     * @brief Create a new enhanced BasicPlugin instance
     * @param parent Parent QObject
     * @param config Initial configuration
     * @return Unique pointer to the created plugin
     */
    static std::unique_ptr<BasicPlugin> create_plugin(
        QObject* parent = nullptr, const QJsonObject& config = {}) {
        ...
        if (!config.isEmpty()) {
            ...
            if (!result) {
                ...
                qWarning() << "Failed to configure BasicPlugin:"
    ...
    static bool can_create_with_requirements(const QJsonObject& requirements) {
        ...
        if (requirements.contains("required_capabilities")) {
            auto required_caps =
                requirements["required_capabilities"].toArray();
            for (const auto& cap : required_caps) {
                QString cap_str = cap.toString();
                if (cap_str != "Service" && cap_str != "Configuration" &&
                    cap_str != "Logging") {
                    return false;
                }
            }
        ...
        if (requirements.contains("min_version")) {
            auto min_version = qtplugin::Version::parse(
                requirements["min_version"].toString().toStdString());
...
/**
 * @brief Demonstrate enhanced basic plugin usage
 *
 * This function shows how to create, configure, and use the enhanced basic
 * plugin demonstrating ALL QtPlugin system capabilities including lifecycle
 * management, configuration, commands, monitoring, and error handling.
 */
inline void demonstrate_enhanced_basic_plugin() {
    ...
    if (!init_result) {
        qCritical() << "Failed to initialize plugin:"
    ...
    if (status_result) {
        qInfo() << "Status command result:"
                << QJsonDocument(status_result.value()).toJson();
    }

    auto echo_result = plugin->execute_command(
        "echo", QJsonObject{{"message", "Test message"}});
    if (echo_result) {
        qInfo() << "Echo command result:"
                << QJsonDocument(echo_result.value()).toJson();
    }

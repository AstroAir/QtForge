...
int main(int argc, char** argv) {
    ...
    comp.set_description("Demonstrates composition data model and validation")
        ...
        .add_binding(CompositionBinding{"com.examples.primary", "produce", "com.examples.secondary", "consume"});

    auto valid = comp.validate();
    if (!valid) {
        qWarning() << "Composition validation failed:" << static_cast<int>(valid.error().code);
    }

    auto json = comp.to_json();
    qInfo() << "Composition JSON:" << QJsonDocument(json).toJson(QJsonDocument::Compact);

...
public slots:
    void run_tests() {
        ...
        qDebug() << "Get all config result:" << QJsonDocument(get_all_result).toJson(QJsonDocument::Compact);
        
        QJsonObject get_specific_result = plugin->execute_command("get_config", QJsonObject{{"key", "logging_enabled"}});
        qDebug() << "Get specific config result:" << QJsonDocument(get_specific_result).toJson(QJsonDocument::Compact);
        
        // Test 3: Set configuration
        qDebug() << "\n--- Test 3: Set Configuration ---";
        QJsonObject set_result = plugin->execute_command("set_config", QJsonObject{
            {"key", "test_setting"},
            {"value", "test_value"}
        });
        qDebug() << "Set config result:" << QJsonDocument(set_result).toJson(QJsonDocument::Compact);
        
        // Test 4: Validate configuration
        qDebug() << "\n--- Test 4: Validate Configuration ---";
        QJsonObject validate_result = plugin->execute_command("validate_config");
        qDebug() << "Validate config result:" << QJsonDocument(validate_result).toJson(QJsonDocument::Compact);
        
        // Test 5: Invalid configuration
        qDebug() << "\n--- Test 5: Invalid Configuration ---";
        QJsonObject invalid_set_result = plugin->execute_command("set_config", QJsonObject{
            {"key", "validation_interval"},
            {"value", -1000}  // Invalid value
        });
        qDebug() << "Invalid set config result:" << QJsonDocument(invalid_set_result).toJson(QJsonDocument::Compact);
        
        // Test 6: Save configuration
        qDebug() << "\n--- Test 6: Save Configuration ---";
        QJsonObject save_result = plugin->execute_command("save_config");
        qDebug() << "Save config result:" << QJsonDocument(save_result).toJson(QJsonDocument::Compact);
        
        // Test 7: Reload configuration
        qDebug() << "\n--- Test 7: Reload Configuration ---";
        QJsonObject reload_result = plugin->execute_command("reload_config");
        qDebug() << "Reload config result:" << QJsonDocument(reload_result).toJson(QJsonDocument::Compact);
        ...
        qDebug() << "\n--- Test 9: Complex Configuration Update ---";
        QJsonObject complex_set_result = plugin->execute_command("set_config", QJsonObject{
            {"key", "features"},
            {"value", QJsonObject{
                {"advanced_mode", true},
                {"debug_mode", true},
        ...
        qDebug() << "Complex set config result:" << QJsonDocument(complex_set_result).toJson(QJsonDocument::Compact);

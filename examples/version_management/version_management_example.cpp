...
public:
    VersionManagementExample() {
        ...
        std::cout << "=== QtForge Plugin Version Management Example ===" << std::endl;
    }
...
private:
    void demonstrateBasicVersionManagement() {
        ...
        for (const auto& version : versions) {
            ...
            std::filesystem::path plugin_file = createMockPluginFile(plugin_id, version);
            
            auto result = plugin_manager_->install_plugin_version(
                plugin_id, version, plugin_file);
            
            if (result) {
                std::cout << "✓ Installed " << plugin_id << " v" << version.to_string() << std::endl;
            } else {
                std::cout << "✗ Failed to install " << plugin_id << " v" << version.to_string() 
                         << ": " << result.error().message << std::endl;
            }
        }
        
        // List installed versions
        auto installed_versions = plugin_manager_->get_plugin_versions(plugin_id);
        ...
        // Get active version
        auto active_version = plugin_manager_->get_plugin_active_version(plugin_id);
        if (active_version) {
            std::cout << "Active version: v" << active_version->version.to_string() << std::endl;
        }
    }
    ...
    void demonstrateVersionMigration() {
        ...
        auto migration_result = version_manager.register_migration(
            ...
            [](const MigrationContext& context) -> qtplugin::expected<void, PluginError> {
                std::cout << "  Performing custom migration from " 
                         << context.from_version.to_string() 
                         << " to " << context.to_version.to_string() << std::endl;
                
                // Simulate migration work
                std::cout << "  - Converting configuration format..." << std::endl;
        ...
        if (migration_result) {
            std::cout << "✓ Registered custom migration for " << plugin_id << std::endl;
        }
        ...
        std::cout << "Migration available: " << (migration_available ? "Yes" : "No") << std::endl;
        ...
        if (migrate_result) {
            ...
            std::cout << "✗ Migration failed: " << migrate_result.error().message << std::endl;
        }
    }
    ...
    void demonstrateRollbackFeatures() {
        ...
        // Create backup
        auto backup_result = version_manager.create_backup(plugin_id, Version(1, 2, 0));
        if (backup_result) {
            std::cout << "✓ Created backup for v1.2.0" << std::endl;
            std::cout << "  Backup location: " << backup_result->backup_path.string() << std::endl;
        }
        
        // Get rollback points
        auto rollback_points = version_manager.get_rollback_points(plugin_id);
        std::cout << "Available rollback points: " << rollback_points.size() << std::endl;
        
        for (const auto& point : rollback_points) {
            std::cout << "  - v" << point.current_version.to_string() 
                     << " (backup: " << point.backup_time.time_since_epoch().count() << ")" << std::endl;
        }
        ...
        std::cout << "Cleaned up " << cleaned_count << " old backups" << std::endl;
    }
    ...
    void demonstrateCompatibilityManagement() {
        ...
        switch (compatibility) {
            case CompatibilityLevel::Breaking:
                std::cout << "Breaking (manual migration required)" << std::endl;
                break;
            case CompatibilityLevel::Major:
                std::cout << "Major (automatic migration possible)" << std::endl;
                break;
        ...
        auto compatible_versions = version_manager.get_compatible_versions(plugin_id, host_version);
        std::cout << "Compatible versions with host v" << host_version.to_string() << ":" << std::endl;
    ...
    void demonstrateVersionStatistics() {
        ...
        if (stats.contains("storage_size_bytes")) {
            auto storage_size = stats["storage_size_bytes"].toVariant().toLongLong();
            std::cout << "  Storage usage: " << storage_size << " bytes" << std::endl;
        }
        
        // Get storage usage for specific plugin
        auto storage_usage = version_manager.get_storage_usage("example.calculator");
    ...
    std::filesystem::path createMockPluginFile(const std::string& plugin_id, const Version& version) {
        // Create a temporary mock plugin file for demonstration
        auto temp_dir = std::filesystem::temp_directory_path() / "qtforge_example";
        std::filesystem::create_directories(temp_dir);
        
        auto plugin_file = temp_dir / (plugin_id + "_v" + version.to_string() + ".dll");

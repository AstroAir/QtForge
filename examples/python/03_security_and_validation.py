...
try:
    import qtforge
    from qtforge import security  # type: ignore
...
def demonstrate_security_manager() -> Any:  # type: ignore
    ...
    try:
        ...
        if hasattr(manager, "check_permissions") and hasattr(
            security, "PluginPermission"
        ):
        ...
        if hasattr(manager, "set_security_level") and hasattr(
        ...
        if hasattr(validator, "validate"):
            ...
            for test_file in test_files:
                try:
                    ...
                    print(
                        f"  📊 Validation result for {os.path.basename(test_file)}: {result}"
                    )
                except Exception as e:
                    print(
                        f"  ⚠️  Validation failed for {os.path.basename(test_file)}: {e}"
                    )
...
def demonstrate_signature_verifier() -> Any:  # type: ignore
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(
            mode="wb", delete=False, suffix=".sig"
        ) as sig_file:
        ...
        if hasattr(verifier, "verify_with_signature"):
            try:
                result = verifier.verify_with_signature(
                    content_file_path, sig_file_path
                )
            ...
            for perm_name in permissions_to_test:
                if hasattr(security.PluginPermission, perm_name):
                    ...
                    if hasattr(perm_manager, "has_permission"):
                        try:
                            has_perm = perm_manager.has_permission(
                                test_plugin, permission
                            )
                    ...
                    if hasattr(perm_manager, "has_permission"):
                        try:
                            ...
                            print(
                                f"    📊 Has {perm_name} after revocation: {has_perm}"
                            )
                        except Exception as e:
                            print(
                                f"    ⚠️  Failed to check {perm_name} after revocation: {e}"
                            )
    ...
    try:
        ...
        if hasattr(policy_engine, "evaluate_policy"):
            ...
            for plugin_name, action in test_scenarios:
                try:
                    ...
                    print(
                        f"  📊 Policy evaluation for {plugin_name}.{action}: {result}"
                    )
                except Exception as e:
                    print(
                        f"  ⚠️  Policy evaluation failed for {plugin_name}.{action}: {e}"
                    )
        ...
        if hasattr(policy_engine, "load_policy"):
            ...
            try:
                ...
                print(
                    f"  ✅ Successfully loaded policy from {os.path.basename(policy_file_path)}"
                )
            except Exception as e:
...
def demonstrate_security_context() -> Any:  # type: ignore
    ...
    if hasattr(security, "SecurityContext"):
        try:
            ...
            if (
                hasattr(context, "set_trust_level")
                and hasattr(security, "TrustLevel")
                and hasattr(security.TrustLevel, "Trusted")
            ):
            ...
            # Test permission operations
            if hasattr(context, "add_permission") and hasattr(
                security, "PluginPermission"
            ):
        ...
        if hasattr(security, "create_permission_manager"):
            ...
            permission_error_tests = [
                (
                    ...
                    lambda: perm_manager.grant_permission(
                        None, security.PluginPermission.FileSystemRead
                    )
                    if hasattr(security, "PluginPermission")
                    and hasattr(security.PluginPermission, "FileSystemRead")
                    else None,
                ),
            ]
            ...
            for test_name, test_func in permission_error_tests:
                if test_func is not None:
                    try:
                        ...
                        print(
                            f"  ✅ {test_name}: Correctly caught {type(e).__name__}: {e}"
                        )

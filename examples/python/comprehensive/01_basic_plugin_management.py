...
try:
    ...
    print(
        f"QtForge version: {qtforge.__version__ if hasattr(qtforge, '__version__') else 'Unknown'}"
    )
        ...
        if hasattr(core, "PluginLifecycleConfig"):
            try:
                ...
                if hasattr(config, "initialization_timeout"):
                    ...
                    print(
                        f"Set initialization timeout: {config.initialization_timeout}ms"
                    )

                if hasattr(config, "enable_health_monitoring"):
                    config.enable_health_monitoring = True
                    print(
                        f"Health monitoring enabled: {config.enable_health_monitoring}"
                    )
    ...
    def demonstrate_error_handling(self) -> None:
        ...
        error_tests = [
            (
                ...
                lambda: self.manager.load_plugin("/non/existent/path.so")
                if self.manager and hasattr(self.manager, "load_plugin")
                else None,
            ),
            (
                "Getting non-existent plugin",
                lambda: self.manager.get_plugin("non_existent")
                if self.manager and hasattr(self.manager, "get_plugin")
                else None,
            ),
            (
                "Unloading non-existent plugin",
                lambda: self.manager.unload_plugin("non_existent")
                if self.manager and hasattr(self.manager, "unload_plugin")
                else None,
            ),
        ]

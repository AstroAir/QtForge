...
class CommunicationExample:
    ...
    def demonstrate_message_creation(self) -> List[Any]:
        ...
        if hasattr(comm, "create_message"):
            try:
                message2 = comm.create_message(
                    "factory.topic", {"data": "from_factory"}
                )
    ...
    def demonstrate_message_publishing(self) -> None:
        ...
        test_messages = [
            ...
            {
                "topic": "test.event",
                "payload": {"event": "user_action", "timestamp": time.time()},
            },
        ]
            ...
            try:
                ...
                if hasattr(comm, "create_message"):
                    message = comm.create_message(
                        msg_data["topic"], msg_data["payload"]
                    )
    ...
    def demonstrate_error_handling(self) -> None:
        ...
        error_tests = [
            (
                "Publishing null message",
                lambda: self.message_bus.publish(None)
                if self.message_bus and hasattr(self.message_bus, "publish")
                else None,
            ),
            (
                "Subscribing with null callback",
                lambda: self.message_bus.subscribe("test", None)
                if self.message_bus and hasattr(self.message_bus, "subscribe")
                else None,
            ),
            (
                "Unsubscribing non-existent topic",
                lambda: self.message_bus.unsubscribe("non.existent")

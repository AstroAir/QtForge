...
try:
    import qtforge
    from qtforge import orchestration
...
def demonstrate_workflow_creation() -> Any:  # type: ignore
    ...
    try:
        ...
        if hasattr(workflow, "set_execution_mode") and hasattr(
...
def demonstrate_workflow_steps() -> Any:  # type: ignore
    ...
    if len(steps) >= 2 and hasattr(steps[1], "add_dependency"):
        ...
        for i in range(1, len(steps)):
            try:
                steps[i].add_dependency(
                    steps[i - 1].id if hasattr(steps[i - 1], "id") else f"step_{i - 1}"
                )  # type: ignore
                print(
                    f"  ✅ Added dependency: {steps[i].name if hasattr(steps[i], 'name') else f'step_{i}'} depends on {steps[i - 1].name if hasattr(steps[i - 1], 'name') else f'step_{i - 1}'}"
                )  # type: ignore
    ...
    if workflow and steps and hasattr(workflow, "add_step"):
        ...
        for step in steps:
            try:
                ...
                print(
                    f"  ✅ Added step to workflow: {step.name if hasattr(step, 'name') else 'unnamed'}"
                )
            except Exception as e:
        ...
        if hasattr(workflow, "get_steps"):
            try:
                ...
                print(
                    f"  📊 Total steps in workflow: {len(workflow_steps) if workflow_steps else 0}"
                )
            except Exception as e:
            ...
            if hasattr(orchestration.ExecutionMode, mode_name):
                ...
                if hasattr(orchestrator, "execute_workflow"):
                    try:
                        ...
                        print(
                            f"  ⚠️  Workflow execution failed: {e} (expected if plugins not available)"
                        )
...
def demonstrate_step_results() -> Any:  # type: ignore
    ...
    try:
        ...
        failure_result = orchestration.StepResult(
            False, "Step failed due to invalid input"
        )
    ...
    if hasattr(orchestration, "Workflow") and hasattr(orchestration, "WorkflowStep"):
        ...
        for i in range(3):
            step = orchestration.WorkflowStep(
                f"step_{i}", f"Step {i + 1}", f"plugin_{i}"
            )
            steps.append(step)
        ...
        if steps and hasattr(orchestration, "StepStatus"):
            ...
            for step in steps[:2]:  # Test first 2 steps
                for status_name in status_progression:
                    if hasattr(orchestration, "StepStatus") and hasattr(
                        orchestration.StepStatus, status_name
                    ):  # type: ignore
                        ...
                        if hasattr(step, "set_status"):
                            try:
                                ...
                                print(
                                    f"  ✅ Set {step.name if hasattr(step, 'name') else 'step'} status to {status_name}"
                                )
...
def demonstrate_error_handling() -> None:
    ...
    try:
        ...
        try:
            ...
            print(
                f"  ✅ Correctly caught error for None workflow: {type(e).__name__}: {e}"
            )
        ...
        if hasattr(orchestration, "WorkflowStep"):
            try:
                ...
                print(
                    f"  ✅ Correctly caught error for None step ID: {type(e).__name__}: {e}"
                )
            ...
            try:
                ...
                print(
                    f"  ✅ Correctly caught error for None step name: {type(e).__name__}: {e}"
                )

        # Test circular dependency detection
        if hasattr(orchestration, "Workflow") and hasattr(

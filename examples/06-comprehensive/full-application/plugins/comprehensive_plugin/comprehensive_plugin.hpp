...
public:
    ...
    handle_service_request(const QString& method,
                           const QJsonObject& params) override;

public slots:
    // Communication slots
    void onMessageReceived(const QString& topic, const QJsonObject& message);
    void onServiceRequest(const QString& requestId, const QString& method,
...
signals:
    ...
    void requestProcessed(const QString& requestId,
...
private:
    ...
    std::atomic<qtplugin::ServiceStatus> m_serviceStatus{
        qtplugin::ServiceStatus::Stopped};
...
public:
    explicit NetworkHandler(QNetworkAccessManager* manager,

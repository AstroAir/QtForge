# QtForgeCompiler.cmake
# Compiler detection and configuration module for QtForge
...
# Supported compilers
set(QTFORGE_SUPPORTED_COMPILERS
    msvc
    gcc
    clang
    intel
...
function(qtforge_detect_compiler)
    ...
    set(QTFORGE_IS_MSVC FALSE PARENT_SCOPE)
    set(QTFORGE_IS_GCC FALSE PARENT_SCOPE)
    set(QTFORGE_IS_CLANG FALSE PARENT_SCOPE)
    set(QTFORGE_IS_INTEL FALSE PARENT_SCOPE)
    set(QTFORGE_IS_MINGW FALSE PARENT_SCOPE)
    ...
    elseif(CMAKE_CXX_COMPILER_ID STREQUAL "GNU")
        if(MINGW OR QTFORGE_IS_MSYS2)
            ...
            set(QTFORGE_IS_MINGW TRUE PARENT_SCOPE)

            # Detect specific MSYS2 MinGW variant
            if(QTFORGE_IS_MSYS2)
                if(QTFORGE_IS_MINGW64)
                    set(QTFORGE_MINGW_VARIANT "mingw64" PARENT_SCOPE)
                elseif(QTFORGE_IS_MINGW32)
                    set(QTFORGE_MINGW_VARIANT "mingw32" PARENT_SCOPE)
                elseif(QTFORGE_IS_UCRT64)
                    set(QTFORGE_MINGW_VARIANT "ucrt64" PARENT_SCOPE)
                endif()
            endif()
        else()
            set(QTFORGE_COMPILER_NAME "gcc" PARENT_SCOPE)
    ...
    elseif(CMAKE_CXX_COMPILER_ID STREQUAL "Clang")
        ...
        if(QTFORGE_IS_MSYS2)
            if(QTFORGE_IS_CLANG64)
                set(QTFORGE_CLANG_VARIANT "clang64" PARENT_SCOPE)
            elseif(QTFORGE_IS_CLANG32)
                set(QTFORGE_CLANG_VARIANT "clang32" PARENT_SCOPE)
            endif()
        endif()
...
function(qtforge_configure_compiler)
    cmake_parse_arguments(QTFORGE_COMPILER
    ...
    if(QTFORGE_IS_MSVC)
        qtforge_configure_msvc(
            ENABLE_WARNINGS ${QTFORGE_COMPILER_ENABLE_WARNINGS}
            ENABLE_WERROR ${QTFORGE_COMPILER_ENABLE_WERROR}
            ENABLE_LTO ${QTFORGE_COMPILER_ENABLE_LTO}
            ENABLE_FAST_MATH ${QTFORGE_COMPILER_ENABLE_FAST_MATH}
        )
    endif()
    ...
    if(QTFORGE_IS_GCC OR QTFORGE_IS_MINGW)
        qtforge_configure_gcc(
            ENABLE_WARNINGS ${QTFORGE_COMPILER_ENABLE_WARNINGS}
            ENABLE_WERROR ${QTFORGE_COMPILER_ENABLE_WERROR}
            ENABLE_SANITIZERS ${QTFORGE_COMPILER_ENABLE_SANITIZERS}
            ENABLE_LTO ${QTFORGE_COMPILER_ENABLE_LTO}
            ENABLE_FAST_MATH ${QTFORGE_COMPILER_ENABLE_FAST_MATH}
        )
    endif()
...
function(qtforge_configure_msvc)
    cmake_parse_arguments(MSVC
        "ENABLE_WARNINGS;ENABLE_WERROR;ENABLE_LTO;ENABLE_FAST_MATH"
        ""
        ""
        ${ARGN}
    )
    ...
    message(STATUS "QtForge: Configured MSVC compiler settings")
endfunction()
...
function(qtforge_configure_gcc)
    ...
    message(STATUS "QtForge: Configured GCC compiler settings")
endfunction()
...
function(qtforge_configure_clang)
    ...
    message(STATUS "QtForge: Configured Clang compiler settings")
endfunction()
...
function(qtforge_configure_intel)
    ...
    message(STATUS "QtForge: Configured Intel compiler settings")
endfunction()

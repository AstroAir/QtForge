# QtForgeTargets.cmake
# Target creation and configuration utilities for QtForge
...
function(qtforge_add_library)
    ...
    if(QTFORGE_BUILD_SHARED)
        ...
        qtforge_configure_library_target(${SHARED_TARGET_NAME}
            PUBLIC_HEADERS ${LIB_PUBLIC_HEADERS}
            QT_COMPONENTS ${LIB_QT_COMPONENTS}
    ...
    if(QTFORGE_BUILD_STATIC)
        ...
        set_target_properties(${STATIC_TARGET_NAME} PROPERTIES
            EXPORT_NAME ${LIB_EXPORT_NAME}Static
            OUTPUT_NAME ${LIB_OUTPUT_NAME}-static
            AUTOMOC ON
        )

        # Configure the static target
        qtforge_configure_library_target(${STATIC_TARGET_NAME}
            PUBLIC_HEADERS ${LIB_PUBLIC_HEADERS}
            QT_COMPONENTS ${LIB_QT_COMPONENTS}
            DEPENDENCIES ${LIB_DEPENDENCIES}
            DEFINITIONS ${LIB_DEFINITIONS}
...
function(qtforge_add_plugin)
    cmake_parse_arguments(PLUGIN
        ""
        "NAME;METADATA"
        "SOURCES;HEADERS;QT_COMPONENTS;DEPENDENCIES"
        ${ARGN}
    )
    ...
    qtforge_configure_library_target(${PLUGIN_NAME}
        QT_COMPONENTS ${PLUGIN_QT_COMPONENTS}
        DEPENDENCIES ${PLUGIN_DEPENDENCIES}
    )

    # Add plugin-specific definitions
    target_compile_definitions(${PLUGIN_NAME} PRIVATE
        QTFORGE_PLUGIN_NAME="${PLUGIN_NAME}"
    )

    # Copy metadata file if provided
    if(PLUGIN_METADATA AND EXISTS "${CMAKE_CURRENT_SOURCE_DIR}/${PLUGIN_METADATA}")
        configure_file(
            "${CMAKE_CURRENT_SOURCE_DIR}/${PLUGIN_METADATA}"
            "${CMAKE_CURRENT_BINARY_DIR}/${PLUGIN_NAME}.json"
            COPYONLY
        )
    endif()
endfunction()
...
function(qtforge_configure_ios_target TARGET_NAME)
    ...
    set_target_properties(${TARGET_NAME} PROPERTIES
        XCODE_ATTRIBUTE_IPHONEOS_DEPLOYMENT_TARGET ${QTFORGE_IOS_DEPLOYMENT_TARGET}
    )
...
function(qtforge_install_targets)
    ...
    install(TARGETS ${INSTALL_TARGETS}
        ...
        LIBRARY DESTINATION ${QTFORGE_INSTALL_LIBRARY_DIR}
            COMPONENT ${INSTALL_COMPONENT}
        ARCHIVE DESTINATION ${QTFORGE_INSTALL_ARCHIVE_DIR}

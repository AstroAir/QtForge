# QtForgeDependencies.cmake Dependency management module for QtForge Provides
# centralized dependency detection and configuration
...
function(qtforge_find_qt)
  cmake_parse_arguments(QT "REQUIRED" "" "COMPONENTS;OPTIONAL_COMPONENTS"
                        ${ARGN})
  ...
  if(Qt6_FOUND)
    set(QTFORGE_QT_FOUND
        TRUE
        PARENT_SCOPE)
    set(QTFORGE_QT_VERSION
        "${Qt6_VERSION}"
        PARENT_SCOPE)
    set(QTFORGE_QT_MAJOR_VERSION
        6
        PARENT_SCOPE)
...
function(qtforge_find_development_dependencies)
  ...
  if(QTFORGE_BUILD_DOCS)
    ...
    if(NOT DOXYGEN_FOUND)
      message(
        WARNING
          "QtForge: Doxygen not found, documentation will not be generated")
    endif()
  endif()
  ...
  if(QTFORGE_BUILD_BENCHMARKS)
    ...
    if(NOT benchmark_FOUND)
      message(
        WARNING
          "QtForge: Google Benchmark not found, benchmarks will not be built")
    endif()
  endif()
endfunction()
...
function(qtforge_find_python_dependencies)
  ...
  if(Python_FOUND)
    ...
    if(Python_VERSION VERSION_LESS ${QTFORGE_PYTHON_MIN_VERSION})
      message(
        WARNING
          "QtForge: Python ${Python_VERSION} is below minimum required version ${QTFORGE_PYTHON_MIN_VERSION}"
      )
      set(QTFORGE_PYTHON_FOUND
          FALSE
          PARENT_SCOPE)
      return()
    endif()
    ...
    if(Python_VERSION VERSION_GREATER ${QTFORGE_PYTHON_MAX_VERSION})
      message(
        WARNING
          "QtForge: Python ${Python_VERSION} is above maximum supported version ${QTFORGE_PYTHON_MAX_VERSION}"
      )
    endif()
    ...
    set(QTFORGE_PYTHON_FOUND
        TRUE
        PARENT_SCOPE)
    set(QTFORGE_PYTHON_VERSION
        ${Python_VERSION}
        PARENT_SCOPE)
    set(QTFORGE_PYTHON_EXECUTABLE
        ${Python_EXECUTABLE}
        PARENT_SCOPE)
    ...
    if(pybind11_FOUND)
      set(QTFORGE_PYBIND11_FOUND
          TRUE
          PARENT_SCOPE)
    else()
      message(
        WARNING "QtForge: pybind11 not found, Python bindings will not be built"
      )
      set(QTFORGE_PYBIND11_FOUND
          FALSE
          PARENT_SCOPE)
    ...
    message(
      WARNING
        "QtForge: Python interpreter not found, Python bindings will not be built"
    )
    set(QTFORGE_PYTHON_FOUND
        FALSE
        PARENT_SCOPE)
...
function(qtforge_find_lua_dependencies)
  ...
  if(Lua_FOUND)
    ...
    if(LUA_VERSION_STRING VERSION_LESS ${QTFORGE_LUA_MIN_VERSION})
      message(
        WARNING
          "QtForge: Lua ${LUA_VERSION_STRING} is below minimum required version ${QTFORGE_LUA_MIN_VERSION}"
      )
      set(QTFORGE_LUA_FOUND
          FALSE
          PARENT_SCOPE)
      return()
    endif()
    ...
    if(LUA_VERSION_STRING VERSION_GREATER ${QTFORGE_LUA_MAX_VERSION})
      message(
        WARNING
          "QtForge: Lua ${LUA_VERSION_STRING} is above maximum supported version ${QTFORGE_LUA_MAX_VERSION}"
      )
    endif()
    ...
    if(sol2_FOUND)
      set(QTFORGE_SOL2_FOUND
          TRUE
          PARENT_SCOPE)
      set(QTFORGE_LUA_FOUND
          TRUE
          PARENT_SCOPE)
      set(QTFORGE_LUA_VERSION
          ${LUA_VERSION_STRING}
          PARENT_SCOPE)
      set(QTFORGE_LUA_INCLUDE_DIR
          ${LUA_INCLUDE_DIR}
          PARENT_SCOPE)
      set(QTFORGE_LUA_LIBRARIES
          ${LUA_LIBRARIES}
          PARENT_SCOPE)
    else()
      ...
      find_path(
        SOL2_INCLUDE_DIR
        NAMES sol/sol.hpp
        PATHS ${CMAKE_CURRENT_SOURCE_DIR}/third_party/sol2/include
              ${CMAKE_CURRENT_SOURCE_DIR}/external/sol2/include /usr/include
              /usr/local/include ${CMAKE_INSTALL_PREFIX}/include
        PATH_SUFFIXES sol2)
      ...
      if(SOL2_INCLUDE_DIR)
        ...
        set(QTFORGE_LUA_LIBRARIES
            ${LUA_LIBRARIES}
            PARENT_SCOPE)
        set(QTFORGE_SOL2_INCLUDE_DIR
            ${SOL2_INCLUDE_DIR}
            PARENT_SCOPE)
      else()
        message(
          WARNING "QtForge: sol2 not found, Lua bindings will not be built")
        set(QTFORGE_LUA_FOUND
            FALSE
            PARENT_SCOPE)
        set(QTFORGE_SOL2_FOUND
    ...
    message(
      WARNING
        "QtForge: Lua interpreter not found, Lua bindings will not be built")
    set(QTFORGE_LUA_FOUND
        FALSE
        PARENT_SCOPE)
    set(QTFORGE_SOL2_FOUND
...
function(qtforge_setup_dependencies)
  ...
  qtforge_find_qt(REQUIRED COMPONENTS ${QT_REQUIRED_COMPONENTS}
  ...
  set(QTFORGE_QT_VERSION
      ...
      PARENT_SCOPE)
  set(QTFORGE_QT_MAJOR_VERSION
      ${QTFORGE_QT_MAJOR_VERSION}
      PARENT_SCOPE)
  set(QT_VERSION_MAJOR
      ${QT_VERSION_MAJOR}
      PARENT_SCOPE)
  set(QTFORGE_QT_COMPONENTS_FOUND
      ${QTFORGE_QT_COMPONENTS_FOUND}
      PARENT_SCOPE)
  ...
  set(QTFORGE_HAS_SQL
      ...
      PARENT_SCOPE)
  set(QTFORGE_HAS_CONCURRENT
      ${QTFORGE_HAS_CONCURRENT}
      PARENT_SCOPE)
  set(QTFORGE_HAS_STATEMACHINE
      ${QTFORGE_HAS_STATEMACHINE}
      PARENT_SCOPE)

  # Propagate Python variables to parent scope
  set(QTFORGE_PYTHON_FOUND
      ${QTFORGE_PYTHON_FOUND}
      PARENT_SCOPE)
  set(QTFORGE_PYBIND11_FOUND
      ${QTFORGE_PYBIND11_FOUND}
      PARENT_SCOPE)
  set(QTFORGE_PYTHON_VERSION
      ${QTFORGE_PYTHON_VERSION}
      PARENT_SCOPE)
  set(QTFORGE_PYTHON_EXECUTABLE
      ${QTFORGE_PYTHON_EXECUTABLE}
      PARENT_SCOPE)
...
function(qtforge_link_qt_libraries TARGET_NAME)
  cmake_parse_arguments(LINK "" "" "COMPONENTS" ${ARGN})
  ...
  foreach(COMPONENT ${LINK_COMPONENTS})
    if(TARGET ${QT_PREFIX}::${COMPONENT})
      ...
      message(
        WARNING "QtForge: Qt component ${COMPONENT} not available for linking")
    endif()
  endforeach()
endfunction()

# Note: qtforge_setup_dependencies() should be called explicitly from main
# CMakeLists.txt after all options are configured, not automatically when this
# module is included

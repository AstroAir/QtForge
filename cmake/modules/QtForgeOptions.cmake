# QtForgeOptions.cmake Build options and feature flags module for QtForge
...
function(qtforge_define_options)
  ...
  cmake_dependent_option(
    QTFORGE_SANDBOX_PROCESS_ISOLATION
    "Enable process isolation for sandboxed plugins" ON
    "QTFORGE_ENABLE_SANDBOX" OFF)
  cmake_dependent_option(
    QTFORGE_SANDBOX_RESOURCE_MONITORING
    "Enable resource monitoring for sandboxed plugins" ON
    "QTFORGE_ENABLE_SANDBOX" OFF)
  cmake_dependent_option(
    QTFORGE_SANDBOX_SECURITY_ENFORCEMENT
    "Enable security policy enforcement for sandboxed plugins" ON
    "QTFORGE_ENABLE_SANDBOX" OFF)
  ...
  option(QTFORGE_BUILD_PYTHON_BINDINGS "Build Python bindings using pybind11"
         ON)
  ...
  option(QTFORGE_PYTHON_ENABLE_ALL_MODULES "Enable all Python binding modules"
         ON)
  option(QTFORGE_PYTHON_ENABLE_TYPE_STUBS
         "Generate Python type stubs (.pyi files)" ON)

  # Individual module controls for progressive enablement
  option(QTFORGE_PYTHON_ENABLE_SECURITY_MODULE
         "Enable security module bindings" OFF)
  option(QTFORGE_PYTHON_ENABLE_MANAGERS_MODULE
         "Enable managers module bindings" ON)
  option(QTFORGE_PYTHON_ENABLE_COMMUNICATION_MODULE
         "Enable communication module bindings" ON)
  option(QTFORGE_PYTHON_ENABLE_ORCHESTRATION_MODULE
         "Enable orchestration module bindings" ON)
  # Temporarily disabled due to compilation issues - see
  # BUILD_OPTIMIZATION_REPORT.md
  option(QTFORGE_PYTHON_ENABLE_MONITORING_MODULE
         "Enable monitoring module bindings" OFF)
  option(QTFORGE_PYTHON_ENABLE_THREADING_MODULE
         "Enable threading module bindings" OFF)
  option(QTFORGE_PYTHON_ENABLE_TRANSACTIONS_MODULE
         "Enable transactions module bindings" OFF)
  option(QTFORGE_PYTHON_ENABLE_COMPOSITION_MODULE
         "Enable composition module bindings" ON)
  ...
  option(QTFORGE_ENABLE_COMPONENT_LOGGING "Enable detailed component logging"
         ON)
  option(QTFORGE_ENABLE_PLUGIN_PROFILING "Enable plugin performance profiling"
         ON)
  option(QTFORGE_ENABLE_SECURITY_HARDENING "Enable security hardening features"
         ON)

  # Compiler options
  option(QTFORGE_ENABLE_WARNINGS "Enable comprehensive compiler warnings" ON)
  option(QTFORGE_ENABLE_WERROR "Treat warnings as errors" OFF) # Keep OFF to
                                                               # avoid build
                                                               # failures
  option(QTFORGE_ENABLE_SANITIZERS "Enable sanitizers in debug builds" OFF
  )# Disable for Release builds
  option(QTFORGE_ENABLE_LTO "Enable Link Time Optimization" OFF) # Disable to
                                                                 # avoid build
                                                                 # issues
  option(QTFORGE_ENABLE_FAST_MATH "Enable fast math optimizations" OFF
  )# Disable to avoid precision issues

  # Platform-specific options
  if(QTFORGE_IS_WINDOWS)
    option(QTFORGE_ENABLE_WINDOWS_MANIFEST
           "Enable Windows application manifest" ON)
  ...
  if(QTFORGE_IS_ANDROID)
    ...
    set(QTFORGE_ANDROID_MIN_SDK_VERSION
        "21"
        CACHE STRING "Minimum Android SDK version")
    set(QTFORGE_ANDROID_TARGET_SDK_VERSION
        "33"
        CACHE STRING "Target Android SDK version")
  endif()

  if(QTFORGE_IS_IOS)
    option(QTFORGE_ENABLE_IOS_SIMULATOR "Build for iOS Simulator" OFF)
    set(QTFORGE_IOS_DEPLOYMENT_TARGET
        "12.0"
        CACHE STRING "iOS deployment target")
  endif()

  # Cross-compilation options
  option(QTFORGE_ENABLE_CROSS_COMPILATION "Enable cross-compilation support"
         OFF)
  ...
  set(QTFORGE_INSTALL_RUNTIME_DIR
      "bin"
      CACHE STRING "Runtime installation directory")
  set(QTFORGE_INSTALL_LIBRARY_DIR
      "lib"
      CACHE STRING "Library installation directory")
  set(QTFORGE_INSTALL_ARCHIVE_DIR
      "lib"
      CACHE STRING "Archive installation directory")
  set(QTFORGE_INSTALL_INCLUDE_DIR
      "include"
      CACHE STRING "Header installation directory")
  set(QTFORGE_INSTALL_CMAKE_DIR
      "lib/cmake/QtForge"
      CACHE STRING "CMake files installation directory")
  set(QTFORGE_INSTALL_PKGCONFIG_DIR
      "lib/pkgconfig"
      CACHE STRING "pkg-config files installation directory")
  set(QTFORGE_INSTALL_DOC_DIR
      "share/doc/qtforge"
      CACHE STRING "Documentation installation directory")

  # Python binding installation options
  if(QTFORGE_BUILD_PYTHON_BINDINGS)
    set(QTFORGE_PYTHON_INSTALL_DIR
        ""
        CACHE STRING
              "Python module installation directory (auto-detected if empty)")
    set(QTFORGE_PYTHON_MIN_VERSION
        "3.8"
        CACHE STRING "Minimum required Python version")
    set(QTFORGE_PYTHON_MAX_VERSION
        "3.13"
        CACHE STRING "Maximum supported Python version")
  endif()

  # Lua binding installation options
  if(QTFORGE_BUILD_LUA_BINDINGS)
    set(QTFORGE_LUA_INSTALL_DIR
        ""
        CACHE STRING
              "Lua module installation directory (auto-detected if empty)")
    set(QTFORGE_LUA_MIN_VERSION
        "5.4"
        CACHE STRING "Minimum required Lua version")
    set(QTFORGE_LUA_MAX_VERSION
        "5.4"
        CACHE STRING "Maximum supported Lua version")
    set(QTFORGE_SOL2_MIN_VERSION
        "3.3.0"
        CACHE STRING "Minimum required sol2 version")
  endif()

  # Configuration summary function available but not called automatically
endfunction()
...
function(qtforge_validate_options)
  ...
  if(NOT QTFORGE_BUILD_SHARED AND NOT QTFORGE_BUILD_STATIC)
    message(
      FATAL_ERROR
        "QtForge: At least one of QTFORGE_BUILD_SHARED or QTFORGE_BUILD_STATIC must be enabled"
    )
  endif()
  ...
  if(QTFORGE_BUILD_UI AND NOT QTFORGE_BUILD_NETWORK)
    message(
      WARNING
        "QtForge: UI components may require network support for some features")
  endif()

  # Validate test options
  if(QTFORGE_BUILD_COMPONENT_TESTS AND NOT QTFORGE_BUILD_TESTS)
    set(QTFORGE_BUILD_TESTS
        ON
        PARENT_SCOPE)
  endif()
  ...
  if(QTFORGE_IS_ANDROID AND QTFORGE_BUILD_SHARED)
    # Android builds typically use static libraries
  endif()

  if(QTFORGE_IS_IOS AND QTFORGE_BUILD_SHARED)
    message(
      WARNING
        "QtForge: iOS does not support shared libraries, forcing static build")
    set(QTFORGE_BUILD_SHARED
        OFF
        PARENT_SCOPE)
    set(QTFORGE_BUILD_STATIC
        ON
        PARENT_SCOPE)
  endif()
...
function(qtforge_print_configuration_summary)
  ...
  message(
    STATUS "Compiler: ${QTFORGE_COMPILER_NAME} ${QTFORGE_COMPILER_VERSION}")
  ...
  if(QTFORGE_ENABLE_SANDBOX)
    ...
    message(
      STATUS "  Resource Monitoring: ${QTFORGE_SANDBOX_RESOURCE_MONITORING}")
    message(
      STATUS "  Security Enforcement: ${QTFORGE_SANDBOX_SECURITY_ENFORCEMENT}")
  endif()
  message(STATUS "")
  ...
  if(QTFORGE_IS_ANDROID)
    ...
    message(
      STATUS "  Target SDK Version: ${QTFORGE_ANDROID_TARGET_SDK_VERSION}")
    message(STATUS "")
  endif()
  ...
  message(
    STATUS "  Runtime: ${CMAKE_INSTALL_PREFIX}/${QTFORGE_INSTALL_RUNTIME_DIR}")
  message(
    STATUS "  Libraries: ${CMAKE_INSTALL_PREFIX}/${QTFORGE_INSTALL_LIBRARY_DIR}"
  )
  message(
    STATUS "  Headers: ${CMAKE_INSTALL_PREFIX}/${QTFORGE_INSTALL_INCLUDE_DIR}")
  message(
    STATUS "  CMake: ${CMAKE_INSTALL_PREFIX}/${QTFORGE_INSTALL_CMAKE_DIR}")
  message(STATUS "")
endfunction()

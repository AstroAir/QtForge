...
public:
     ...
     */
    virtual qtplugin::expected<void, PluginError> write(
        const std::string& formatted_message, const LogEntry& entry) = 0;
...
public:
     ...
     * @return Success or error information
     */
    virtual qtplugin::expected<void, PluginError> log(
     ...
     * @return Success or error information
     */
    virtual qtplugin::expected<void, PluginError> log_with_location(
        LogLevel level, std::string_view category, std::string_view message,
    ...
    virtual qtplugin::expected<void, PluginError> trace(
        std::string_view category, std::string_view message,
        std::string_view plugin_id = {}, const QJsonObject& context = {}) = 0;
    virtual qtplugin::expected<void, PluginError> debug(
        std::string_view category, std::string_view message,
        std::string_view plugin_id = {}, const QJsonObject& context = {}) = 0;
    virtual qtplugin::expected<void, PluginError> info(
        std::string_view category, std::string_view message,
        std::string_view plugin_id = {}, const QJsonObject& context = {}) = 0;
    virtual qtplugin::expected<void, PluginError> warning(
        std::string_view category, std::string_view message,
        std::string_view plugin_id = {}, const QJsonObject& context = {}) = 0;
    virtual qtplugin::expected<void, PluginError> error(
        std::string_view category, std::string_view message,
        std::string_view plugin_id = {}, const QJsonObject& context = {}) = 0;
    virtual qtplugin::expected<void, PluginError> critical(
        std::string_view category, std::string_view message,
        std::string_view plugin_id = {}, const QJsonObject& context = {}) = 0;
    virtual qtplugin::expected<void, PluginError> fatal(
        std::string_view category, std::string_view message,
        std::string_view plugin_id = {}, const QJsonObject& context = {}) = 0;
     ...
     * @return Success or error information
     */
    virtual qtplugin::expected<void, PluginError> set_configuration(
        const LoggingConfiguration& config) = 0;
     ...
     * @param level Log level
     */
    virtual void set_category_level(std::string_view category,
                                    LogLevel level) = 0;
     ...
     * @param level Log level
     */
    virtual void set_plugin_level(std::string_view plugin_id,
                                  LogLevel level) = 0;
     ...
     */
    virtual LogLevel get_effective_level(
        std::string_view category, std::string_view plugin_id = {}) const = 0;
     ...
     * @return Success or error information
     */
    virtual qtplugin::expected<void, PluginError> add_output_handler(
        LogOutput output_type, std::unique_ptr<ILogOutputHandler> handler) = 0;
     ...
     * @return Success or error information
     */
    virtual qtplugin::expected<void, PluginError> remove_output_handler(
        LogOutput output_type) = 0;
     ...
     * @return Success or error information
     */
    virtual qtplugin::expected<void, PluginError> remove_filter(
     ...
     */
    virtual std::vector<LogEntry> get_recent_entries(
        size_t count,
        std::optional<LogLevel> level_filter = std::nullopt) const = 0;
};
...
#define QTPLUGIN_LOG_MSG(manager, level, category, message, plugin_id,    \
                         context)                                         \
    (manager).log_with_location((level), (category), (message), __FILE__, \
                                __LINE__, __FUNCTION__, (plugin_id),      \
                                (context))

#define QTPLUGIN_LOG_TRACE(manager, category, message, plugin_id, context)  \
    QTPLUGIN_LOG_MSG(manager, qtplugin::LogLevel::Trace, category, message, \
                     plugin_id, context)

#define QTPLUGIN_LOG_DEBUG(manager, category, message, plugin_id, context)  \
    QTPLUGIN_LOG_MSG(manager, qtplugin::LogLevel::Debug, category, message, \
                     plugin_id, context)

#define QTPLUGIN_LOG_INFO(manager, category, message, plugin_id, context)  \
    QTPLUGIN_LOG_MSG(manager, qtplugin::LogLevel::Info, category, message, \
                     plugin_id, context)

#define QTPLUGIN_LOG_WARNING(manager, category, message, plugin_id, context)  \
    QTPLUGIN_LOG_MSG(manager, qtplugin::LogLevel::Warning, category, message, \
                     plugin_id, context)

#define QTPLUGIN_LOG_ERROR(manager, category, message, plugin_id, context)  \
    QTPLUGIN_LOG_MSG(manager, qtplugin::LogLevel::Error, category, message, \
                     plugin_id, context)

#define QTPLUGIN_LOG_CRITICAL(manager, category, message, plugin_id, context)  \
    QTPLUGIN_LOG_MSG(manager, qtplugin::LogLevel::Critical, category, message, \
                     plugin_id, context)

#define QTPLUGIN_LOG_FATAL(manager, category, message, plugin_id, context)  \
    QTPLUGIN_LOG_MSG(manager, qtplugin::LogLevel::Fatal, category, message, \
                     plugin_id, context)

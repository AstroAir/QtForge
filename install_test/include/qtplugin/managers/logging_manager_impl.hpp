...
public:
    qtplugin::expected<void, PluginError> write(
        const std::string& formatted_message, const LogEntry& entry) override;
...
public:
    ...
    bool should_log(const LogEntry& entry) const override {
        return m_allowed_plugins.empty() ||
               m_allowed_plugins.find(entry.plugin_id) !=
                   m_allowed_plugins.end();
    }

    std::string name() const override { return "plugin"; }

    void add_plugin(const std::string& plugin_id) {
        m_allowed_plugins.insert(plugin_id);
    }
    void remove_plugin(const std::string& plugin_id) {
        m_allowed_plugins.erase(plugin_id);
    }
...
public:
    ...
    // Convenience methods
    qtplugin::expected<void, PluginError> trace(
        std::string_view category, std::string_view message,
        std::string_view plugin_id = {},
        const QJsonObject& context = {}) override;
    qtplugin::expected<void, PluginError> debug(
        std::string_view category, std::string_view message,
        std::string_view plugin_id = {},
        const QJsonObject& context = {}) override;
    qtplugin::expected<void, PluginError> info(
        std::string_view category, std::string_view message,
        std::string_view plugin_id = {},
        const QJsonObject& context = {}) override;
    qtplugin::expected<void, PluginError> warning(
        std::string_view category, std::string_view message,
        std::string_view plugin_id = {},
        const QJsonObject& context = {}) override;
    qtplugin::expected<void, PluginError> error(
        std::string_view category, std::string_view message,
        std::string_view plugin_id = {},
        const QJsonObject& context = {}) override;
    qtplugin::expected<void, PluginError> critical(
        std::string_view category, std::string_view message,
        std::string_view plugin_id = {},
        const QJsonObject& context = {}) override;
    qtplugin::expected<void, PluginError> fatal(
        std::string_view category, std::string_view message,
        std::string_view plugin_id = {},
        const QJsonObject& context = {}) override;
    ...
    qtplugin::expected<void, PluginError> set_configuration(
        const LoggingConfiguration& config) override;
    ...
    LogLevel get_effective_level(
        std::string_view category,
        std::string_view plugin_id = {}) const override;

    // Output management
    qtplugin::expected<void, PluginError> add_output_handler(
        LogOutput output_type,
        std::unique_ptr<ILogOutputHandler> handler) override;

    qtplugin::expected<void, PluginError> remove_output_handler(
        LogOutput output_type) override;
    ...
    qtplugin::expected<void, PluginError> remove_filter(
        const std::string& filter_id) override;
    ...
    std::vector<LogEntry> get_recent_entries(
        size_t count,
        std::optional<LogLevel> level_filter = std::nullopt) const override;
...
private:
    ...
    // Output handling
    std::unordered_map<LogOutput, std::unique_ptr<ILogOutputHandler>>
        m_output_handlers;
 ...
 */
std::unique_ptr<ILoggingManager> create_logging_manager(
